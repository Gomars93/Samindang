...
## 2026-09-20 (최신 53): **부항·추나·도침·재활 정본 8개 완료** — 술기별 SSOT **7/7 완료**

**저장소 변경 없음.** 산출물은 임상지식베이스 문서 8개 + 볼트 INDEX v0.2이며, PO에게 직접 전달했다(볼트 경유).
PO 지시 "순차적으로 모두 부탁해요 저에게 묻지말고"에 따라 남은 4종을 연속 작성했다.

### 산출 (이번 라운드)

| 볼트 위치 | 파일 | 비고 |
|---|---|---|
| `09_치료술기/03_부항/` | `부항-SSOT.md` · `부항-근거매트릭스.md` | |
| `09_치료술기/04_추나/` | `추나-SSOT.md` · `추나-근거매트릭스.md` | **사실 오류 0건** |
| `09_치료술기/05_도침/` | `도침-SSOT.md` · `도침-근거매트릭스.md` | 정정 5 + **누락 근거 2편 편입** |
| `09_치료술기/07_재활/` | `재활-SSOT.md` · `재활-근거매트릭스.md` | 정정 1 + 보완 3. **§12 저장소 구현 대응표** |
| `09_치료술기/` | `00_INDEX.md` v0.2 | 7/7 REVIEWED, 술기별 정정 요약, 원장 실사 대기표 |

전 7종 상태: **`REVIEWED` 7 / `SSOT` 0.** 승격에는 각 문서의 실사 항목 처리 + 원장 승인이 필요하다.

### 이번 라운드에서 가장 중요한 발견 2건 ★

**1. 도침 — 딥리서치가 "없다"고 단정한 안전성 근거가 실제로 존재했다.**

DR_05 §11-6은 "시술당 분모를 가진 안전성 자료가 없다"를 연구 공백으로 선언했다. 그러나
한국 연구진의 **전향적 실제진료 안전성 pilot**(J Integr Med 2022;20(6):514-523, PMID 36050272)이
분모를 제공한다 — n=28, **전신 AE 8/258 세션(3.11%)**, **국소 AE 27/1,185 시술점(2.28%)**, 중대 AE 0건.

> **더 중요한 것은 그 연구가 발견한 연관 요인이다 — AE 발생이 `blade width`와 `시술점당 자입 자극 횟수`와 연관됐다.**
> 설문은 "임상의가 그렇게 생각한다"고 말했지만, 이 연구는 **전향적으로 측정**했다.
> → 도침 정본에서 **"무반응에 용량을 올리지 않는다"가 임상 추론이 아니라 관찰된 연관**이 됐다.

함께 누락돼 있던 **ACUPOCHECK Delphi**(J Integr Med 2024;22(5):579-587, PMID 39183098)도 편입했다 —
원내 도침 AE 기록 서식의 기준.

**2. 재활 — Hayden 2021 Cochrane의 하위군이 누락돼 있었다.**

DR_07은 이 Cochrane을 §2.1.1에서 인용하면서 **하위군 분석을 빠뜨렸다**:
**운동은 조언·교육 단독이나 전기치료보다는 낫지만, 도수치료와는 차이가 관찰되지 않았다.**

> 이 한 줄이 두 방향을 동시에 막는다 — **"추나가 운동보다 못하다"**도, **"운동이 추나보다 낫다"**도 근거가 없다.
> 정본은 둘을 **경쟁 관계가 아니라 층이 다른 도구**로 배치했다.

### 정정 목록 (이번 4종)

| 술기 | # | 내용 |
|---|---|---|
| 부항 | 1 | Cramer 2020의 **통증** vs sham SMD −0.27 — CI가 장애 수치와 동일하고 초록은 통증에 "no significant effects"만 제시 → **⛔ 인용 보류** |
| 추나 | 0 | **사실 오류 발견되지 않음.** 원자료가 분절 특이성 반증·audible pop 무관·"serious AE 0/7,518 ≠ 위험 0"을 스스로 먼저 배치 → 정본의 가치는 정정이 아니라 원내 규칙화 |
| 도침 | 5 | ① **Yoon 2022 누락**(위) ② **ACUPOCHECK 누락** ③ 설문 분모 `2/110` → **`2/107`**(185명 중 107명 응답) ④ 설문 AE 목록에서 **2위 피로 57.7% 누락** 복원 ⑤ 인용번호 2건(CTS 2025를 `[R11]`로, LSS 2026을 `[R18]`로 잘못 지목) |
| 재활 | 1+3 | ① **저자 오기** — PMID 32056828을 "May 2020"으로 기재했으나 제1저자는 **Runge N**. **DR_05는 Runge로 올바르게 기재**해 볼트 안에서 같은 논문이 두 이름으로 불리고 있었다 ② Hayden 하위군 보완(위) ③ n=24,486 보완 ④ 2025 painful-exercise 저자 결론 문장 보완 |

### 재활 정본이 다른 6종과 다른 점 — **§12 저장소 구현 대응표**

재활은 3층 본체이자 **이미 코드로 구현된 부분이 있는 유일한 술기**다. §12에 SSOT 항목 ↔ 저장소 위치를 매핑했다.

| SSOT 항목 | 구현 |
|---|---|
| §10-1 "1분 — Target task" | `LBP_15`/`LBP_16` (`src/spec/coreSpec.ts`) |
| picker prefill | `lbpTargetFunction.ts` · `DoctorWorkspace.tsx` |
| §10-2 재진 2번 "몇 회/몇 분/몇 kg까지 했습니까" | `achievedDose` (`revisitQuickCheck.ts`) |
| §10-2 재진 3번 "그날 밤과 다음날 아침은" | `nextDayRecovery` (같은 파일) |
| §4-2 "급성에 고강도 운동 강제 금지" | `LBP_STAGE_0_GUIDANCE_KO` (`lbpExerciseStage.ts`) |
| §2-1 목적 정의 · §9-2 금지 표현 | `docs/CLINICAL_OS_NORTH_STAR.md` |

**미구현 4건**도 같은 절에 기록: Load decomposition 7항목 구조화 입력 / §5-3 신호등의 UI 반영 /
"PSFS라 부르지 않기" UI 문구 확정 / 고관절·발목 region pack.

### 검수 방법의 한계 (7종 공통, 반복 기록)

**PubMed·Cochrane Library·Europe PMC 직접 접근이 egress로 차단**되어 모든 검증은 **WebSearch 기반
서지·초록 대조**다. **전문 대조가 아니다.** 근거매트릭스의 `✅ 확인`은 그 범위를 뜻한다.
이번 라운드에서 Cochrane Library·cochrane.org·research.vu.nl에 WebFetch를 시도했으나 모두 `EGRESS_BLOCKED`였고,
Hayden 2021의 두 비교군 값(−15.2/−6.8 vs −9.1/−4.1)은 plain-language summary의 "15 points ... compared to
people who had no treatment or usual care"로 **간접 확인**했다.

### 이 라운드가 남긴 PO 판단 항목

- **도침**: 시행 부위 범위(**경추 심부·흉부 인접 표적을 할 것인가**), 초음파 유도 필수 표적, 기본 용량(blade width·시술점 상한), ACUPOCHECK 기반 AE 서식
- **부항**: 습부항 시행 여부 · 위생 체계 실사
- **추나**: 경추 thrust 정책 · 회수 기준
- **재활**: target task 점수를 **"PSFS"라 부르지 않기로 확정**(UI 문구에 직결) · §5-3 신호등 채택 여부 · 원내 registry 여부

---

## 2026-09-20 (52): **침 정본 2개 완료** — 술기별 SSOT 2/6

**저장소 변경 없음.** 산출물은 임상지식베이스 문서 2개이며 PO에게 직접 전달했다(볼트 경유).

### 산출

`09_치료술기/01_침/` — `침-SSOT.md`(28KB) · `침-근거매트릭스.md`(16KB). 둘 다 `status: REVIEWED`.

### 검수에서 바로잡은 것 2건 ★ (둘 다 같은 문헌)

Busse 2023 (BMJ TMD NMA, PMID 38101924, 153 RCT / 8,713명) — **원문을 WebSearch로 재확인했다.**

| 딥리서치 원문 | 정정 |
|---|---|
| "침은 **신체기능** MID 달성 RD +42% (33~50)" | **42%는 「통증 완화」 기준이다.** 근거를 다른 결과지표로 옮긴 것이므로 바로잡았다 |
| "통증은 운동·CBT·동원술 근거가 더 강함" | **같은 NMA에서 도수치료 43% · 복합 43% · 턱 운동·동원술 36%.** 침 42%는 이들과 비슷하고 **턱 운동보다 낮지 않다** |

> **임상 배치는 바꾸지 않았다.** TMD에서 턱 운동·대처·수면 관리를 상위 축에 두는 판단은 유지하되, **이유를 "근거가 더 강해서"가 아니라 "지속성과 자기관리 가능성 때문"으로 바꿔 적었다.** 이 정정의 목적이 그것이다.

### 정본이 원자료보다 나아진 지점

- **비교군 열을 문서의 축으로 고정.** 같은 요통 Cochrane 안에서 **sham 대비 −9.22 / 무치료 대비 −20.32**다. 두 배 이상 차이 나는 숫자를 섞어 읽으면 안 된다 — 그 간극이 맥락효과의 크기다.
- **요통에서 "기능은 sham 대비 유의하지 않다"**(SMD −0.16, CI가 0 포함)를 명시. **통증 개선을 기능 개선으로 옮겨 적지 않기 위해서다.**
- **무릎 OA Tu 2021** — EA만 유의하고 **MA의 신뢰구간은 0을 포함**한다는 점, **EA 우월폭 하한이 0.2%p**라는 점을 표에 올렸다. 점추정 13%p만 인용하지 않는다.
- **외측상과통에서는 전침이 수기침보다 낫지 않다**(SMD −0.08, 유의하지 않음)를 별도 표시 — 무릎의 EA 우월을 다른 부위로 외삽하지 않기 위해.
- **안전성 3개 자료를 나란히 비교하지 않는다**는 읽기 규칙 신설. **한국 12.05% vs 일본 6.03%는 위험 차이가 아니라 능동 문진 여부의 차이**일 가능성이 크다.
- **어깨 하위군은 "숫자 인용 보류"로 표시** — 원문 표 미확인 상태를 감추지 않는다.

### 남은 것 (침 SSOT §10-2)

1. **어깨 하위군 효과크기** — Vickers 2018 IPD 원문 표 확인 필요. 현재 "전체 방향은 침 우세"까지만 확정
2. **원내 전침 프로토콜 고정** — 세션마다 주파수·강도를 바꾸지 않는 것이 반응 해석에 유리하다. **지금 고정된 프로토콜이 있는지 원장 확인 필요**
...
1. **침·약침 정본 볼트 배치**(PO 작업) — `01_침/`, `02_약침/`
2. **매선 v1.1 볼트 반영**(PO 작업) — `06_매선/` 2개 파일 덮어쓰기
3. **봉약침 응급 물품·동선 실사** — 약침 SSOT §10-2. **문서가 아니라 물건의 문제**
4. **다음 술기: 부항** — sham 대조에서 불확실성이 커 "무엇까지 말해도 되는지" 선을 긋는 값이 크다

---

## 2026-09-19 (51): **술기별 SSOT 착수 — 약침 정본 2개 작성 완료**

**저장소 변경 없음.** 산출물은 임상지식베이스 문서 2개이며 PO에게 직접 전달했다(볼트 경유).

### 왜 약침부터인가

6종(침·약침·부항·추나·도침·재활) 중 약침을 먼저 골랐다.

1. **제제가 많고 근거·안전·규제가 제제마다 다르다.** "약침"을 한 술기로 묶어 말하면 그 자체가 오류다 — 원내 기준이 없으면 매번 기억에 의존한다.
2. **딥리서치 7편 중 유일하게 검수에서 실제 결함이 나온 편이다.** 정본화 과정에서 그 결함을 고정해 넣지 않도록 관리가 필요했다.
3. **안전 비용이 가장 큰 술기다.** 봉약침 아나필락시스는 통합 추정 환자 2,200명당 1명이고, **피부검사가 이를 유의하게 낮추지 못한다.**

### 산출

`09_치료술기/02_약침/` — `약침-SSOT.md`(25KB) · `약침-근거매트릭스.md`(20KB). 둘 다 `status: REVIEWED`.

### 검수에서 바로잡은 것 2건 (둘 다 주장을 낮추는 방향)

| 딥리서치 원문 | 정정 |
|---|---|
| 80,523명 연구 "**연구 전체 환자 기준** 아나필락시스 0.019%" | **"연구 전체 환자 기준"은 원문에 없는 가필.** 3건/80,523 = 0.0037%로 산술이 안 맞는다. 0.019%의 분모는 봉약침 시술 하위군으로 보는 것이 타당 — 확정 전까지 "전체 환자 중 0.019%"로 인용하지 않는다 |
| 봉약침 항목 안에 "기흉 4건" | **기흉 4건은 침치료를 포함한 시술 전체 집계.** 원문이 §8.3에서 스스로 "침치료 후 발생"이라 적어 자기모순. 봉약침 고유 위험으로 귀속하지 않는다 |

### 정본이 원자료보다 나아진 지점

- **제제 열 + 비교군 열을 축으로 재구성.** saline 대조인지 물리치료 대조인지에 따라 같은 숫자가 다른 뜻이 된다.
- **saline은 inert placebo가 아니다**를 문서 전체의 전제로 고정.
- **"확인하지 못한 제제" 절 신설**(산삼·녹용·BU/BUM·NO·고장성 식염수) — "효과 없음"이 아니라 **"판단할 자료 없음"**으로 적는다.
- **"보고가 없다"를 "위험이 없다"로 쓰지 않는다**를 잠금 규칙에 명시.
- 포도당 전 행을 `[prolotherapy 외삽]`으로 고정. **회전근개는 2025 SR/MA가 부정적**이라 무릎 OA 신호와 분리.

### 남은 것 (약침 SSOT §10-2)

1. 자하거 TMD 시험의 **혈당 주입 용적** — 원문 프로토콜 확인 필요
2. **원내 실사용 제제 목록 확정** — 지금은 "쓸 수 있는 제제 전부"를 다룬다. 좁히면 훨씬 실용적이다. **원장 확인 필요**
3. **봉약침 응급 물품·동선 실사** — 문서가 아니라 **물건과 동선**의 문제다

### Next Recommended Action

1. **약침 정본 2개 볼트 배치**(PO 작업) — `삼인당 임상 지식베이스/09_치료술기/02_약침/`
2. **매선 v1.1 볼트 반영**(PO 작업) — `06_매선/` 2개 파일 덮어쓰기
3. **다음 술기 SSOT** — 침 → 부항 → 추나 → 도침 → 재활 순 추천
4. **클리닉 PC**: `git pull` → `npm run build`

---

## 2026-09-19 (50): **PR 2b 완료 — 목표 기능 chip 자동 선택.** 승인 범위의 저장소 작업이 모두 끝났다
...
| 커밋 | PR | 한 줄 |
|---|---|---|
| `c34d2b3` | **4** | 0단계 = "능동 운동 미처방" → **"구조화된 고부하 운동 유예"** |
| `c1ae585` | **1** | North Star에 Task–Load–Capacity 절 + 용어 가이드 + 금지 문장 5개 |
| `dd2b7a1` | **2** | 태블릿 `LBP_15`(목표 task 9개) + `LBP_16`(0~10) + 재질문 배관 |
| `cee142d` | **3** | 재진 간단 체크에 `achievedDose` + `nextDayRecovery`(진행 1차 게이트) |

### 이번에 계획에서 좁힌 것 2건 (둘 다 사유를 `DECISIONS.md`에 남김)

1. **PR 2의 picker 자동 선택 → PR 2b로 분리.** `emptyWorkspaceState()`는 인자를 받지 않고 `deserializeWorkspaceState`와 붙어 있어, 환자 답으로 chip을 미리 누르려면 **기존 기록을 덮어쓸 위험**이 있는 경계를 건드려야 한다. 대신 환자 답이 읽히는 경로(원장 화면 원답 목록 · 재진 비교 카드)는 PR 2에서 함께 연결했다.
2. **PR 3의 운동 중 통증 0~10 칸을 두지 않음.** 고정 허용치에 근거가 없고(DR_07 §1-6/§11-5), 난이도 신호는 `exerciseAdherence`가 이미 담는다. 숫자를 안 받으면 "GREEN ≤3 / YELLOW 4–5를 정책값 단서와 함께" 조건도 발동하지 않는다.

### 눈에 띄는 부작용 2건

- **문진이 두꺼워졌다.** 요통 프로필 23화면/46탭 → **25/50**(+추가상세 27/54, 남성 24/48). 고정값을 손으로 갱신했다 — 의도된 마찰.
- **`LBP_15`는 834×1194 세로에서 8px 넘친다.** 선택지 9개는 chip과 1:1이라 줄일 수 없어, `LBP_11`과 같은 방식으로 그 뷰포트에만 내부 스크롤을 허용했다. 800×1280 기준에서는 그대로 들어간다.

### Next Recommended Action

1. **PR 2b** — 목표 기능 chip을 `LBP_15` 답으로 미리 선택. 경계를 건드리므로 **필드 × 화면 표를 먼저** 쓴다.
2. **매선 v1.1 볼트 반영**(PO 작업) — `삼인당 임상 지식베이스/09_치료술기/06_매선/`에 `매선-SSOT.md`·`매선-근거매트릭스.md` 덮어쓰기. `매선-운영SOP.md`는 변경 없음.
3. **술기별 SSOT 6종** — 침·약침·부항·추나·도침·재활. 딥리서치 7편 검수는 끝났다.
4. **클리닉 PC**: `git pull` → `npm run build`, `.env`의 `VITE_SAMINDANG_SERVER_URL` 확인.

---

## 2026-09-19 (48): **PR 4 완료 — 0단계를 "능동 운동 미처방" → "구조화된 고부하 운동 유예"로 교정**

**브랜치**: `claude/google-drive-questionnaire-review-m3sf70`. `npm run build` 통과, `npm run test:all` **exit 0**.

같은 날 저녁 결정(DR_07 반영)의 첫 구현이자 **이 브랜치 첫 코드 변경**이다.

- **문서 먼저** — `docs/LBP_EXERCISE_STAGE_ASSIGNMENT_v0.4.md`에 §2-1 개정 절을 넣고 §2 안내문을 바꾼 뒤 소스를 고쳤다(`lbpExerciseStageTable.ts` 주석의 "표를 바꾸려면 문서를 먼저 바꾼다" 원칙).
- **바꾼 것은 문장 1개** — `LBP_STAGE_0_GUIDANCE_KO`. 배정 산술(`clamp(base − demotions, 0, 3)` + fear avoidance 상한)은 **그대로**다. DR_07이 "irritability로 게이팅하라"면서 **임계값은 주지 않기** 때문(§11-5).
- **라벨 `'0단계 · 보호/안정'`은 유지.** 바꾸면 파일럿 집계 테스트 3건이 함께 흔들리고 근거상 이득이 없다. 필요하면 별도 PR.
- 같은 정의를 되풀이하던 주석 6곳(목·어깨 팩 포함 — 둘 다 "요통 방식"이라고 명시해 LBP 정의를 상속한다)도 함께 갱신. **CLOSED 결정 기록은 손대지 않았다.**
- 필드 × 화면 표와 교체 경로별 테스트는 `DECISIONS.md` 같은 날 "PR 4 구현" 항목.

**다음**: PR 1(원칙 기록) → PR 2(태블릿 +2 문항) → PR 3(원장 Task Response 블록).

---

## 2026-09-19 (47): **통증 진료 모델을 Task–Load–Capacity로 전환 (PO 승인)** + 임상지식베이스 `09_치료술기` 신설 + 매선 정본 5→3 병합. **저장소 코드 변경 0건**

**브랜치**: `claude/google-drive-questionnaire-review-m3sf70`. 작업 트리 clean, 코드/스펙 변경 없음.
이번 세션 산출물은 전부 **Google Drive 임상지식베이스**에 있다. 저장소에는 이 기록과 DECISIONS 항목만 남긴다.

### 배경 — 외부 검토 문서 1건

PO가 외부(ChatGPT) 검토를 받아 「삼인당 통증 Clinical OS — 기능 중심 / Task–Load–Capacity 인수인계」 문서를 가져왔다.
요지: *구조 교정이 아니라, 실제 생활 기능을 제한하는 수정 가능한 병목을 줄이고 해당 task의 감당 부하(capacity)를 올리는 것이 목표다.*

### 한 것 1 — 검토 문서를 코드에 대고 검증 (수용 범위 확정)

검토 문서의 "핵심 변경 8개" 중 **절반은 이미 구현돼 있었다.** 코드로 확인한 사실:

- **구조 문항 0개** — `LBP_01~14`는 전부 안전·신경·CES·재발·파행·염증·예후 문항. 검토 문서가 "제외하라"고 한 골반 틀어짐·다리 길이 류 문항은 **애초에 없다.**
- **목표 기능은 이미 anchor** — `lbpTargetFunction.ts`의 chip 9개가 follow-up target·EMR·재진 배관에 연결돼 있다.
- **관찰/가설 이미 분리** — `directionalResponse`(관찰값)와 `workingHypothesis.supports`(가설)가 별도 저장.
- **fear avoidance는 격하가 아니라 상한** — `HIGH_FEAR_AVOIDANCE_MAX_STAGE = 2`. 검토 문서가 현재 규칙을 잘못 파악했다.
- **부위 팩 8개 이미 존재** — `regionPacks/`.

→ 결론: 철학은 수용, **환자 문항 7개 신설은 거절**(2026-09-06 "새 척도 0개" 결정 및 North Star 게이트 4와 충돌), 재발 격하 규칙은 **파일럿 데이터까지 보류**. 상세는 `DECISIONS.md` 같은 날 항목.

### 한 것 2 — 재활 라이브러리 점검 (결론: 라이브러리는 안 고친다)

PO 질문 "새 철학에 맞추려면 재활 라이브러리를 고칠 게 있나"에 대해 Core-20을 전수 확인했다.

- **고칠 것 없음.** `lbpExerciseCoreMetadata.ts` 20행이 이미 시작조건·허용반응·중단기준·후퇴·진행을 **"증상이 누적되지 않고 휴식 후 baseline으로 회복되는가"**라는 부하 반응 기준으로 서술한다. 진행 규칙도 "한 번에 한 요소만". 정책 문장 자체가 `진단명 단독이 아니라 기능·irritability·증상반응·목표·안전성`을 선언한다.
- **진짜 빈 곳 2개**:
  1. **재진에 "달성한 부하" 기록 칸이 없다.** `RevisitWorkspace`는 "이전에 채택한 운동" 제목만 보여준다. `progressionKo`는 데이터에만 있고 추천 모듈이 의도적으로 읽지 않는다(진행은 원장 판단). 그런데 그 판단을 적을 곳이 없다.
  2. **capacity 끝단이 얇다.** 3단계 운동이 20개 중 2개(`LBP_FUNC_05` 고관절 접기, `LBP_LOAD_02` 들어올리기)뿐. 카탈로그 57행의 loaded squat·carry·box lift는 Core 밖. 목표 기능 chip 9개에 **계단·달리기가 없다.**

→ `LBP_EXERCISE_STAGE_BY_ID` 주석의 "표를 바꾸려면 문서를 먼저 바꾼다" 원칙대로, Core 확장은 `docs/LBP_EXERCISE_LEVEL_DRAFT_v0.2.md` 개정이 선행이다.

### 한 것 3 — Google Drive `임상지식베이스/09_치료술기/` 신설

- 폴더 9개: `01_침` `02_약침` `03_부항` `04_추나` `05_도침` `06_매선` `07_재활` `_inbox` + `00_INDEX.md`
- 술기별 딥리서치 외주 프롬프트 8개를 `_inbox/`에 배치. 공통 블록 + 술기 블록이 합쳐진 **파일 하나 = 프롬프트 하나** 형태.
- 프롬프트 설계 핵심: 출력 12개 항목 고정, 그중 **「Task–Load–Capacity 상의 역할」과 「즉시 retest 규칙」을 필수 칸**으로 지정(이게 비면 효능 요약에 그친다). 국내 저널·KCI 명시. 국내 CPG 권고문 인용 금지.
- **딥리서치 7편 수령 완료(2026-09-19, 총 411KB).** `_inbox/`에 `DR_01_침_v1.md`~`DR_07_재활_v1.md`로 보관.
- **검수 7/7 완료.** 인용 검증은 WebSearch로 수행했다 — 이 환경의 egress가 PubMed·Europe PMC 직접 접근을 차단해 원문 fetch가 불가능하므로 **서지·초록 수준 대조**다. 03·04·05는 형식(12항목)·`[미확인]` 태그·"중국어 단독" 표시·NCKM 권고문 미인용을 모두 지켰고, 원문 대조 7건이 전부 실재했다. **결함 1건**: DR_03이 Cramer 2020의 *통증* sham SMD를 −0.27(−0.58~0.05)로 적었는데 초록은 그 값을 주지 않고 *장애* 값이 −0.26(−0.57~0.05)이다 — 전문 확인 전까지 인용 금지(결론 방향은 영향 없음).

### 한 것 4 — 매선 정본 5개 → 3개 병합 (PO 승인)

원내에 "정본"을 자처하는 매선 문서가 **두 계열 5개**로 흩어져 있었다. 실사 결과 **단일 승자가 없었다** — 5개가 각각 고유 항목을 갖고 있어, 하나만 남기면 **고유 항목 9개가 소실**된다.

- 산출: `09_치료술기/06_매선/`에 `매선-SSOT.md`(49KB) · `매선-근거매트릭스.md`(17KB) · `매선-운영SOP.md`(31KB).
- **원본 5개는 건드리지 않았다** — `임상지식베이스/`가 `.git`으로 로컬 Obsidian 볼트와 동기화 중이라 이동·수정 시 충돌 위험.
- 병합 중 **근거 충돌 3건**이 드러났고 전부 처리했다. 상세는 `DECISIONS.md`.
- **같은 날 오후, DR_06 수령으로 보류였던 충돌 2건(회전근개·무릎 OA)을 확정 해소했다.** 결과는 로컬 `매선-SSOT.md` v1.1 / `매선-근거매트릭스.md` v1.1에 반영됐다. ⚠️ **Drive 반영은 보류** — 이 세션의 Drive 커넥터 `create_file`이 mimetype을 무엇으로 지정하든(`text/markdown`·`text/plain`·`application/octet-stream` 전부 시도) 생성 결과를 **Google Docs로 변환**해 한글이 손상된다(프로브로 확인). Drive의 `06_매선/` 3개 파일은 **v1.0 상태 그대로 보존**돼 있고, v1.1 두 파일은 PO에게 직접 전달했다.

### 검증

저장소 변경이 없어 빌드·테스트 대상 없음. `git status` clean 확인.
Drive 산출물은 내부 상호 참조(3개 파일이 서로를 가리킴)와 출처 표(절별로 5개 원본 중 어디서 왔는지)로 자체 검증했다.

### Next Recommended Action

1. **딥리서치 7편 검수 마무리** — 형식 준수·의심 항목·검증 우선순위 PMID를 술기별로 확인 중. 이어서 술기별 SSOT + 근거매트릭스 작성.
2. **매선 미해결 4건 → 2건.** 회전근개(Park YC 2026, PMID 41496087)와 무릎 OA(Zhao 2025, PMID 41431000) **해소 완료**. 남은 2건 = ① DR_06 신규 근거를 §6까지 전면 반영 ② `_공통-안전관리.md` 소재 확인. 둘이 끝나고 PO 승인이 나면 `status: SSOT` 승격.
   - **배포 경로 확정: 볼트 경유.** PO가 로컬 Obsidian 볼트 `삼인당 임상 지식베이스/09_치료술기/06_매선/`에 `매선-SSOT.md`·`매선-근거매트릭스.md`(v1.1)를 덮어쓰고 Drive 동기화가 올린다. `매선-운영SOP.md`는 변경 없음. Drive의 현재 3개 파일은 v1.0 원본 그대로 보존돼 있다.
3. **저장소 PR 4건 중 PR 4 완료, 3건 남음.** DR_07을 반영해 계획을 수정했다(PO 승인). 상세는 `DECISIONS.md` 2026-09-19(저녁) 항목.
   - **PR 1 원칙 기록** — 금지 용어에 DR_07 §10.2 5개 추가. task-specific 정당화를 "우월"이 아니라 "용량·전이·성과측정 정렬"로 적는다(Desmeules 2021, PMID 33684362은 유의차 없음).
   - **PR 2 태블릿 +2 문항** — `LBP_16_TARGET_FUNCTION_SCORE`를 **PSFS라고 부르지 않는다**(MCID 1.2–1.3은 상지 유래, 요추 값 없음). 라벨·주석·EMR 어디에도 `PSFS` 문자열 금지 + 이를 단언하는 소스 테스트 1개. `questionnaire-volume` 요통 프로필 3개 고정값(현재 23화면/46탭) 수기 갱신은 그대로.
   - **PR 3 원장 Task Response 블록** — 진행 1차 게이트를 **"다음날 아침 회복"**으로. `0~4/10` 밴드 **금지**, 표시한다면 GREEN ≤3 / YELLOW 4–5 + "정책값, RCT 규칙 아님" 단서 동반.
   - **PR 4 — ✅ 완료(2026-09-19).** 위 항목 48 참조.
   - **코드 실사 결과**: 저장소에 `0~4/10`은 없다. 숫자로 된 통증 허용치는 `ankleFoot.ts` `AF_TEND_01`의 "10점 중 5 이하" 한 줄뿐이고, 이는 DR_07이 유일하게 출처를 인정한 건병증 모델(Silbernagel 2007) 맥락에 정확히 놓여 있다. **고칠 것 없음.**
4. **클리닉 PC**: 직전 세션 항목 그대로 — `git pull origin main` → `npm run build`, `.env`의 `VITE_SAMINDANG_SERVER_URL=http://<원장PC IP>:4317` 확인.

---

## 2026-09-08 (46): **문진 "없음" 최상단 부위 팩 34문항 확장 (v2.4 §22 후속)** — 이제 문진 전체 단일 규칙(none/NONE = index 0). PR #34 merge 완료
