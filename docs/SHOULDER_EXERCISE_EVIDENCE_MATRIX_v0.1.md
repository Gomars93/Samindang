...
| # | 결정 | 추천안 | 원장 표기 |
|---|---|---|---|
| 1 | 가설 패턴 7번째 `AC_LOCAL_CONTRIBUTION` 추가 여부 | **추가** — 원장 검사 스크립트에 수평내전(AC 자극) 검사가 이미 있어 걸 곳이 필요 | ☑ 확정(2026-09-07) — **정정**: 원문 §8에 `AC_OR_LOCAL_JOINT_CONTRIBUTION`이 처음부터 있었다(요약본이 6개로 줄여 옮김). 이 항목은 '추가'가 아니라 **원문 복원**이다(CLOSED §1-2). |
