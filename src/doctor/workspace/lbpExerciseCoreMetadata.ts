/**
 * LBP Exercise Core Metadata (LBP v1 Batch 2, G6).
 *
 * Ported verbatim (structure and every field byte-identical) from
 * `lbpExerciseCoreMetadata.v01.experimental.ts` on
 * `origin/claude/feat-lbp-action-adaptive-engine-prototype` (head `b099417`)
 * — Opus bounded validation (`docs/LBP_EXERCISE_ELIGIBILITY_OPUS_BOUNDED_VALIDATION_v0.1.md`
 * §4/§5) raised no findings against the metadata rows themselves; every
 * required fix (RF-4/5/6/7/7b/10) is a translation-layer fix in
 * `lbpExerciseEligibility.ts`, not a content change here. `progressionKo` on
 * every row below is READ NOWHERE in the v1 recommendation/adoption path
 * (`lbpExerciseRecommendation.ts`) — progression is a future-visit clinician
 * decision (RF-8), not something v1 hands to the patient.
 *
...
import { getLbpExerciseById } from './lbpExerciseLibrary'

export type LbpCoreExerciseMetadata = {
  exerciseId: string
  status: 'DRAFT_CLINICAL_METADATA'
  /**
   * Opus delta review defect 3: the catalog's `canonicalName`
   * (`lbpExerciseLibrary.ts`) stays English/Latin on 17/20 Core-20 rows —
   * it exists for ID/provenance fidelity with the source 57-item catalog,
   * not for patient display. This is the plain-Korean clinic name shown on
   * the candidate card title and in the adopted Care Plan text
   * (`lbpExerciseRecommendation.ts`). DRAFT naming for Product Owner
   * review — see that defect's report for the full list of 20.
   */
...
export const LBP_CORE_EXERCISE_METADATA: readonly LbpCoreExerciseMetadata[] = [
  ...
  row(
    ...
    ['새 신경증상', '뚜렷한 하지 원위부 증상 증가', '견디기 어려운 악화'],
  ...
  row(
    ...
    '골반을 조금만 들거나 짧게 등척성으로 유지하는 것으로 축소',
  ...
  row(
    ...
    '부하 제거, 시작 위치 높이기, 고관절 접기 기술 연습으로 회귀',
  ...
  row(
    ...
    '고관절·무릎·발목의 움직임 범위를 줄이고 한 관절의 움직임 폭을 최소화',
  ...
  row(
    ...
    '편안한 범위에서 3~5회 통제된 노출부터 시작하고 반응을 기록.',
