...
이 문서는 NECK_V1 임상 설계의 source-of-truth 초안이며, 아래 역할 분담을 따른다.
...
- safety threshold 검토
- 임상적 누락/과잉탐지
- 근거 오용, 과도한 진단 추론, 단일 검사 확진 표현 검출
- clinical decision sign-off

금지:
- 구현 세부를 임의로 재설계하지 않음
- clinical decision이 `CLOSED` 된 뒤 임의 변경 금지
- 근거가 불명확한 항목을 확정값으로 만들지 않음
...
## Fable — 장기 작업 리드 / 통합 감독
역할:
- 전체 repo 구조 파악
- 작업 분해 및 순서 결정
- Core / Tablet / Doctor / Sigma / module 간 통합
- 기존 기능 보존
- 회귀테스트 반복
- 장시간 작업 중 중간 실패를 수정하며 끝까지 완주

금지:
- CLOSED된 임상결정을 재해석하거나 변경하지 않음
- Core를 특정 모듈에 맞춰 갈아엎지 않음
- 임상근거가 없는 새 규칙을 만들어 넣지 않음

핵심 원칙:
> 공통 경험은 통합하고, 임상 도메인은 모듈로 분리한다.

---

## Sonnet — 구현 워커
역할:
- YAML / TypeScript / Python 코드 수정
- 테스트 작성
- UI 컴포넌트 구현
- field/enum mapping
- telemetry / payload / note generator 구현
- Fable과 Opus가 확정한 범위 안에서 반복 실행
...
- Opus가 `CLINICAL DECISION REQUIRED`를 내면 구현을 진행하지 않는다.
- 임상결정이 CLOSED 된 뒤에는 Fable/Sonnet이 임의 수정하지 않는다.
- 구현 후 typecheck/build/test/regression 중 하나라도 실패하면 `PASS` 선언 금지.
- `PASS / FROZEN`은 실제 repo 통합까지 검증된 경우에만 사용한다.
