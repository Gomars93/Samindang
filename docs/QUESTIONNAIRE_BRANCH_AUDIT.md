...
3. **Test coverage:** `STAFF_CHECK_TRIGGERS` urgent points (NECK_02/02A/03B/04) are asserted in section **I** (`I1`), and the NS01/F1 cross-module non-hiding invariant is heavily tested in sections **L/M**. ~~However there is no dedicated visibility-branch matrix...~~ **Closed as a follow-up in this same PR**: section **X** (`X-C1`–`X-C5`, `tests/integration.spec.mjs`) now covers NECK_02A's parent-positive gate, NECK_08's NECK_07-dependency, NECK_10A/11's NECK_10-dependency (including the deliberate difference between NECK_10A's widened and NECK_11's narrow gate), and NECK_12's chronic-onset gate, each with visible/hidden/stale-prune assertions — bringing NECK up to par with KNEE/ELBOW/WRIST_HAND/HIP/TMJ.
...
3. **Test coverage:** sections **L/M** give SHOULDER the *best*-tested cross-module invariant (F1: NS01 never hides SH0x/canonical-NECK) and urgent-trigger coverage (SH02/SH04/SH05) of any module in this audit. ~~There is, however, no dedicated visibility test for SH02/SH03's SH01-dependency or SH09A's SH09-dependency...~~ **Closed as a follow-up in this same PR**: section **Y** (`Y-C1`–`Y-C3`) adds SH02/SH03's SH01-dependency, SH09A's SH09-dependency, and a SH05-specific check that `shoulderSafetyStatus` stays URGENT via the `core_safety_already_urgent` passthrough even when SH05 itself is skipped by the F2 fail-safe (the existing M4 test only exercised this pattern for SH04).
...
3. **Test coverage: was the thinnest-tested module in the audit** (no lettered section, only a short ~15-line "AF core" block). **Closed as a follow-up in this same PR**: section **Z** (`Z-C1`–`Z-C5`, `Z-D1`–`Z-D2`) adds the full AF_03/AF_04/AF_05/AF_07 branch-visibility matrix (confirming the exact-YES gate against source rather than assuming it), a dedicated stale-prune test, and a real blank-`{}` walk across all 6 `AF_00` regions confirming every AF_0x question is actually reached. ANKLE_FOOT is now at parity with KNEE/ELBOW/WRIST_HAND/HIP/TMJ's test depth.
...
| ANKLE_FOOT (incl. `AF_00` router) | 9 | 4 | 1 | 0 | 1 | 3 | **Yes** — AF_03/04/05 exact-`'YES'` gating divergence from sibling modules (test-coverage gap closed as a same-PR follow-up, section Z) |
