...
## 2026-09-10 — 운곡본초학 ingestion: 원격 세션에서 "중간본 생산" 대신 "로컬 실행 툴체인"을 산출물로 택함

**결정자**: 구현 세션(Opus). PO 승인 대기 항목이 아니라, 환경 제약에서 나온 실행 판단이다.

### 사실 확인
운곡본초학 상·하 PDF(588MB / 517MB)는 사용자 Drive 에 존재하나, 이 원격 세션에서는 본문에
도달할 수 없다. `read_file_content` 가 두 권 모두 `{"fileContent": ""}` 를 반환하고(대조군인
같은 계정 소형 PDF 2건은 전문 정상 반환), 컨테이너는 `drive.google.com` 에 CONNECT 403,
`notebooklm` MCP 는 로컬 Windows 경로 바이너리다. `share_file` 은 특정 이메일 공유만 지원해
링크 공개 후 curl 우회도 불가능하며, 애초에 네트워크가 막혀 있다.

### 결정
1. **원서 없이 SOURCE 를 채우지 않는다.** 기존 하권 note 나 일반 본초학 지식으로 상권을
   추정 생성하는 선택지는 배제했다 — 기존 `00_본초_INDEX` 자체가 "상권이 없다고 추정하거나
   하권 내용으로 대신 채우지 않는다"고 못박고 있고, 임상 의사결정 지원 KB 에서 출처 위조는
   가장 비싼 실수다.
2. **산출물을 툴체인으로 전환한다.** 로컬 PC(원서 + vault 동거)에서 전수 실행 가능한
   `_tools/ungok/` 파이프라인과 회귀 테스트를 만들고, 현재 KB 실사 보고를 남긴다.
3. **툴체인의 안전 규칙을 산문이 아니라 테스트로 고정한다** — 근거 없는 페이지 번호 금지,
   OCR 교정 3중 게이트, 스키마 마이그레이션 완전성, 사람 작성 구역 보존, QA 실패 시 exit 1.

### 트레이드오프
- 얻는 것: 재현성(같은 입력 → 같은 출력), 검증 가능성(83개 테스트), 확장성(상권이 열리면
  같은 명령으로 처리), 그리고 **틀린 SOURCE 를 KB 에 심지 않는 것**.
- 잃는 것: 이번 세션에서 note 수가 늘지 않는다. 사용자가 직접 로컬에서 한 번 돌려야 한다.
- 숨은 리스크: 툴체인이 실제 원서 판면에서 표제/라벨을 얼마나 잘 잡는지는 **아직 실측되지
  않았다**. 합성 PDF 로만 검증됐다. 첫 로컬 실행의 MASTER INDEX 검출 수와 목차 항목 수 차이가
  그 실측치이며, 차이가 크면 `lib/boundaries.py` 의 임계값 조정이 첫 작업이 된다.

### CLAUDE.md "경로 교체 시 필드×화면 표" 규칙 적용
기존 KB 의 v2 frontmatter → 새 canonical 스키마 교체는 `_tools/ungok/lib/schema.py` 의
`LEGACY_FIELD_MAP` 에 **필드 1개당 목적지 1개**로 표를 적었고, 버리는 필드(`use`)는
`DELIBERATE_DROPS` 에 근거와 함께 남겼다. `tests/ungok/test_schema.py::
test_every_legacy_key_has_a_destination_or_documented_drop` 가 이 표의 완전성을 강제한다 —
새 필드가 표에 없으면 테스트가 깨진다.
