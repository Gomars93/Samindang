...
- **경로를 지우기 전에 그 경로를 쓰는 모든 화면을 확인한다.** 어떤 컨트롤·버튼·
  표시를 "중복"이나 "불필요"로 판단해 제거할 때는, 그 필드를 읽거나 쓰는 **모든**
  화면(초진 / 재진 / 한약 / mixed / fixture 미리보기)에 대체 경로가 실제로 남는지
  하나씩 확인하고, 그 확인 결과를 브리프나 커밋 메시지에 적는다. 이 규칙은 같은
  사고가 **세 번** 난 뒤에 추가됐다:
  - Batch 2.6 D-1 — 초진에서 중복이라 지운 `다음 방문 확인 사항`이 재진에는 대체
    칸이 없어, 보이지도 고쳐지지도 않는데 이어받기는 계속 그 필드에 썼다.
  - Batch 2.6 N-2 — 그 회귀를 고치는 수정이 mount latch를 잃어, 입력 도중 칸이
    사라지는 새 회귀를 만들었다.
  - Batch 4 D-1/D-2 — EMR 복사를 한 곳으로 모으면서 한약 진료는 빈 값을 복사하고
    "복사됨"을 띄웠고, 원장이 직접 타이핑한 판단 3필드가 출력에서 사라졌다.
  세 번 다 "지운 쪽 화면에서는 옳았다". 확인해야 하는 것은 **지우지 않은 쪽 화면**이다.
