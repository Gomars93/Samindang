 ...
 * 그래서 문진 길이는 그대로다.
 *
 * 닿는 화면 (2026-09-25 최종): **mixed 진료** · **재진 화면**
 * (`RevisitWorkspace`) · **한약 단독 진료**.
 *
 * 경위: PR-A(2026-09-21, PO 승인)가 herbal 단독의 `다음` 레인을 통째로
 * 폐기하면서 그 안의 picker까지 사라졌고, 그래서 이 집합이 한약 단독에는
 * 닿지 않았다(PR #56 검수 F5가 그 사실을 짚었다). 같은 날 PO 지시로 **그 한
 * 칸만** 되살렸다 -- 판단·처치 레인의 `HerbalFollowUpTargetsCard`. 레인의
 * 나머지는 여전히 없다.
 *
 * tests/doctor-workspace.spec.mjs가 세 갈래로 고정한다: mixed 경로(NRS
 * 스위치가 HerbalWorkspaceNext 안) · 한약 단독 경로(그 카드가 같은 집합을
 * 넘긴다) · PR-A 잔여 계약(레인의 나머지는 없다).
 *
