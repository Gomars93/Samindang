# ELBOW_V1 — Tablet Question Set v0.1.1

작성일: 2026-08-25
상태: **DRAFT — Opus v0.2 재검수의 기계적 불일치 2건 반영 / 최종 Opus 검증 필요**
대상: 삼인당 Clinical OS — MSK Elbow module

상위 근거:
- `ELBOW_V1_Evidence_Matrix_v0.1_HANDOFF.md`
- `ELBOW_V1_Opus_Clinical_Review_v0.1.md`
- `ELBOW_V1_Tablet_Question_Set_v0.1.md`
- `ELBOW_V1_Opus_Clinical_Review_v0.2.md`

이 문서는 v0.1 문서에서 Opus v0.2 재검수가 발견한 **섹션 간 내부 불일치 2건만** 수정한다. E1–E10 clinical tier, 문항 구성, safety threshold는 v0.1과 동일하며 재해석하지 않았다. **아직 CLINICAL DECISIONS CLOSED가 아니다.** 다음 단계는 Opus 최종 검증이다.

## v0.1 → v0.1.1 변경사항 (Opus v0.2가 요구한 것 외 변경 없음)

1. **§11 `expedited_referral_consider`**: "`ELBOW_09 == YES` + `ELBOW_09A` UNKNOWN/invalid/missing" 분기를 명시적으로 추가했다. 이 조건은 §5가 이미 결정했던 semantics이며(v0.1 §5 L309 그대로), `neuro_assessment_required`가 이미 올바르게 포함하고 있던 것과 동일한 조건을 `expedited_referral_consider`에도 맞춘 것뿐이다.
2. **§10 `ELBOW_02`/`ELBOW_02A`**: REVIEW_REQUIRED 요약 bullet을 다른 9개 항목과 동일하게 "UNKNOWN/invalid/missing"으로 명시했다(기존 "UNKNOWN/invalid"에서 "missing" 추가). §3의 개별 semantics는 원래부터 정확했으므로 실제 계산 로직은 바뀌지 않는다 — 요약표 표기만 통일했다.
...
- **ELBOW_09 == YES + ELBOW_09A UNKNOWN/invalid/missing**(v0.1.1 추가 — §5 Semantics가 이미 결정했던 조건을 여기 반영. 진행 여부가 불확실하면 배제 불가로 fail-closed, `neuro_assessment_required`와 동일한 조건)

missing(순수 무응답)은 safety REVIEW를 만들지만 위 flag를 임의로 true로 만들지 않는다(ELBOW_04/05/06 자체 missing 시). **단 ELBOW_09A의 missing/UNKNOWN/malformed는 예외다** — ELBOW_09(게이트)가 YES로 확정된 상태에서 후속 진행 여부만 불확실한 경우이므로, 위 목록대로 `expedited_referral_consider`를 true로 fail-closed 처리한다.
...
# 18. Opus Final Verification Questions (v0.1.1)

이번 최종 검증은 새 임상 쟁점을 열지 않는다. v0.2가 요구한 2건만 확인한다.

1. `ELBOW_09 == YES` + `ELBOW_09A` UNKNOWN/invalid/missing이 `REVIEW_REQUIRED` + `neuro_assessment_required=true` + `expedited_referral_consider=true` 셋 다 문서 전체(§5/§10/§11)에서 일관되게 나오는가?
2. `ELBOW_02`/`ELBOW_02A`의 UNKNOWN/invalid/missing fail-closed semantics가 §3(개별 문항)과 §10(Safety Engine 요약) 양쪽에서 일치하는가?
...
> **Opus final verification of ELBOW_V1 Tablet Question Set v0.1.1 (2건만 확인)**

이 문서는 CLINICAL DECISIONS CLOSED가 아니다. Fable 통합계획·TypeScript/UI/테스트 구현·production code 수정은 Opus 최종 검증 PASS 이후에만 진행한다.
