...
function applyNeutralResult(input, checkId) {
  ...
  switch (checkId) {
    ...
    case 'LBP_CHECK_LUMBAR_MOVEMENT_RESPONSE':
      ...
      break
    default:
      throw new Error(`No neutral resolution mapping for ${checkId}`)
  }
  return next
}
    ...
    for (const walkingStandingLegPattern of cueValues) {
      for (const hipContributionCue of cueValues) {
        for (const sijContributionCue of cueValues) {
          ...
          // Domain isolation: an uncertain cue alone must not be promoted to a
          // positive cue. Only explicit PRESENT or clinician override may open
          // that domain-specific exam.
          if (hipContributionCue !== 'PRESENT') {
            assert.ok(!ids(output).includes('LBP_CHECK_HIP_CONTRIBUTION'), `${label}: hip uncertainty/absence auto-promoted`)
          }
          if (sijContributionCue !== 'PRESENT') {
            assert.ok(!ids(output).includes('LBP_CHECK_SIJ_CONTRIBUTION'), `${label}: SIJ uncertainty/absence auto-promoted`)
          }
          if (radicularCue !== 'PRESENT') {
            assert.ok(!ids(output).includes('LBP_CHECK_NEURODYNAMIC'), `${label}: radicular uncertainty/absence auto-promoted`)
          }
          if (walkingStandingLegPattern !== 'PRESENT') {
            assert.ok(!ids(output).includes('LBP_CHECK_WALKING_TOLERANCE'), `${label}: walking uncertainty/absence auto-promoted`)
          }

          // Resolving any currently suggested check to a neutral result must
          // remove that exact check and must not create a new unrelated branch.
          for (const checkId of ids(output)) {
            ...
            assert.ok(
              resolvedOutput.checks.length <= output.checks.length - 1,
              `${label}: resolving ${checkId} did not reduce unresolved check burden`,
            )
            pairwiseResolutionChecks += 1
          }

          // Sequentially resolve the highest-priority candidate. The engine
          // must converge without loops or same-check resurrection.
          let current = input
          const seen = new Set()
          let steps = 0
          while (true) {
...
assert.deepEqual(
  ...
  'candidate-burden baseline changed: review whether this is a deliberate reduction or a new cascade',
)
...
// A naive deduper based only on the coarse `changesManagement` categories is
// unsafe. In the all-cue case, some clinically distinct checks add no NEW
// top-level category after earlier checks have already covered that category.
// Therefore category-set coverage alone cannot decide that they are redundant.
const allCue = evaluateLbpActionAdaptiveExperiment(
  scenario({
    legSymptoms: 'PRESENT',
...
// Clinician override matrix: every subset of manual concerns must open exactly
// the requested domains (when unassessed) and no unrelated domain.
...
// Same-visit terminal-state suppression matrix. NOT_PERFORMED/LIMITED are
// explicitly non-negative results and must not be re-suggested in the same
// visit merely because the cue remains present.
const terminalCases = [
...
// STALE-SCOPE NEGATIVE CONTROL.
// Current experimental context does not carry a visit key/provenance for exam
// statuses. Therefore the engine cannot distinguish "NOT_PERFORMED this visit"
// from "NOT_PERFORMED on the previous visit" if a caller accidentally carries
// that status forward. Keep this observation executable so production wiring
// cannot forget to solve it at the adapter/state boundary.
const sameVisitSuppressed = evaluateLbpActionAdaptiveExperiment(
  scenario({ visitKind: 'INITIAL', legSymptoms: 'PRESENT', objectiveNeuro: 'NOT_PERFORMED' }),
)
const followUpSuppressed = evaluateLbpActionAdaptiveExperiment(
  scenario({ visitKind: 'FOLLOW_UP', legSymptoms: 'PRESENT', objectiveNeuro: 'NOT_PERFORMED' }),
)
...
// Safety stress: every cue combination must be unable to leak routine checks or
// rehab/treatment action tags when disease safety is not CLEAR.
let safetyCases = 0
...
// Treatment-safety stress: unlike disease safety, treatment-safety review must
// not erase assessment candidates; it only marks finalization for clinician review.
