...
## 2026-09-20 — 술기별 정본 **7/7 완료**. 딥리서치가 "근거 공백"이라 선언한 항목을 **그대로 믿지 않기로** 한 것이 이번 라운드의 판단

**결정자**: Gomars93(원장, PO) — "순차적으로 모두 부탁해요 저에게 묻지말고".
남은 4종(부항·추나·도침·재활)을 중간 확인 없이 연속 작성하도록 위임받았다.

**저장소 코드 변경 없음.** 산출물은 볼트 문서 8개 + `09_치료술기/00_INDEX.md` v0.2.

### 결정 1 — **딥리서치의 "근거 공백" 선언을 검증 대상으로 취급한다**

도침 DR_05는 §11-6에서 **"시술당 분모를 가진 안전성 자료가 없다"**를 연구 공백으로 선언했다.
이전 6편에서는 *기재된 수치*만 검증했지, *"없다"는 진술*은 검증하지 않았다. 이번에 검증했고 **틀렸다.**

| 실제 | 출처 |
|---|---|
| n=28, 261 세션 중 258 세션(1,185 시술점) 추적 | Yoon SH, Kwon CY, Jo HG, Sul JU, Lee H, Won J, Jeong SJ, Lee JH, Leem J. *Safety of acupotomy in a real-world setting: A prospective pilot and feasibility study.* J Integr Med 2022;20(6):514-523. PMID **36050272** |
| 전신 AE **8/258 세션 = 3.11%** · 국소 AE **27/1,185 시술점 = 2.28%** · 중대 AE **0건** | 〃 |
| **AE 발생이 `blade width` 및 `시술점당 자입 자극 횟수`와 연관** | 〃 |

> **이 마지막 줄이 도침 정본의 성격을 바꿨다.**
> 한국 한의사 설문(2023)은 임상의가 **깊이·blade width·조작량을 위험요인이라고 "생각한다"**고 보고했다.
> 이 연구는 **실제 AE 발생이 그 변수들과 연관됐다고 측정했다.**
> → 정본 §5(최소 유효 침습 용량)와 §1-5 #2("무반응에 용량을 올리지 않는다")의 근거가
> **"임상 추론"에서 "관찰된 연관"으로 한 단계 올라갔다.**

함께 누락돼 있던 **ACUPOCHECK**(도침 AE 표준 체크리스트, Delphi 합의, J Integr Med 2024;22(5):579-587,
PMID **39183098**)도 편입해 원내 AE 기록 서식의 기준으로 지정했다.

**규칙화:** 앞으로 딥리서치가 "~에 대한 자료가 없다 / 연구가 없다"고 쓴 항목은
**검증 없이 정본에 옮기지 않는다.** 근거의 부재를 주장하는 문장도 근거가 필요하다.

### 결정 2 — 재활 정본에 **저장소 구현 대응표(§12)**를 둔다

재활은 7종 중 **이미 코드에 반영된 부분이 있는 유일한 술기**다(PR 1~4, 2b).
임상 문서와 코드가 따로 흘러가면 둘 다 stale해지므로, **SSOT 항목 ↔ 저장소 위치**를 한 표에 고정했다.

| SSOT 항목 | 구현 위치 |
|---|---|
| §10-1 "1분 — Target task" | `src/spec/coreSpec.ts` — `LBP_15`(`lbp_target_task`) · `LBP_16`(`lbp_target_function_score`) |
| picker prefill | `src/doctor/workspace/lbpTargetFunction.ts` · `DoctorWorkspace.tsx` (`seedWorkspaceState`) |
| §10-2 재진 2번 "몇 회/몇 분/몇 kg까지 했습니까" | `revisitQuickCheck.ts` — `achievedDose` |
| §10-2 재진 3번 "그날 밤과 다음날 아침은" · §5-3 신호등 판정축 | 〃 — `nextDayRecovery` |
| §4-2 "급성에 고강도 운동 강제 금지" | `lbpExerciseStage.ts` — `LBP_STAGE_0_GUIDANCE_KO` |
| §2-1 목적 정의 · §9-2 금지 표현 | `docs/CLINICAL_OS_NORTH_STAR.md` |

**미구현 4건**도 같은 절에 명시했다 — Load decomposition 구조화 입력 / §5-3 신호등 UI 반영 /
"PSFS라 부르지 않기" UI 문구 / 고관절·발목 region pack.
**다만 §11-3의 PO 판단 3건이 먼저다** — 특히 PSFS 명명은 UI 문구에 직결된다.

### 결정 3 — 재활: **우리 점수를 "PSFS"라 부르지 않는다**

PSFS의 COSMIN SR(57편)은 근골격계에서 신뢰도·반응성은 충분하지만
**construct validity가 불충분**(일반적 physical function을 완전히 대표하지 않음)하다고 보고했다.
더 중요하게, **원 PSFS는 복수의 환자선택 활동을 쓴다.** 우리가 쓰는 `task 1개 + 0–10`은 **검증되지 않은 단일항목 변형**이다.

> **따라서 원내에서는 `target task 수행점수`라 부르고, PSFS의 MID 1.2 / MCIC 1.29를 빌려 쓰지 않는다.**
> "1.2점 이상 올랐으니 임상적으로 의미 있는 변화"라는 문장을 쓰지 않는다.
> 이 규칙은 정본 §11-4 잠금 규칙에 들어갔고, **UI 문구 확정이 PO 판단 대기 항목**이다.

### 결정 4 — 재활: **비교군 없이 효과크기를 인용하지 않는다**

Hayden 2021 Cochrane(249 trials / 24,486명)은 **같은 리뷰 안에서 비교군에 따라 두 배 가까이 다른 값**을 낸다.

| 비교군 | 통증 | 기능제한 |
|---|---|---|
| 무치료·통상치료·placebo | MD −15.2 (−18.3 ~ −12.2), **MCID 15에 근접** | MD −6.8 (−8.3 ~ −5.3), MCID 10 미달 |
| 다른 보존치료 | MD −9.1 | MD −4.1 — **둘 다 MCID 미달** |

−9.1만 인용하면 "운동은 별것 아니다"로 오독되고, −15.2만 인용하면 과대평가된다.
**정본 §1-3은 두 쌍을 나란히 싣고, §11-4에 "비교군 없이 효과크기를 인용하는 것을 금지한다"를 잠금 규칙으로 넣었다.**

### 결정 5 — **추나 정본은 "정정 0건"임을 그대로 적는다**

DR_04에서는 사실 오류가 발견되지 않았다. 원자료가 자기 술기에 불리한 근거를 스스로 먼저 배치했다 —
분절 특이성 반증, audible pop 무관, "serious AE 0/7,518 ≠ 위험 0".

> 검수 문서에 "정정 사항 없음"을 적는 것은 검수를 안 한 것처럼 보일 위험이 있다.
> 그래도 **그대로 적는다.** 정정 건수를 검수의 성과 지표로 삼으면 없는 오류를 만들어내게 된다.
> 대신 **이 정본의 가치는 "정정"이 아니라 「원내 규칙화」**라고 문서 §0에 명시했다.
...
| 추나 | 0 | (위) |
| 도침 | 5 | ① Yoon 2022 누락 ② ACUPOCHECK 누락 ③ 설문 분모 `2/110` → `2/107` ④ 설문 AE 목록 **2위 피로 57.7%** 누락 복원 ⑤ 인용번호 2건 |
| 재활 | 1+3 | ① **PMID 32056828의 제1저자는 May가 아니라 Runge** — DR_05는 Runge로 올바르게 썼으므로 **볼트 안에서 같은 논문이 두 이름으로 불리고 있었다.** Runge 2020으로 통일 ② Hayden 하위군 보완 ③ n=24,486 ④ 2025 painful-exercise 저자 결론 |

### 검수 방법의 한계 — 7종 전체에 공통

**PubMed·Cochrane Library·Europe PMC 직접 접근이 egress로 차단**되어 있다.
모든 검증은 **WebSearch 기반 서지·초록 대조이며 전문 대조가 아니다.**
이번 라운드에서 `cochranelibrary.com` · `cochrane.org` · `research.vu.nl`에 WebFetch를 시도했으나
전부 `EGRESS_BLOCKED`였고, Hayden 2021의 두 비교군 값은 plain-language summary를 통해 **간접 확인**했다.
각 근거매트릭스의 `✅ 확인`은 이 범위를 뜻한다 — 이 한계를 정본에서 지우지 않는다.

### 다음 — 원장 실사 대기 (술기 횡단)

| 술기 | 항목 |
|---|---|
| 도침 | **시행 부위 범위(경추 심부·흉부 인접 표적 여부)** · 초음파 유도 필수 표적 · 기본 용량(blade width·시술점 상한) · ACUPOCHECK 기반 AE 서식 |
| 부항 | 습부항 시행 여부 · 위생 체계 |
| 추나 | 경추 thrust 정책 · 회수 기준 |
| 재활 | **"PSFS"라 부르지 않기 UI 문구 확정** · §5-3 신호등 채택 여부 · 원내 registry 여부 |
| 침·약침 | 전침 프로토콜 · 실사용 제제 목록 · 봉약침 응급 물품·동선 |
| 공통 | 어깨 하위군 효과크기(Vickers 2018 원문 표) |

**7종 모두 `REVIEWED`이며 `SSOT`는 0종이다.** 승격은 위 실사 처리 + 원장 승인 후.
...
### PR 1 — `CLINICAL_OS_NORTH_STAR.md`에 Task–Load–Capacity 기록 (`c1ae585`)

canonical 문서에 절 하나를 신설했다. 3층 배치표(1층 침·약침·부항 / 2층 추나·도침·매선 / 3층 재활), 용어 가이드(쓰지 않는다 ↔ 대신 쓴다) 5행, DR_07 §10.2 금지 문장 5개, task-specific 정당화 방식, 통증 허용치를 숫자로 고정하지 않는다는 원칙.

**이 절이 스스로 막는 것 2가지** — ① "새 척도를 늘리는 방향이 아니다"를 절 안에 명시해 기능 추가 Gate 4와 2026-09-06 "새 척도 0개" 결정이 그대로 살아 있게 했다. ② task-specific loading을 **"일반 운동보다 우월"이라 쓰지 않는다** — Desmeules 2021(PMID 33684362)의 18 RCT / n=1,719에서 유의차가 없다. 정당화 근거는 우월성이 아니라 용량·전이·성과측정 정렬이다.

### PR 2 — 태블릿 `LBP_15`(목표 task) + `LBP_16`(목표 기능 점수) (`dd2b7a1`)

| | 내용 |
|---|---|
| `LBP_15` | `lbp_target_task`, 단일 선택 9개. 원장 화면 목표 기능 chip `lbp_tf_*` 9개와 **1:1** |
| `LBP_16` | `lbp_target_function_score`, 0~10 척도. **`LBP_15`를 답했을 때만** 보인다 — 빈 목표 task에 점수를 매기지 않는다 |
| 배관 | payload `safety_flags.lbp` 2필드 · 재질문 표(server + 팩) · 초진 답 경로(`detailCheckBaseline.ts`) · 원장 화면 원답 목록(`DoctorView.tsx`) |

**PSFS라고 부르지 않는다 (가드 3개).** PSFS는 3항목 평균이고 보고된 SEM 0.35~1.5 / MID 1.2~1.3은 **어깨·상지 연구 값**이다(DR_07 §1-4). 요추 MCID 값은 존재하지 않는다. `questionnaire-volume`에 ① 환자 노출 문구 검사 ② 목표 기능 경로 소스의 **문자열 리터럴** 검사(설명하는 주석은 허용) ③ 가드가 실제로 잡는지 보는 **양성 대조**를 넣었다. ③이 없으면 정규식이 조용히 아무것도 안 잡아도 초록이다.

**문진이 두꺼워졌음을 숫자로 인정했다.** 요통 프로필 3개 고정값: 23화면/46탭 → 25/50, 25/50 → 27/54, 22/44 → 24/48.

**레이아웃 부작용 1건.** `LBP_15`(선택지 9개)는 834×1194 세로에서 858px로 가용 850px를 **8px** 넘는다. 선택지를 줄이면 chip 1:1이 깨지므로, 이미 같은 이유로 올라가 있는 `LBP_11`(10개)과 동일하게 **이 뷰포트에만** 내부 스크롤을 허용했다. 800×1280 기준 뷰포트에서는 그대로 들어간다.

**테스트 하네스 구멍 1개.** `integration.spec.mjs`의 walker 2개가 `numeric_scale`을 처음 만나 터졌다 — `LBP_12`는 만성 게이트라 walker가 도달하지 않았기 때문이다. 결정적 walker는 척도 최솟값, "가장 양성인 답" walker는 최댓값을 쓰도록 채웠다.

**계획에서 좁힌 것 — picker 자동 선택은 하지 않았다.** 원장 화면 chip을 환자 답으로 미리 눌러두려면 `emptyWorkspaceState()`에 payload를 물려야 하는데, 이 함수는 인자를 받지 않고 `deserializeWorkspaceState` 경로와 붙어 있어 **기존 기록을 덮어쓸 위험**이 있다. CLAUDE.md가 네 번 기록한 사고 유형이라 **별도 PR(2b)로 분리**했다. 대신 환자 답이 "쓰기는 되는데 읽히지 않는 필드"가 되지 않도록 원장 화면 원답 목록과 재진 비교 카드 경로를 이 PR에서 함께 연결했다.

### PR 3 — 재진 간단 체크에 "해낸 부하" + "다음날 아침 회복" (`cee142d`)

승인된 계획은 `regionClinicalState`에 필드를 추가하는 것이었으나 **`RevisitQuickCheck` 확장으로 바꿨다.** 사유:

- 메우려는 빈 곳이 **재진** 쪽이다("재진에 달성한 부하 기록 칸이 없다"). `regionClinicalState`는 초진 `WorkspaceState`를 위한 어댑터다.
- 요통은 옛 필드 3개를 쓰고 다른 부위는 맵을 쓰는 **이중 모양**이라, 4번째 값을 넣으려면 최상위 `lbpTaskResponse`를 새로 만들어야 한다. `RevisitQuickCheck`는 이미 모듈·정화기·카드·EMR 경로·테스트를 갖춘 **단일 모양**이다.
- 결과물(달성 부하 + 진행 판단 근거가 기록되고 EMR에 실린다)은 같고 표면적이 훨씬 작다.

| 필드 | 값 |
|---|---|
| `achievedDose` | 자유 입력 한 줄. **어떤 규칙 엔진도 읽지 않는다**(진행은 원장 판단) |
| `nextDayRecovery` | `RECOVERED` / `LINGERED` / `WORSE_NEXT_DAY`. **진행 판단의 1차 게이트** |

안내는 칩→문장 직접 대응 하나뿐이고 점수·가중치를 만들지 않는다(이 파일의 기존 원칙). 칩을 누르지 않았으면 아무 줄도 만들지 않는다 — **NOT_ASSESSED를 '회복'으로 읽지 않는다.** 기존 rule 7("유지·진행 가능")이 5칩만으로 계속 발동하는지도 단언으로 고정했다.

**계획에서 좁힌 것 — 운동 중 통증 0~10 칸을 두지 않았다.** ① `0~4/10` 같은 고정 허용치는 근거가 없다(DR_07 §1-6, §11-5). ② 난이도 신호는 이미 `exerciseAdherence`의 '너무 어려움'/'너무 쉬움'이 담는다. ③ North Star 기능 추가 Gate 3. 숫자를 받지 않으면 "GREEN ≤3 / YELLOW 4–5 밴드를 정책값 단서와 함께 띄운다"는 조건부 요구도 발동하지 않는다 — **띄울 숫자 자체가 없다.**

**읽히는 경로** — `summarizeRevisitQuickCheckKo`가 두 값을 실어 `emrPreview`의 재진 O/S "경과 요약" 줄에 도달한다.

**정화·역호환** — 두 필드가 없는 옛 저장본이 정화를 거치지 않고 요약 함수로 직접 들어와도 죽지 않도록 valid 검사와 타입 검사를 함께 걸었다(실제로 기존 테스트 픽스처가 이 경로로 터졌다).

**금지 표현 가드** — 이 카드가 원장에게 보여주는 모든 문장에 대해 통증 허용치 숫자 고정 문구와 DR_07 §10.2 금지 문장 5개가 없음을 단언한다.

### 검증

네 PR 모두 각각 `npm run build` + `npm run test:all` **exit 0** 확인 후 커밋했다.

### 남은 것

- **PR 2b** — 원장 화면 목표 기능 chip을 환자 답(`LBP_15`)으로 미리 선택. `emptyWorkspaceState`/`deserializeWorkspaceState` 경계를 건드리므로 필드 × 화면 표를 먼저 쓴다.
- **LBP_15 9개 ↔ chip 9개 1:1 확장 절차** — 계단·달리기를 넣으려면 `docs/LBP_EXERCISE_LEVEL_DRAFT_v0.2.md` 개정 → Core 확장 → 분포 고정 테스트 → 양쪽 9개를 함께 10개로. 한쪽만 늘리면 매핑이 깨진다(테스트가 막는다).
- **매선 v1.1 볼트 반영**(PO 작업) · **술기별 SSOT 6종**(침·약침·부항·추나·도침·재활).

---

## 2026-09-19 (PR 4 구현) — 0단계를 "능동 운동 미처방"에서 **"구조화된 고부하 운동 유예"**로 교정

**결정자**: Gomars93(원장, PO) — "PR 4부터 진행해줘". 같은 날 저녁 결정(DR_07 반영)의 첫 구현.
**브랜치**: `claude/google-drive-questionnaire-review-m3sf70` (지정 브랜치. 논리적 작업 단위로는 별도 브랜치가 맞지만 세션 지정이 우선한다).

### 무엇을 바꿨나

`LBP_STAGE_0_GUIDANCE_KO` 문장 1개와, 그 문장의 의미를 되풀이하던 주석·문서.
**배정 산술은 건드리지 않았다.**

| | 전 | 후 |
|---|---|---|
| 안내문 | "오늘은 **능동 운동을 처방하지 않습니다.** 통증이 없는 자세…와 편안한 호흡, 통증 없는 범위의 자발적 움직임만 안내하고 다음 진료에서 다시 평가합니다." | "오늘은 **구조화된 고부하 운동을 유예합니다. 걷기·일상활동과 통증 없는 범위의 움직임은 그대로 유지**하시고, 편안한 자세(눕기·무릎 세우고 눕기)와 호흡을 함께 안내합니다. **다음 진료에서 자극 민감도와 위험신호를 다시 확인해** 운동 시작 시점을 정합니다." |

**근거** — `DR_07_재활_v1.md` §11-9: "비특이적 급성 요통은 장기 bed rest보다 active approach가 타당하지만, 고강도 structured exercise가 초기에 필수라는 근거도 약하다. '3일/1주 무조건 쉰다' 같은 고정 기간보다 **irritability와 red flag에 따라** 조절해야 한다."

근거가 지지하는 것은 **"고강도 구조화 운동의 초기 추가이득이 불확실하다"**이지 **"능동 움직임을 중단한다"**가 아니다. 특히 `STAGE_0_BLOCKED_MESSAGE_KO`에서 `0단계(보호/안정) 확정 — 오늘은 능동 운동을 처방하지 않습니다`로 이어 붙으면 **"눕혀두라"로 읽힌다.** 문장 안에 탈출 조건(자극 민감도·위험신호 재확인)이 없던 것도 함께 고쳤다.

### CLAUDE.md "제거 또는 교체" 표 — 필드 × 화면

교체한 경로는 `LBP_STAGE_0_GUIDANCE_KO` 상수 1개다.

**(1) 출력 방향** — 이 상수가 나르던 값이 도달하는 곳 전부:

| 도달 경로 | 화면 | 교체 후 |
|---|---|---|
| `suggestLbpExerciseStage().guidance` → `PainWorkspace.tsx:493` (`showGuidance`) | 원장 화면 · 운동 단계 카드 | **같은 자리, 새 문장.** 렌더 단언을 새 문구로 갱신(`doctor-workspace.spec.mjs`) |
| `STAGE_0_BLOCKED_MESSAGE_KO` (= `0단계(보호/안정) 확정 — ` + 상수) → `EMPTY_RESULT(…, 'STAGE_0', …)` | 원장 화면 · 운동 후보 블록(접힘) | **같은 자리.** 접두사는 그대로, 뒤 문장만 교체 |
| `STAGE_0_GUIDANCE_KO` (부위 일반 별칭) | 소비처 없음 — 테스트만 | 별칭 유지 |
| 초진 / 재진 / 한약 / mixed / fixture 미리보기 | — | 0단계 안내는 **통증 진료 화면 전용**이며 한약·mixed 경로에 도달하지 않는다. 버린 값 없음 |

**(2) 입력 방향** — 이 상수는 읽기 전용이다. 원장이 편집하는 필드가 아니므로 "쓰이는데 읽히지 않는 필드"가 생기지 않는다. 해당 없음.

**(3) 표시 조건** — `stage === 0`일 때만 렌더된다. latch가 아니라 파생식이고, 상수이므로 **편집 도중 빈 문자열이 되는 경로가 없다.** 해당 없음.

**(4) 테스트** — 교체 경로 1개당 단언:

- `tests/lbp-exercise-stage.spec.mjs` +2 — 상수에 "능동 운동을 처방하지 않"이 **없을 것**, "고부하"와 "일상활동"이 **있을 것**. `SOURCE`가 아니라 **상수 자체**를 본다(교체 경위를 적은 주석에는 옛 문구가 인용으로 남아야 하므로).
- `tests/lbp-exercise-recommendation.spec.mjs` — 블록 메시지에 같은 두 조건. 옛 문구를 pin하던 단언을 **새 계약으로 교체**(삭제 아님).
- `tests/doctor-workspace.spec.mjs` — 실제 렌더 HTML에 새 문구가 있고 옛 문구가 없을 것.

### 함께 고친 곳 — 같은 정의를 되풀이하던 주석 6곳

`lbpExerciseStageTable.ts`(2), `lbpExerciseRecommendation.ts`(2), `persistence.ts`(1), `regionPacks/neck.ts`(2), `regionPacks/shoulder.ts`(1), `docs/NECK_EXERCISE_EVIDENCE_MATRIX_v0.1.md`(2). 목·어깨 팩은 주석에서 **"요통 방식"이라고 명시**하며 LBP 정의를 상속하므로, 여기를 놔두면 두 정의가 공존한다.

`docs/NECK_EXERCISE_DECISIONS_v1.0_CLOSED.md`의 같은 표현은 **고치지 않았다** — CLOSED 결정 기록은 그 시점의 역사이고, 사후 수정 대상이 아니다.

### 바꾸지 않은 것과 그 이유

- **배정 산술** (`clamp(base − demotions, 0, 3)` + fear avoidance 상한). DR_07은 "irritability로 게이팅하라"고 하면서 **어떤 지표의 몇 점이 게이트인지 주지 않는다**(§11-5: "0–3, 0–4, 0–5/10 중 어느 기준이 최적이라는 근거가 없다"). 지금 바꾸면 근거 없이 정책값을 한 번 더 뒤집는 것이다. 회고 파일럿이 결정하게 둔다 — 같은 날 오전 "재발 격하 보류"와 같은 논리.
- **라벨 `'0단계 · 보호/안정'`.** 저녁 계획에서 `'0단계 · 고부하 유예'`를 대안으로 제시했으나 채택하지 않았다. 라벨은 버킷 이름이고 원장이 환자에게 읽는 것은 안내문이다. 바꾸면 파일럿 집계 테스트 3건과 `doctor-workspace` 1건, 목 문서 1곳이 함께 흔들린다 — 근거상 이득 없이 diff만 커진다. **필요하면 별도 PR.**
- **`ankleFoot.ts` `AF_TEND_01`의 "통증 10점 중 5 이하".** DR_07이 유일하게 출처를 인정한 건병증 모델(Silbernagel 2007, PMID 17307888) 맥락에 정확히 놓여 있다. 그대로 둔다.

### 검증

- `npm run build` (`tsc -b && vite build`) 통과.
- `npm run test:all` **exit 0** — 전 스위트 통과. 관련 스위트 개별 확인: `lbp-exercise-stage` 285, `lbp-exercise-recommendation` 30, `doctor-workspace` 303, `region-pack` 347, `lbp-stage-pilot` 46, `region-pilot-observation` 69.
- 옛 문구 잔존 검색: `src/`·`docs/`·`tests/`에 `능동 운동 미처방` 없음(CLOSED 기록 1곳 제외, 의도적).

---

## 2026-09-19 (저녁) — 딥리서치 검수 7/7 완료. **DR_07을 반영해 승인된 PR 2·3 계획을 수정하고 PR 4를 신설**

**결정자**: Gomars93(원장, PO) — "1.진행 2.DR_07 반영해서 수정 3.볼트 경유". 저장소 **코드 변경은 여전히 0건**(계획 문서만 갱신).

### 1. 딥리서치 인용 검증 — 03 부항 · 04 추나 · 05 도침 완료 (7/7)

검증은 이 환경의 egress 제약으로 **WebSearch 기반 서지·초록 대조**다. 전문 대조가 아니다.

**형식 준수** — 세 편 모두 12개 항목 제목을 정확히 지켰고, `[미확인]` 태그(3·4·5편에서 각 4·2·3회), "근거 없음" 명시, "외삽" 표시, "중국어 단독/중국 중심" 표시를 사용했다. **NCKM 권고문 원문을 인용한 편은 없다**(등장하는 "권고문"은 전부 "인용하지 않았다"는 메타 서술).

**원문 대조 4건 — 전부 실재 확인**

| 문헌 | 확인 결과 |
|---|---|
| Jenkins LC et al. **2026**, *JOSPT Open* 4(1):23-35, DOI 10.2519/josptopen.2025.0159 | ✅ 실재. dry cupping vs **placebo**, n=281, GRADE very low. PEDro 요약도 "very uncertain" |
| de Zoete A et al. **2026**, Cochrane CD008112.pub3 (2026 Issue 1) | ✅ 실재. **76 RCT / 11,866명**, 비교군 3종(sham·무치료·다른 보존치료) 구조 일치 |
| Almeida Silva 2021, *J Physiother* 67:132-139 | ✅ 실재. "dry cupping is **not superior to sham**" — DR_03의 음성 결과 서술과 일치 |
| Lee JH et al. 2021, *JAMA Netw Open*, PMID 34259850 | ✅ 실재. 추나 vs usual care, **n=108, 5개 기관**. DR_04는 5주 군간차를 인용했고 원문 초록은 1년 누적치를 제시 — **시점이 다를 뿐 모순 아님** |
| Xu 2022, PMID 36484186 (중국어) | ✅ 실재. n=88(44/44) sham-acupotomy. WOMAC·VAS는 도침군 우세, **내·외측 측부인대 두께와 ROM은 군간 차이 없음** — DR_05가 "구조 기전 단정의 반례"로 쓴 해석이 원문과 정확히 일치 |
| An S et al. **2026**, *J Pain Res*, PMID 42707450 | ✅ 실재. US-guided 도침 vs conventional PT, **pilot RCT n=50, 1:1**. DR_05의 "TENS+microwave diathermy"는 CPT의 구성 |
| Lee 2023, *J Pain Res*, PMID 36908927 | ✅ 실재. **n=34(17/17), 8주**, 침(17혈)+ICT 2회/주, 도침군만 7혈 추가. DR_05가 이를 **음성(도침 추가효과 확인 실패)**으로 보고한 것이 정확 |

**발견한 결함 1건 — DR_03, Cramer 2020의 통증 sham 수치 (`요검증`)**

Cramer H et al. 2020 (*J Pain*, PMID 31982686, 18 RCT / n=1,172) 초록은 **장애(disability)** sham 대비 SMD **−0.26 (−0.57, 0.05)**를 제시하고, **통증** sham 대비는 "no significant effects"라고만 쓴다. DR_03은 **통증** sham 대비를 **−0.27 (−0.58~0.05)**로 적었다 — 장애 수치와 CI가 사실상 동일하다.

- 본문에 별도로 존재하는 값일 수 있으나 **초록으로는 확인되지 않았다.** 전문 확인 전까지 이 숫자를 인용하지 않는다.
- **결론 방향은 영향 없다.** "무처치 대비 크고 sham 대비 불확실"은 원문과 일치한다. 수치 정밀도 문제이지 판정 오류가 아니다.
- DR_03이 누락한 것 1건: 장애에서 **active 대비 −0.52는 유의**했다.

**종합 판정** — 03·04·05는 01·02·06·07과 같은 수준으로 **형식·정직성 모두 양호**하다. 특히 DR_04는 분절 특이성 반증(de Oliveira 2020), audible pop 무관(Moorman 2022), "serious AE 0/7,518 ≠ 위험 0"을 스스로 적어 **자기 술기에 불리한 근거를 먼저 배치**했다. 정본화 단계로 넘긴다.

### 2. DR_07 반영 — 승인된 PR 계획 수정 (**PO 승인**)

**근거는 DR_07의 다음 진술이다.**

- §1-6 — "`0–4/10` 같은 고정 규칙을 전신 근골격계에 적용할 근거도 없다." Achilles pain-monitoring model(≤5/10, 다음날 아침 회복, 주간 악화 없음)은 **건병증 원전의 보고 범위**이며 타 부위 적용은 외삽.
- §6 — 운영 기준은 **GREEN ≤3/10 · YELLOW 4–5/10 · RED(신규 신경증상·급격한 기능저하·야간 지속통)**. DR 스스로 "**임상 운영안이며 전체 MSK에서 검증된 RCT 규칙이 아니다**"라고 단서를 달았다.
- §10.2 금지표현 4번 — **"아프더라도 4점까지는 무조건 안전합니다."**
- §11-5 — "0–3, 0–4, 0–5/10 중 어느 기준이 최적이라는 근거가 없다."
- §11-9 — "'3일/1주 무조건 쉰다' 같은 고정 기간보다 **irritability와 red flag에 따라** 조절해야 한다."
- §1-4 — PSFS SEM 0.35–1.5, MID/MCID 1.2–1.3은 **어깨·상지 연구** 값. **단일항목 변형은 PSFS의 심리측정 근거를 그대로 적용할 수 없다.**
- §2.1.5 / R4 — region-specific vs general exercise 18 RCT / n=1,719에서 **통증·장애 유의차 없음**(Desmeules F et al. 2021, *Arch Phys Med Rehabil*, PMID 33684362). task specificity는 "우월한 운동 종류"가 아니라 **용량·전이·성과측정을 정렬시키는 프레임**.

**먼저, 코드 실사 결과 — 현재 저장소에 `0~4/10` 규칙은 없다.**

`src/`·`docs/`·`tests/`·`tablet core/` 전수 검색 결과 통증 허용치를 숫자로 박은 곳은 **단 한 줄**이다.

```
src/doctor/workspace/regionPacks/ankleFoot.ts:237  (AF_TEND_01 아킬레스 부하 점진)
  startingDoseKo: '… 운동 중 통증 10점 중 5 이하.'
  acceptableResponseKo: ['…', '다음 날 아침 뻣뻣함·통증이 기저로 회복됨']
```

이 한 줄은 **DR_07이 유일하게 출처를 인정한 그 모델**(Silbernagel 2007, PMID 17307888)이 나온 건병증 맥락에 정확히 놓여 있다. 나머지 부위 팩은 전부 숫자 없이 "통증 허용 범위"로만 쓴다. **즉 문제는 기존 코드가 아니라, PR에서 새 숫자를 넣으려던 계획이다.** 고칠 것은 없고, 넣지 않으면 된다.

#### PR 2 수정 — 명칭에서 PSFS를 끊는다

| 항목 | 기존 계획 | **수정 후** |
|---|---|---|
| 필드 id | `LBP_16_TARGET_FUNCTION_SCORE` | **유지** |
| 라벨 | (미정) | **"목표 기능 자기평가(0–10, 단일 항목)". `PSFS`라는 문자열을 라벨·주석·EMR 출력 어디에도 쓰지 않는다** |
| 주석 | — | **신설**: SEM 0.35–1.5 / MID 1.2–1.3은 상지·어깨 유래이며 **요추 MCID 값은 존재하지 않는다**. 추적 anchor 전용, 효과 주장 outcome 금지 |
| 테스트 | fixture 5개 갱신 | **+1**: 이 필드의 사용자 노출 문자열에 `PSFS`가 없음을 단언하는 소스 텍스트 테스트 |

나머지(chip 9개와 1:1, `detailCheck` 재질문, `questionnaire-volume` 요통 프로필 3개 고정값 수기 갱신)는 **변경 없음**.

#### PR 3 수정 — 진행 게이트를 "그 자리 통증"에서 "다음날 회복"으로 옮긴다

| 항목 | 기존 계획 | **수정 후** |
|---|---|---|
| 재진 기록 | "운동별 달성 dose + 유지/진행/후퇴" 한 줄 | **"달성 dose + 운동 중 통증(0–10) + 다음날 아침 회복(회복/지속/악화)"**. 진행 판단의 1차 게이트는 **다음날 회복 여부**이며 그 자리 통증 점수가 아니다 (DR_07 §2.1.3 "핵심 성과는 task tolerance와 다음날 회복 패턴") |
| 허용 밴드 표시 | (미정) | **`0~4/10` 금지.** 밴드를 띄운다면 **GREEN ≤3 / YELLOW 4–5 / RED(신경증상·급격한 기능저하·야간통)**만 쓰고, 화면과 주석에 **"삼인당 정책값 — RCT 검증 규칙 아님"**을 함께 띄운다 |
| 금지 문자열 | — | DR_07 §10.2 5개를 **원장 화면 어디에도 출력하지 않는다** |

`regionClinicalState` 필드 **추가(교체 아님)**, persistence 옛 기록 호환, CLAUDE.md "제거 또는 교체" 규칙에 따른 **필드 × 화면 표** 요구는 **그대로 유지**된다.

#### PR 4 신설 — 0단계 정의 교정 (`lbpExerciseStage.ts`)

현재 문구가 DR_07과 어긋난다.

```
LBP_STAGE_0_GUIDANCE_KO =
  '오늘은 능동 운동을 처방하지 않습니다. …'
```

DR_07 §11-9는 급성 비특이적 요통에서 **active approach 자체는 타당하고**, 불확실한 것은 "초기 고강도 structured exercise의 추가이득"이라고 본다. 즉 유예 대상은 **능동 운동 전부가 아니라 구조화된 고부하 loading**이다.

- **문구 교정** — "능동 운동을 처방하지 않습니다" → **"오늘은 구조화된 고부하 운동을 유예합니다. 통증 없는 범위의 움직임과 일상활동은 유지하고, 다음 진료에서 irritability와 red flag를 다시 평가합니다."**
- **라벨 검토** — `'0단계 · 보호/안정'`이 "쉬는 단계"로 읽힐 여지. 대안 `'0단계 · 고부하 유예'` 제시, 최종은 PO 판단.
- **주석 보강** — 0단계는 **고정 기간이 아니라 irritability 게이트**이며 매 진료 재평가된다는 점을 명시. 현재 `severe + 급성` 격하 산술은 **irritability의 대리지표**이지 근거 기반 임계값이 아니라는 기존 경고("삼인당의 정책값이며 원장 승인 대상")를 유지·강화.
- **순서** — `lbpExerciseStageTable.ts` 주석의 "**표를 바꾸려면 문서를 먼저 바꾼다**" 원칙에 따라 `docs/LBP_EXERCISE_STAGE_ASSIGNMENT_v0.4.md` 개정이 **선행**.
- **테스트** — `LBP_STAGE_0_GUIDANCE_KO`에 "능동 운동을 처방하지 않"이 **없고** 일상활동 유지 문구가 **있음**을 단언 (소스 텍스트 1건).

#### PR 1 보강 — 금지 용어에 DR_07 §10.2 5개 추가

`CLINICAL_OS_NORTH_STAR.md`의 용어 가이드에 편입: "코어가 약해서 아프다" / "골반이 틀어져서 과부하" / "바로 좋아졌으니 원인을 찾았다" / "4점까지는 무조건 안전" / "풀어놓고 운동하면 고정된다".

또한 task-specific loading의 정당화 문구를 **"일반 운동보다 우월"이 아니라 "용량·전이·성과측정을 목표 task에 정렬시키기 위함"**으로 적는다 (Desmeules 2021, PMID 33684362: 유의차 없음).

### 3. 유지되는 결정

- **재발 격하 규칙 보류 유지.** DR_07에 재발 **예측인자**가 없다. 다만 WalkBack RCT(R17: 활동제한성 재발 HR 0.72, 재발 중앙값 112→208일)는 **"통증이 없어졌으니 종료"가 아니라 회복 후 걷기·일상부하를 유지하는 2차 예방 단계**를 지지한다 — 이는 격하 규칙이 아니라 **재진 종료 시점 정책**의 문제이며 별도 안건으로 둔다.
- **재활 라이브러리 본문은 고치지 않는다**(오전 결정 4 유지). 위 실사가 이를 재확인했다 — 허용 반응은 이미 "다음날 아침 회복"으로 서술돼 있다.

### 4. 매선 v1.1 배포 경로 — **볼트 경유로 확정**

오후 결정 3(Drive 반영 보류)을 대체한다. Drive 커넥터 `create_file`이 mimetype과 무관하게 Google Docs로 변환하는 문제는 해결되지 않았으므로, **원장이 로컬 Obsidian 볼트에 파일을 직접 배치**하고 Drive 동기화가 올리게 한다.

- 대상 경로: `삼인당 임상 지식베이스/09_치료술기/06_매선/`
- 덮어쓸 파일: `매선-SSOT.md`, `매선-근거매트릭스.md` (둘 다 v1.1)
- `매선-운영SOP.md`는 **변경 없음** — 덮어쓰지 않는다.
- Drive `06_매선/`의 현재 3개 파일은 **v1.0 원본 그대로 보존**돼 있다(아카이브 이름 변경은 되돌렸다).

---

## 2026-09-19 (오후) — 매선 근거 충돌 2건을 **확정 해소**. 회전근개 "sham 대비 우월" 표현 폐기, 무릎 OA 근거강도 하향 확정

**결정자**: Gomars93(원장, PO) — "너의 추천안으로 모두 진행해줘" 위임. 저장소 코드 변경 여전히 0건.

### 배경

오전 병합에서 `매선-SSOT.md` §9-2에 미해결 4건을 남겼다. 그중 2건은 **딥리서치 `DR_06_매선_v1.md` 수령 전에는 판정할 수 없다**고 명시했었다. 같은 날 오후 DR 7편이 도착해 전수 확인했다.

### 결정 1 — 회전근개: "2024 RCT, sham 대비 우월"은 **오기록**. 표현 폐기

- 구 매뉴얼 3종(v2.0·v3.0·통합 레퍼런스 1권)이 적은 2024년 회전근개 RCT는 **존재하지 않는다.** DR_06 전수 확인 결과 회전근개 매선 임상시험은 **Park YC, Chae S, Kim Y, et al. 2026 (*Medicine* 105(1):e46638, PMID 41496087, PMC12778091) 한 편뿐**이다.
- 그 한 편의 **1차 결과(100-mm VAS)는 군간 유의차 없음(primary negative)**. SPADI·RC-QoL·EQ-5D도 유의차 없음. 외회전 ROM만 week4에서 제한적 유의(ITT p=.046). DR_06이 해당 행에 "양성으로 요약하면 안 됨"이라고 직접 경고를 달았다.
- **판정**: 회전근개는 "1차 유효성 미입증". `"sham 대비 우월"` 표현은 전 문서에서 폐기. 환자 설명 상한은 "무작위대조시험에서 연구된 바 있는 보조적 선택지".
- **오기록 경위 추정** — Zhao 2025가 *Medicine* 104(51), Park 2026이 105(1)로 **같은 저널 연속 호**다. 두 논문이 섞였을 가능성이 가장 높다. (추정이며 확정 아님.)

### 결정 2 — 무릎 OA: Zhao 2025는 **catgut** 연구. 근거강도 강함 → 제한적 확정

- Zhao H et al. 2025 (PMID 41431000, *Medicine* 104(51):e46676)는 **acupoint catgut embedding(ACE) SR/MA** (28 RCT / 2,120명). 저자 결론 자체가 "근거 질 낮음, inconclusive".
- DR_06과 지식베이스 v1.0 **두 독립 출처가 동일 판정**이므로 확정. 기존 매뉴얼 3종이 이를 PDO 매선 근거로 인용해 "강함"을 준 것은 오기록.
- **판정**: 무릎 OA 근거강도 **강함 → 제한적(낮음·비결정적)** 확정. 남는 PDO 직접근거는 Woo SH 2022 파일럿(군×시간 상호작용 무의)뿐. **PDO vs catgut 직접 비교 RCT는 현재 없다.**

### 판정 방향에 대한 메모

두 결정 모두 **주장을 낮추는 방향**이다. 낮추는 판정은 올리는 판정보다 요구 근거가 적어도 된다 — 틀렸을 때의 임상적 비용이 비대칭이기 때문이다. 반대 방향(근거강도 상향)이었다면 두 출처 일치만으로는 확정하지 않았다.

### 결정 3 — Drive 반영은 **보류**. 원본 형식 보존 경로가 확보될 때까지 v1.0 유지

- 이 세션의 Google Drive 커넥터 `create_file`은 mimetype을 `text/markdown`·`text/plain`·`application/octet-stream` 중 무엇으로 지정해도 **생성물을 Google Docs로 변환**한다(프로브 3회로 확인). 변환본은 표 구조가 무너지고 한글 문자 손상이 발생했다.
- **임상 문서를 손상된 형식으로 덮어쓰지 않는다.** Drive `09_치료술기/06_매선/`의 3개 파일은 **v1.0 원본 그대로 보존**하고, v1.1 두 파일은 PO에게 직접 전달한다.
- 후속: Obsidian 볼트 직접 배치 또는 `.md` 보존 업로드 수단 확보 — PO 판단 사항.

### 딥리서치 인용 검증 — 4편 완료, 3편 미착수

검증은 **WebSearch로만** 수행했다. 이 실행 환경의 egress가 `pubmed.ncbi.nlm.nih.gov`·`europepmc.org` 직접 접근을 차단해 원문 fetch가 불가능하다. 따라서 아래는 **서지·초록 수준 대조**이며 전문 대조가 아니다.

- **DR_01 침** — Busse 2023 (BMJ, DOI 10.1136/bmj-2023-076226) 실재. RD 42%(33–50) 수치 일치. "지표 혼용" 혐의는 **기각**(DR가 RD로 정확히 라벨링). 다만 DR_01은 42%를 **신체기능**에 귀속시켰는데 원문은 **통증 완화(pain relief)** 기준이다 — **귀속 대상 불일치 1건**.
- **DR_02 약침** — PMID 41477276(요추 추간판탈출 약침 종류·용량 scoping review, *J Pharmacopuncture* 2025) 실재, 용량 절 유지. PMID 42541009는 실재하나 **포도당 prolotherapy SR**이며 DR_02가 이미 `[prolotherapy 외삽]`으로 표시했으므로 라벨 일관성 문제만 남는다. PMID 27149503(Kim Me-Riong 2016, 80,523명) 실재하고 **0.019% 계산은 오류가 아니다** — 결함은 원문에 없는 "연구 전체 환자 기준"이라는 가필과, 기흉 4건을 봉약침에 귀속시킨 점(원문은 침·약침 합산)이다. DR_02는 §8.3에서 스스로 모순된다.
- **DR_06 매선** — 위 결정 1·2의 근거. 서지·n·결과가 지식베이스 v1.0과 일치.
- **DR_07 재활** — 아래 별도 항목.
- **DR_03 부항 · DR_04 추나 · DR_05 도침** — **미착수.** 같은 깊이의 인용 검증이 필요하다.

### DR_07(재활)이 승인된 PR 계획과 충돌하는 지점 2개 — **PO 판단 대기**

승인된 PR 2·3을 **그대로 구현하면 근거 없는 수치가 코드에 박힌다.** 착수 전 결정이 필요하다.

1. **"통증 허용 0~4/10"에 근거가 없다.** DR_07의 자체 규칙은 GREEN ≤3 / YELLOW 4–5이고, "아프더라도 4점까지는 무조건 안전합니다"를 **금지표현 목록에 올려놨다.**
2. **Stage 0 = "능동 운동 미처방"이 지지되지 않는다.** DR_07은 "고부하 loading 유예 + 교육·일상활동 유지"로, **고정 일수가 아니라 irritability/red flag로 게이팅**하라고 본다. `lbpExerciseStage.ts`의 `LBP_STAGE_0_GUIDANCE_KO`가 직접 영향권이다.

부수 확인 3건: ① 단일 문항 점수를 **PSFS라고 부르면 안 된다**(MCID 1.2–1.3은 상지·어깨 유래이며 요추 값이 없다) — 계획 중인 `LBP_16_TARGET_FUNCTION_SCORE` 명명에 영향. ② task-specific loading은 일반 운동 대비 **우월이 아니라 동등**(Desmeules 2021, PMID 33684362) — 정당화 근거는 우월성이 아니라 용량·결과 정렬이다. ③ DR_07에 **재발 예측인자가 없다** — 재발 격하 규칙 보류 결정(오전)이 유지된다.

---

## 2026-09-19 — 통증 진료 모델을 **Task–Load–Capacity**로 전환. 외부 검토 수용 범위 확정 + 매선 정본 5개 → 3개 병합

**결정자**: Gomars93(원장, PO). 이 세션은 외부 검토를 코드에 대고 검증하고, 수용 범위를 제안해 승인받고, Drive 지식베이스를 재구성했다. **저장소 코드 변경은 0건**(PO 지시: "수정은 아직 하지 말아주고").

### 배경

PO가 외부(ChatGPT) 검토를 받아 「통증 Clinical OS — 기능 중심 / Task–Load–Capacity 인수인계」를 가져왔다. 핵심 주장:

> 구조 자체를 정상화하는 것이 목표가 아니라, 실제 생활 기능을 제한하는 **수정 가능한 병목**을 찾아 줄이고, 해당 task에서 감당 가능한 **부하(capacity)**를 올리는 것이 목표다.
> 상위 모델: **Task → Load → Capacity → Response → Recovery**

### 결정 1 — 철학은 수용, 변경 목록은 선별 수용

검토 문서의 "핵심 변경 8개"를 코드에 대고 하나씩 확인했다. **절반은 이미 구현돼 있었다.**

| 검토 문서 주장 | 코드 확인 결과 | 판정 |
|---|---|---|
| "골반 틀어짐·다리 길이" 류 구조 문항 제외하라 | `LBP_01~14`에 구조 문항 **0개**. 전부 안전·신경·CES·재발·파행·염증·예후 | **이미 충족**. 비평 대상이 없음 |
| `target_function`을 anchor로 승격 | `lbpTargetFunction.ts` chip 9개가 follow-up target·EMR·재진 배관에 이미 연결 | **이미 anchor**. 차이는 "원장이 고른다 → 환자가 먼저 고른다"뿐 |
| `directional_response`를 관찰값으로 격하 | 이미 관찰값(`FLEXION/EXTENSION/NO_CLEAR/DISTAL_WORSENING/UNCLEAR`), 가설은 `workingHypothesis.supports`로 별도 저장 | **이미 분리됨** |
| "fear avoidance → stage 자동 격하" 재검토 | 격하가 아니라 **상한**(`HIGH_FEAR_AVOIDANCE_MAX_STAGE = 2`). 단계를 깎지 않는다 | 검토 문서가 **현재 규칙을 오인** |
| 8개 부위로 확장 | `regionPacks/`에 hip·knee·neck·shoulder·ankleFoot·elbow·wristHand·tmj **이미 존재** | **이미 됨** |
| 도침·매선 1:1 자동매핑 금지 | 코드에 도침·매선 자동 매핑 **자체가 없음**. 운동 라이브러리에 `GRADED_EXPOSURE`·`LOAD_CAPACITY` 카테고리 이미 존재 | 소프트웨어 변경 **없음** |
| 환자 문항 7개 신설 | 2026-09-06 "새 척도 0개, 과설계 회피" 결정 및 North Star 게이트 4와 **충돌** | **거절** (아래) |
| 재발 <3개월 자동 격하 재검토 | 2026-09-05 PO 본인 지시로 도입. v0.4에 파일럿 중단 기준(0단계 비율 >30%) 이미 존재 | **보류** (아래) |

### 결정 2 — 환자 문항 7개 신설은 거절, 2개만 수용

검토 문서는 초진에 7개 문항(목표 task / 목표 기능 점수 / tolerance / limiting response / recovery time / 재발 상황 / 최근 부하 변화)을 제안했다. **2개만 받는다.**

**거절 사유 3가지**

1. 2026-09-06 결정("새 척도를 넣지 않고 기존 필드로")과 North Star 기능 추가 게이트 4("환자 문진을 불필요하게 두껍게 만들지 않는가")에 정면 충돌.
2. `tolerance`는 task마다 단위가 달라(분/거리/kg/층/회) 단위별 분기 입력이 되고, 태블릿 원칙(자유입력·슬라이더 최소화)에 어긋난다.
3. 원장이 어차피 진료실에서 그 task를 재현하고 baseline을 잰다. 환자가 적은 "대략 5kg"은 30초 뒤 덮어써진다.

**수용 2개** — 목표 task(단일 선택, 기존 `lbp_tf_*` chip 9개와 1:1) + 목표 기능 점수(0~10, 기존 `NumericScale` 재사용). 이 둘은 2026-09-06에 만든 "재검 시점에 같은 id로 다시 묻는" 메커니즘에 그대로 올라타 초진↔재진 비교가 공짜로 생긴다.

**남는 5개**는 원장 화면 Task Response 블록에 원장이 찍는 필드로 둔다.

**지적한 약점 2개** — ① 검토 문서의 권장 흐름에 **회복 기대(`LBP_12`)가 빠졌다.** 요통 예후 인자 중 강한 축이고 이미 문진에 있다. 새 흐름을 적용하면서 지우면 안 된다. ② 단일 task 0–10 점수는 PSFS 3항목 평균의 심리측정치(MCID 약 2점)를 물려받지 못한다. **추적 anchor로만 쓰고 효과 주장 outcome으로 쓰지 않는다.**

### 결정 3 — 재발 격하·fear 상한은 파일럿 데이터까지 보류

검토 문서의 논리("재발 ≠ 현재 낮은 capacity")는 맞다. 그러나 지금 바꾸면 **파일럿 데이터 없이 의견으로 규칙을 두 번 뒤집는 것**이다. `LBP_EXERCISE_STAGE_ASSIGNMENT_v0.4.md`에 이미 중단 기준이 있으니 데이터가 결정하게 둔다. 실제 효과도 `within_3m`에만 한 단계 걸리므로 검토 문서가 암시하는 것보다 작다.

### 결정 4 — 재활 라이브러리는 고치지 않는다

PO 질문("새 철학에 맞추려면 재활 라이브러리를 수정할 게 있나")에 Core-20을 전수 확인한 결과:

- **데이터는 이미 맞다.** 20행 전부가 시작조건·용량·허용반응·중단기준·후퇴·진행을 **부하 반응 기준**("증상이 누적되지 않고 휴식 후 baseline으로 회복되는가")으로 서술한다. 진행 규칙은 "한 번에 한 요소만". `LBP_EXERCISE_CORE_METADATA_POLICY.selectionPrincipleKo`가 이미 "진단명 단독이 아니라 기능·irritability·증상반응·목표·안전성"을 선언한다.
- **선택 규칙도 맞다.** 목표 기능 필터 → 안전·신경 게이트 → 단계 필터 → 방향 반응·신경역학이 뒷받침하면 앞 버킷. 순서가 Task → Safety → Response다.

**고칠 것은 라이브러리가 아니라 그 바깥 두 곳이다.**

1. **재진에 "달성한 부하" 기록 칸이 없다** — `progressionKo`는 데이터에만 있고 추천 모듈이 의도적으로 읽지 않는다(진행은 원장 판단). 그런데 그 판단을 적을 곳이 없어, 새 철학의 최종 outcome("같은 task에서 감당 부하가 늘었는가")이 시스템에 남지 않는다.
2. **capacity 끝단이 얇고 목표 task 목록이 짧다** — 3단계 운동이 20개 중 2개. 목표 기능 chip 9개에 계단·달리기가 없다. **따라서 결정 2의 태블릿 목표 task 선택지는 반드시 기존 chip 9개와 같아야 한다.** 계단을 넣으면 매핑되는 운동이 없어 후보가 빈다. 확장하려면 `docs/LBP_EXERCISE_LEVEL_DRAFT_v0.2.md` 개정 → Core 확장 → 분포(9/8/2/1) 고정 테스트 갱신 순서를 지킨다(`lbpExerciseStageTable.ts` 주석의 "표를 바꾸려면 문서를 먼저 바꾼다").

**하지 않기로 한 것** — 방향 반응 경로 교체, 부위 확장 재작업, 치료수단 매핑 소프트웨어화. modifier 기반 "직접 뒷받침" 확장(hip cue → 고관절 운동 등)은 **원장 승인 매핑 표가 있어야** 한다(North Star: clinician-approved rule 없이 발명하지 않는다).

### 결정 5 — 승인된 저장소 작업 3건 (아직 착수 안 함)

PO 승인 완료. 단 "수정은 아직 하지 말아주고" 지시로 **구현은 시작하지 않았다.**

| PR | 내용 | 규모 |
|---|---|---|
| 1. 원칙 기록 | `CLINICAL_OS_NORTH_STAR.md`에 Task–Load–Capacity 한 단락 + 용어 가이드(피할 용어/권장 용어) | 문서 3개, 테스트 0 |
| 2. 태블릿 +2 문항 | `LBP_15_TARGET_TASK`(단일 선택, `lbp_tf_*`와 1:1) + `LBP_16_TARGET_FUNCTION_SCORE`(0~10). payload `safety_flags.lbp`에 필드 2개. 재검 재질문 목록(`server/detailCheck.js` + `src/spec/detailCheckQuestions.ts`)에 id 추가. picker가 환자 답을 기본 선택으로 미리 채우되 원장이 변경 가능 | 소스 4~5, 테스트 5 갱신 |
| 3. 원장 Task Response 블록 | `regionClinicalState`에 필드 추가(교체 아님), persistence 옛 기록 호환, PainWorkspace UI, emrPreview, provenance. 재진에 "운동별 달성 dose + 유지/진행/후퇴" 한 줄 | 소스 8~10, 테스트 3~4 + 신규 1 |

**주의 2건** — ① PR 2는 `tests/questionnaire-volume.spec.mjs`의 요통 프로필 3개 고정값(현재 23화면/46탭)을 손으로 갱신해야 한다. 버그가 아니라 "문진이 두꺼워졌음을 숫자로 인정하는" 의도된 마찰이다. ② PR 3은 `directionalResponse` 참조 파일이 20개라 **교체가 아니라 추가**로 간다. CLAUDE.md "제거 또는 교체" 규칙에 따라 필드 × 화면 표가 필요하다.

**PR 2·3 분리 이유** — 태블릿 변경과 원장 화면 스키마 변경을 한 diff에 섞으면 독립 검수가 어려워진다. 잃는 건 PR 오버헤드 1회, 얻는 건 회귀 원인 특정 가능성.

---

### 결정 6 — 매선 정본 5개 → 3개 병합. **단일 승자 없음**을 확인

PO 지시로 매선 정본 후보를 diff했다. **후보는 4개가 아니라 두 계열 5개였다.**

| # | 문서 | 날짜 | 계열 |
|---|---|---|---|
| A | 매선 임상 매뉴얼 v1.0 (+v1.1) | 2026.04~05 | 신경과학 매뉴얼 |
| B | 매선 임상 매뉴얼 v2.0 (+a/b 노트) | 2026.05~06 | 신경과학 매뉴얼 |
| C | 매선 임상 레퍼런스 통합본 v3.0 | 2026.06 | 신경과학 매뉴얼 |
| D | 매선 통합 레퍼런스 1권 | 2026.06.18 | 신경과학 매뉴얼 |
| E | **매선 임상 지식베이스 v1.0** | **2026-07-22** | **운영·안전·컴플라이언스** |

**D의 "1권이 정본, B·C는 아카이브" 선언은 절반만 맞았다.** D를 남기고 나머지를 버리면 **고유 항목 9개가 소실**된다.

| 고유 항목 | 유일 보유 |
|---|---|
| 기전 정직성 보정 6점 (S1 재조직 반박·PDO 추정경로·PBN 논쟁·SRT 단순화·Han2025 비통각·인용정확성) | **D** |
| 말초감작 정밀 구분 (매선은 단기 pro-sensitizing, "감작 치료" 표현 금지) | **B** |
| 말초 근거 심화 (매선 분자연구가 전부 중추 측정, 말초 0건) | **B** |
| PAG 7문항 체크리스트 본문 표 / 유산소 종목 우선순위 / 지연효과 박스 / 2025 신규 인용 7건 | **C** |
| 최대심박수 60~70% / Akbarzadeh 2024 / PDO 분해산물 전체 목록 / NCT05987683 | **A** |
| 제품·로트 추적 / 무균 SOP / 이상반응 대응 5종 / 동의서·차트 템플릿 / 금지표현 10쌍 / 중단 기준 / AI 사용 규칙 | **E** |

### 결정 7 — 근거 충돌 3건 처리

병합 과정에서 **E(7월)가 A~D(4~6월)의 근거 주장을 반박**하는 것이 드러났다.

**충돌 1 — 회전근개. 판정 보류.**
A~D: "2024 RCT(Medicine), sham 대비 우월" (저자·PMID 없음)
E: Park YC et al. 2026, Medicine, **PMID 41496087**, n=64 → **1차 결과지표(VAS) 군간 유의차 없음**, 2차 외회전 ROM만 유의
→ 같은 연구의 상반된 요약인지 별개 연구인지 **미확인**. **원문 확인까지 "sham 대비 우월" 인용 금지.** 환자 설명은 "RCT로 연구된 바 있는 보조적 선택지"까지만.

**충돌 2 — 무릎 OA Zhao 2025. E 채택, 근거강도 하향.**
A~D: 매선(TEA) SR/MA로 인용 → KOA 근거강도 "강함"
E: **PMID 41431000, acupoint catgut embedding** SR/MA. "PDO 직접근거가 아니므로 간접 참고만"
→ **E가 옳을 가능성이 높다.** E만 PMID와 재료를 명시했고, A~D는 캣거트/PDO 혼동을 다른 자리(Huang 2021 육아종 경고)에서는 스스로 경계하면서 이 행에서만 놓쳤다.
→ **KOA 근거강도 "강함" → "제한적" 하향.** 남는 PDO 직접근거는 Woo SH 2022 파일럿(군×시간 무의)뿐. "침보다 우월하다"고 말하지 않는다.

**충돌 3 — 술기 수치. 둘 다 맞다, 층을 분리.**
A~D: 다열근 4cm / 5-0 PDO 29G×40mm = "RCT 근거 표준"
E: "깊이·각도·개수·간격은 제품별 SOP 승인 전 생성 금지. 노션 수치는 연구·집필용·구버전 혼재"
→ A~D는 *특정 RCT가 사용한 파라미터*(Sung WS 2020), E는 *원내 SOP 승격 절차*. **서로 다른 섹션으로 분리 라벨링**해 해소.

**구조적 해결책** — 근거매트릭스에 **재료 열(PDO / catgut / 혼합·불명 / 인접)**을 신설했다. 충돌 2 같은 사고를 구조로 막는다.

### 산출물

`임상지식베이스/09_치료술기/06_매선/`
- `매선-SSOT.md` (49KB) — 임상 판단. D 골격 + B의 말초감작 2절 + C의 구제 4건 + A의 고아 3건 + E의 금기·적신호 보강 + **§6 Task–Load–Capacity 신규 작성**
- `매선-근거매트릭스.md` (17KB) — 재료 열 포함. 충돌 1·2를 `⚠️ 보류`로 명시
- `매선-운영SOP.md` (31KB) — E 이관 + 충돌 3 해소 문구

**원본 5개는 이동·수정하지 않았다.** `임상지식베이스/`가 `.git`으로 로컬 Obsidian 볼트와 동기화 중이라 Drive에서 건드리면 충돌 위험이 있다. ARCHIVED 표시 여부는 PO 판단으로 남겼다.

### 신규 작성 — 매선의 retest 규칙 (SSOT §6)

원본 5개 어디에도 없어 새로 썼다. **매선은 다른 술기와 달리 즉시 retest가 판정이 아니다.**

근거 둘: ① 작용 타임라인이 즉시→수일→2~4주→4~8주로 펼쳐진다. ② 요추 추간판탈출 샴 대조 RCT(Goo 2022)에서 **8주 무차이 → 종료 후 16주 유의**.

| 시점 | 해석 |
|---|---|
| 시술 직후 | **참고용.** BETTER여도 Gate Control 일시 효과일 수 있어 치료 지속 판단에 쓰지 않는다. WORSE면 자극량 과다 신호 |
| 2·4주 | 1차 판정. 4주에 변화 없으면 실·깊이·부위 재검토 |
| 8주 | 2차 판정. **부분 반응이어도 종료하지 않는다** — 지연효과 구간이 남았다 |
| 16주 | 지연 판정. 최종 판정은 여기서 |

**BETTER 해석 핵심** — 같은 target task에서 **허용 부하가 늘었는가**를 먼저 본다. NRS만 내려가고 수행량이 그대로면 1층 효과에 머문 것이며, 3층(재활)으로 넘길 시점이다.

### 미해결 4건 (`매선-SSOT.md` §9-2)

1. 회전근개 Park YC 2026 (PMID 41496087) 원문 확인
2. 무릎 Zhao 2025 (PMID 41431000) 재료 확인
3. 딥리서치 `DR_06_매선_v1.md` 반영
4. `_공통-안전관리.md` 소재 확인 (운영 SOP가 참조 중)

넷이 끝나면 `status: REVIEWED` → `SSOT` 승격.

### 트레이드오프 / 남는 리스크

- **얻은 것** — 고유 항목 소실 0건. 충돌 3건이 드러나 기록됨. 운영 SOP와 임상 판단이 한 폴더에서 연결됨.
- **잃은 것** — 파일이 5→3으로 줄 뿐 2가 아니다. "단일 정본" 서사가 깨지고 매선만 `09_치료술기` 규격(술기당 2파일)의 예외가 된다.
- **남는 리스크** — 충돌 1을 확인하지 않고 쓰면 원내 주요 시술 부위에 **틀린 근거 강도**를 박제한다. E가 맞다면 "sham 대비 우월"이 이미 상담 문구로 흘러갔을 수 있다.

### 이 건이 남기는 절차 (`09_치료술기/00_INDEX.md` 유지보수 규칙에 반영)

> **충돌이 발견되면 한쪽을 지우지 말고 양쪽을 표로 대조한 뒤 판정한다.**

이 저장소는 같은 사고를 네 번 낸 뒤 CLAUDE.md "제거 또는 교체 전 필드 × 화면 표" 규칙을 만들었다. 이번 건은 그 규칙이 **코드가 아니라 문서에** 적용된 첫 사례다. "정본 하나만 남기자"는 정리 충동이 정확히 그 사고의 형태였다.
