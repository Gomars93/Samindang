Warning: truncated output (original token count: 61007)
Total output lines: 5330
 ...
 * 추가·수정하지 않는다.
 */
import type { AnswerValue, Option, Question, Responses } from '../types'
...
const PAIN_FUNCTION_QUESTIONS: Question[] = [
  ...
  },
  {
    ...
    required: false,
    step: '활동',
    screenProgress: { current: 3, total: 5 },
    layout: 'grid2',
    showIf: (r) =>
      IS_PRIMARY_LBP(r) &&
      (PAIN_TIME_BASED_ACTIVITIES.has(r['PAIN_F01'] as string) ||
...
const LBP_QUESTIONS: Question[] = [
  ...
  },
  {
    ...
    input: 'single_choice',
    question: '허리와 별개로 다리에 저림·감각저하·힘 빠짐 같은 증상이 있나요?',
    required: true,
    step: '상세 증상',
    showIf: (r) => IS_PRIMARY_LBP(r) && r['LBP_01'] === 'BACK_ONLY',
    options: [
      { value: 'no', label: '없어요' },
      { value: 'yes', label: '있어요' },
      { value: 'unknown', label: '잘 모르겠어요' },
...
/**…21007 tokens truncated…두 선택해주세요.',
    required: true,
    step: '상세 증상',
    exclusive: 'none',
