...
## 15. v0.2 갱신 — 원문 확보 후 정정 (2026-09-05)

> **이 섹션부터는 스니펫이 아니라 원문을 직접 읽고 썼다.** v0.1의 §1~§14는
> "스니펫만 확인"이었고, 그중 아래 3건은 **원문과 다르거나 원문이 더 구체적이다.**
> 원문 확보 경로: 원장(PO)이 PDF 3편을 직접 업로드(JOSPT 2021 전문 60쪽,
> Comachio 2024 JSAMS 전문, Alrwaily 2016 TBC 전문 확보 시도 중 — 환경의
> `pdftoppm` 누락으로 3번째는 재시도 중). 나머지 2편(WHO 2023, Saragiotto 2016
> Cochrane)은 원장 로컬 PC에 있으나 첨부 용량 제한으로 아직 미확보.

### 15.1 정정 1 — "MCE는 다른 운동보다 우월하지 않다"는 맞지만, JOSPT 2021은 그렇게만 말하지 않는다

v0.1이 Saragiotto 2016 Cochrane 스니펫만 보고 "심부근 활성화 운동(9,10번)의
위계를 올리면 안 된다"고 결론 낸 것은 **방향은 맞지만 근거가 얇았다.**
JOSPT 2021(Table CPG2, "EXERCISE FOR CHRONIC LOW BACK PAIN WITH MOVEMENT
CONTROL IMPAIRMENT")을 원문으로 확인한 결과:

> **A등급(강한 근거)**: "움직임 조절 장애가 있는 만성 요통 환자에게는
> **특정 체간근 활성화 + 움직임 조절 운동을 써야 한다(should)**."

즉 **"모든 만성 요통 환자에게 우월하지 않다"는 맞지만, "움직임 조절 장애가
확인된 하위군에는 A등급으로 지지된다."** 근거는 체계적 고찰 1편(11개 RCT,
Luomajoki 2018) — 치료 직후 통증 소효과(SMD −0.39), 기능장애는 1년까지 유지
(SMD −0.37).

**교훈**: v0.1이 "MCE는 우월하지 않다"를 **전체 요통 환자**에 대한 진술로
과잉 일반화했다. 실제로는 **하위군(움직임 조절 장애)에서만 A등급**이고, 일반
만성 요통에서는 B등급("may use")이다. **1단계(증상 조절)/2단계(움직임
조절)라는 우리의 TBC 배정 자체가 이 하위군 개념과 정확히 일치한다** — 9번·
10번·11번을 2단계로 배정한 §A의 판단은 원문으로 확인 후에도 유지된다.

### 15.2 정정 2 — 급성/만성 요통에 **같은 운동 목록**을 쓰면 안 되는 근거가 원문에 명시돼 있다

JOSPT 2021 "KEY POINTS FOR EXERCISE INTERVENTIONS"를 그대로 인용한다:

> **급성 요통**: "많은 운동 훈련 중재가 급성 요통의 통증·기능장애를 줄일
> 가능성이 있으나, **현재 근거로는 특정 한 가지 운동 종류를 추천할 수 없다**."
> (C등급 — 단일 저품질 RCT 수준)
>
> **만성 요통**: "운동 훈련 중재는 만성 요통의 통증·기능장애 감소에 **폭넓게
> 권고될 수 있다**." (A등급 — 근력·지구력/복합양식/특정체간활성화/유산소/
> 수중/일반운동 전부 포함)

**이건 v0.1이 몰랐던 사실이다.** 급성 요통에 대한 근거등급은 **C(단일 저품질
연구)**로 만성(A, 다수의 고품질 연구)보다 현저히 낮다. 그리고 급성 요통
전용 RCT 자체가 부족하다는 것을 JOSPT가 스스로 명시한다("Gaps in Knowledge:
There is a lack of RCTs examining exercise training interventions for
patients with acute LBP who do not have related leg pain").

**함의**: 우리 20개 운동 중 다수(1,2,4,5,6,7,17,20 등 1단계 배정)가 **"급성기
증상 조절"** 용도인데, **이 단계 전체가 근거 등급 C 위에 서 있다.** 이건
운동이 틀렸다는 뜻이 아니라 — **"1단계 운동은 안전과 증상 조절이 목적이지,
근거로 효과를 입증하는 단계가 아니다"**를 화면 문구에 명시해야 한다는 뜻이다.
2~3단계로 갈수록(만성, A등급) 근거가 강해진다.

### 15.3 정정 3 — 유산소 운동의 실제 등급: "걷기 단독"과 "걷기+병행"은 다르다

v0.1은 "유산소 운동이 명시적으로 없다"고 지적했는데, **왜 없어도 되는지에
대한 근거가 원문에 있었다.** JOSPT 2021이 인용하는 체계적 고찰
(Sitthipornvorakul 2018, I등급, 9개 RCT):

> **걷기 단독** vs 다른 중재: 통증·기능장애 **차이 없음**(중등도 질 근거,
> pooled SMD 0.03~0.07)
> **걷기 + 다른 중재** vs 다른 중재 단독: 역시 **차이 없음**

즉 **"걷기만 추가한다고 더 좋아지지 않는다"**가 실제 근거다. 그런데도
JOSPT는 유산소 운동을 **A등급**(만성 요통 종합 권고에 포함)으로 뒀다 —
이유는 개별 RCT 2건(Bruehl 2020, Cuesta-Vargas 2011)이 **다른 유형의
유산소**(수중 달리기 등)에서는 유의한 개선을 보였기 때문이다.

**함의**: 우리 목록의 1·2번("짧게 걷기", "짧게 걷고 쉬기")은 **유산소
컨디셔닝이 아니라 활동 재개(activity resumption)** 목적으로 이미 정확하게
분류돼 있었다(§A에서 1단계 배정). **별도의 "유산소 운동" 항목을 추가할
근거는 약하다** — 추가한다면 걷기가 아니라 **개인화된 강도의 유산소**(수중
등)여야 하고, 이는 20개 항목보다 시설 의존적이라 우리 맥락에 안 맞을 수 있다.
**v0.1의 "유산소가 가장 명확한 결손"이라는 결론을 철회한다.**

### 15.4 확인된 것 — v0.1의 다른 결론은 그대로 유지

- **용량(dose)에 대한 가이드라인 침묵**: Comachio 2024 원문 전체를 확인—
  Table 3이 문자 그대로 18개 가이드라인 전부의 type/frequency/intensity/
  volume 유무를 ✓/✗로 표로 보여준다. **frequency 제공 2/18, intensity
  1/18, volume 0/18.** v0.1의 결론이 원문으로 **확정**됐다.
- **"어떤 운동도 우월하지 않다"(전체 수준)**: JOSPT 2021 자체 결론("no clear
  evidence of the superiority of one exercise type or approach over
  another when exercise protocols are directly compared")과 일치.
  **다만 §15.1처럼 하위군 수준에서는 예외가 있다** — 이 구분을 놓치면 안 된다.
- **TBC 3단계 배정과 ODI 컷오프는 여전히 원문에 없다** (JOSPT는 TBC를 급성
  요통 분류법 중 하나로만 언급, B등급 — Alrwaily 원 논문 확인 필요, 재시도 중).

### 15.5 아직 미확인 (다음 라운드)
- Alrwaily 2016 TBC 원문 — 환경 문제로 재시도 중
- WHO 2023, Saragiotto 2016 Cochrane 원문 — 원장 로컬 PC 확보, 미업로드
- 위 2건 확보 시 §15에 계속 추가할 것. **아직 라이브러리 코드는 갱신하지
  않는다** — 5편 원문이 갖춰진 후 일괄 반영.

---

## 16. TBC 원 논문(Alrwaily 2016) 확보 — §15.5 해소, 결정적 정정

> pip로 `pymupdf`를 설치해 페이지 렌더링 없이 텍스트 직접 추출로 확보(환경의
> `pdftoppm` 미설치가 apt 미러 접속 차단으로 설치 불가 — 원문 인용은 위 방법으로
> 확보했으나 자세한 표 레이아웃 확인이 필요하면 재확보 요망).

### 16.1 핵심 표를 원문 그대로 인용한다 (Page 30, "Triage process and matching criteria")

| 분류 변수 | **증상 조절** | **움직임 조절** | **기능 최적화** |
|---|---|---|---|
| 통증 정도(Pain rating) | 높음(high) | 낮음~중등도 | 낮음 |
| **장애 정도(Disability rating)** | **높음(High)** | **중등도(Medium)** | **낮음(Low)** |
| 임상 상태(Clinical status) | **불안정(Volatile)**: 증상이 지배적 | **안정(Stable)**: 움직임 장애가 지배적 | **잘 조절됨(Well-controlled)**: 수행력 결손이 지배적 |
| 치료 조정 변수 | 심리사회적 상태·동반질환 — **세 단계 전부에 +/- 로 표시** (모든 단계에서 별도 평가) | | |

**각주 2 (원문 그대로)**:
> "Disability can be assessed with **any outcome measure of disability** such
> as Modified Oswestry Disability Questionnaire, Roland-Morris Disability
> Questionnaire, etc."

### 16.2 정정 — **숫자 컷오프는 원래 없다. 이게 "미확인"이 아니라 저자의 의도적 설계다**

v0.1/v0.2가 "TBC-ODI 컷오프를 못 찾았다"고 정직하게 적었는데, **원문을 읽어보니
애초에 존재하지 않는다.** 저자들이 준 것은 숫자가 아니라 **"판단 기준 3개
(통증/장애/임상상태)를 원장이 종합 판단한다"는 알고리즘**이다. 그리고
불일치 시 처리 규칙까지 명시돼 있다:

> **각주 1**: "분류 변수들이 일치하지 않을 때는 **장애 등급(disability
> rating)에 의존**해 환자를 치료 접근법에 매칭할 것을 권고한다. 이 판단은
> 환자의 임상 상태로 보완돼야 한다."

**즉 세 지표 중 하나만 고르라면 "장애 정도"가 1순위 기준이라고 원문이 명시한다.**

### 16.3 임상 상태(Clinical status) 정의 — 이것도 원문 그대로, 우리 표에 없던 것

이건 v0.1/v0.2 어디에도 없던 **네 번째 축**이다. ODI 점수만으로는 못 잡는다:

| 상태 | 원문 정의 |
|---|---|
| **불안정(Volatile)** | "임상 상태가 쉽게 악화될 수 있고, 환자가 매우 예민하다(작은 요추 움직임에도 통증이 쉽게 유발됨). 때로는 환자 상태가 신체검사 자체를 허용하지 않는다." |
| **안정(Stable)** | "특정 움직임·자세·검사에 악화될 수 있으나 비교적 빨리 기저 수준으로 돌아온다." |
| **잘 조절됨(Well-controlled)** | "대부분 무증상이나 수행 요구가 커지면 악화될 수 있다." |

**이게 우리 레벨 표 v0.1의 "확신도 ◐/○" 7줄 중 다수를 해소한다** — 예를 들어
13번(서서 다리 옆으로 들기)이 2단계냐 3단계냐를 "근력이냐 조절이냐"로 고민했는데,
**진짜 기준은 "이 운동을 견디는 동안 환자의 임상 상태가 안정(Stable)인가
잘 조절됨(Well-controlled)인가"다.**

### 16.4 1995년판(전신) — "단계"라는 우리 번역어의 원 출처

TBC의 1995년 원형은 숫자로 이름 붙은 **Stage I/II/III**였고, 그 정의가 우리
직관과 정확히 일치한다(원문 그대로):

> **Stage I**: "심한 통증과 장애 상태의 환자. 치료 목표는 **증상 조절**."
> **Stage II**: "통증이 너무 심하지는 않지만 일상활동을 방해하는 환자. 치료
> 목표는 잔여 증상 해소와 일상활동 수행을 위한 신체 기능 개선."
> **Stage III**: "비교적 무증상이고 표준 일상활동은 가능하나 더 높은 수준의
> 신체 기능으로 복귀해야 하는 환자. 치료 목표는 증상 악화 없이 더 높은 수준의
> 신체 기능을 수행하는 능력 향상."

**2015년 개정판(§16.1의 표)이 이 3단계 이름을 "증상 조절/움직임 조절/기능
최적화"로 바꾼 것**이고, 우리가 이미 쓰던 번역(1단계·2단계·3단계)이 원문의
Stage I/II/III과 자연스럽게 대응한다.

### 16.5 이 발견이 레벨 표 v0.1에 미치는 영향

**ODI 경계선 두 개를 원장님께 여쭤봤던 §B-1은 철회한다.** 원문 자체가 숫자
컷오프를 안 쓴다. 대신 원장님께 여쭤야 할 것은 이렇게 바뀐다:

- ~~"ODI 몇 점부터 2단계로 보십니까?"~~ (❌ 원문과 안 맞는 질문이었다)
- **"장애 정도(어떤 척도든) + 임상 상태(불안정/안정/잘 조절됨)를 함께 보고
  판단하시는 게 맞습니까, 아니면 태블릿 문진에서 뽑을 수 있는 특정 지표
  하나로 충분하십니까?"**

그리고 **"임상 상태(Volatile/Stable/Well-controlled)"는 지금 우리 문진에
없는 개념이다.** 가장 가까운 것은 재진 문진의 "오늘 재검 소견"·"악화 반응"
항목인데, 이게 그 세 상태를 구분하기에 충분한지는 확인이 필요하다.

### 16.6 남은 것
- WHO 2023 만성 요통 가이드라인, Saragiotto 2016 Cochrane 원문 — 여전히 미확보
  → **§17에서 WHO 원문 일부 확보, 아래 갱신**
- **TBC 논문 자체가 급성/만성 어느 쪽에 적용되는지**: JOSPT 2021(§15.2)은 TBC를
  "급성 요통 분류법"(B등급)으로만 언급한다. Alrwaily 2016 원문에도 급성/만성
  구분 명시가 있는지는 이번 발췌 범위에서 확인하지 못함 — 추가 확인 필요.

---

## 17. WHO 2023 가이드라인 확보 — Table 1 마스터 권고표 (원문 직접 확인)

원장님이 잘라서 올려주신 WHO PDF 2개(`who___.pdf`, `who___2.pdf`)를 pymupdf로
열어 확인. **두 파일 모두 텍스트 레이어가 없는 이미지 스캔본**이라
(`page.get_text()` 전체 페이지 0자) 페이지를 PNG로 렌더링해 직접 읽었다.

**주의**: 하네스가 보고한 페이지 수(1차 77p/7.4MB, 2차 113p/11MB)와 pymupdf가
실제로 연 페이지 수(1차 19p, 2차 24p)가 4~5배 차이났다. 원인은 확인 못 했고,
pymupdf의 실제 페이지 수를 기준으로 작업했다.

- 1차 파일: 표지, 저작권/ISBN 페이지, 목차, 감사의 글, **용어집(Glossary)**
  — 만성 원발성 요통(CPLBP) 정의 포함.
- 2차 파일: Executive Summary 시작(원문 xix쪽 상당)부터 원문 31쪽(§4.2 정성적
  근거 종합 시작)까지. **여기에 Table 1(마스터 권고표, 원문 xxiii~xxvi쪽)이
  통째로 들어 있었다** — 이번에 확보한 가장 중요한 내용.

**두 파일 모두 B.1(운동) 항목의 서술 섹션(원문 47~61쪽, 용량/하위군별 상세)은
아직 포함하지 못했다** — 아래 §17.4 참고.

### 17.1 CPLBP(만성 원발성 요통) 정의 — 용어집 원문

> "통증이 3개월 넘게 지속되거나 재발하며, 다른 진단으로 설명되지 않는 요통."

WHO는 이 가이드라인 목적상 IASP 정의에 있던 "정서적 고통/기능 장애" 기준을
**의도적으로 제외**한다고 명시한다.

### 17.2 Table 1 — B.1 운동 치료 권고 (원문 그대로 확인)

> **B.1 구조화된 운동 치료 또는 프로그램(Structured exercise therapy or
> programme)**: "성인(고령자 포함) CPLBP 환자에게 치료의 일부로 구조화된
> 운동 치료 또는 프로그램을 **제공할 수 있다**(may be offered)."
> — **조건부 권고(conditional recommendation), 사용에 찬성, 낮은 확실성
> 근거(low certainty evidence)**

이건 원문에서 직접 확인한 최초의 WHO 권고문이다. 나머지 A~E 분류(다른 개입
클래스)도 Table 1에 같이 있으나, 이 문서의 목적(운동 라이브러리)과 직결되는
건 B.1뿐이라 여기만 기록한다.

### 17.3 정정 4 — "근거 등급"이 가이드라인마다 다르다는 걸 원문으로 확인

**JOSPT 2021은 만성 요통 운동 치료를 A등급("사용해야 한다", strong)으로
평가했는데, WHO 2023은 같은 대상(성인 CPLBP)에게 조건부·낮은 확실성("제공할
수 있다")으로 평가한다.** 둘 다 원문 확인했고, 둘 다 정확히 인용했다 —
이건 내 실수가 아니라 **가이드라인마다 근거 채택 기준(GRADE 방법론 적용
엄격도, 포함 연구 범위)이 다르다는 실제 사실**이다.

임상적으로 정리하면: **"운동이 도움된다"는 방향성 자체는 두 가이드라인이
일치하지만, "얼마나 강하게 권고할 수 있는가"는 다르다.** 이걸 우리 앱에
반영한다면 "1단계 운동은 안전 우선이지 강한 근거로 뒷받침되는 단계가
아니다"(§15.2)와 같은 톤으로, **"운동 자체가 근거 없는 게 아니라, '반드시
해야 한다'고 말할 만큼 강한 근거는 아니다"** 정도로 서술하는 게 두 가이드라인
모두와 안 어긋난다.

### 17.4 아직 못 구한 것 — B.1 서술 섹션(원문 47~61쪽)

지금까지 받은 2개 파일 모두 **원문 31쪽에서 끊긴다** — B.1 서술 섹션(운동
종류별 세부 권고, 용량, 하위군 분석이 있는 원문 47~61쪽)이 시작되기 전이다.
Table 1(§17.2)은 "권고문 + 등급"만 주고, **어떤 운동을, 얼마나, 누구에게**
같은 세부는 안 준다.

**원장님께 확인 필요**: Table 1(권고 등급)만으로 충분한지, 아니면 서술
섹션(용량/하위군 근거)까지 더 필요한지? 후자라면 원문 47~65쪽 구간을 한 번
더 잘라서 올려주셔야 한다 — 지금까지 두 번 다 그 앞에서 끊겼다.

### 17.5 남은 것 (갱신)
- ~~WHO 원문 47~61쪽(B.1 서술 섹션)~~ → **§18에서 확보, 아래 참고**
- Saragiotto 2016 Cochrane 원문 — 여전히 미확보 (로컬에 있다고 하셨으나 미업로드)
- TBC가 급성/만성 어느 쪽에 적용되는지 — 여전히 미확인

---

## 18. WHO 2023 B.1 서술 섹션 확보 (원문 47~54쪽) — 운동 종류별 근거 표 포함

세 번째 WHO PDF 파일(`who___3.pdf`, 하네스 보고 230쪽/22.4MB, pymupdf 실제
43쪽 — 역시 이미지 스캔본, 렌더링 후 확인)이 정확히 **원문 48쪽부터** 시작해서
§17.4에서 요청했던 B.1 서술 섹션을 확보했다. 지금까지 읽은 범위는 원문
48~54쪽(이 파일의 1~8페이지)이다.

### 18.1 정의 (원문 48쪽)

> "운동은 신체 활동의 하위 범주로, 신체 체력의 하나 이상의 구성요소를
> 향상·유지하는 것을 목적으로 계획적·구조적·반복적이다. 구조화된 운동
> 치료/프로그램은 보건의료 종사자가 처방하거나 계획하며, 지도·감독을
> 동반하고, 표준화되거나 개별 맞춤일 수 있다."

**자가 주도(self-directed) 걷기 프로그램 등 단순 신체활동과는 구분된다**고
명시 — 이건 우리 Core-20 중 #1/#2(짧게 걷기류)를 "구조화된 운동"이 아니라
"활동 재개"로 분류해 온 것과 정확히 일치하는 구분이다(§15.2에서 이미
같은 결론에 도달했었는데, 이번에 원문 근거를 확인한 것).

### 18.2 권고 원문 (원문 48쪽, §17.2와 동일 — 재확인)

> "성인(고령자 포함) CPLBP 환자에게 치료의 일부로 구조화된 운동 치료 또는
> 프로그램을 **제공할 수 있다**." — 조건부 권고, 사용 찬성, 낮은 확실성 근거.

### 18.3 Remarks(비고) — 실무에 바로 쓸 수 있는 내용

- **"현재까지의 근거로는 어떤 운동 양식(modality)이 다른 것보다 더/덜
  효과적이라고 결론 내리기에 근거가 불충분하다."** → 특정 운동 종류를
  "이게 더 낫다"고 서열화하면 안 된다는 뜻. 원장님이 처음 원하셨던 "레벨"은
  **효과 서열이 아니라 기능제한 정도에 따른 적합성**으로 설계하는 게 원문과
  맞다(우리가 이미 그렇게 방향을 잡은 것과 일치).
- **개인 선호·상황(접근성·비용·문화적 맥락)을 고려해 공동 의사결정으로
  운동 양식을 정하라**고 명시.
- **"최소 20시간의 총 프로그램 시간을 달성하도록 순응도를 높이는 것이
  일반적으로 더 유익하다."** — 이번에 원문으로 처음 확인한 구체적 용량
  기준. (개인 맞춤/감독/순응도 독려와 묶여서 나온 조건이며, "20시간 넘게만
  하면 된다"는 식의 단일 컷오프로 오독하면 안 된다.)
- 고령자는 **내재역량(intrinsic capacity)** — 동반질환, 역량 저하, 운동
  내약성 저하, 무감독 프로그램의 부작용 위험 — 을 신중히 평가해야 한다고
  명시. (WHO ICOPE 핸드북 인용)
- 그룹 형식이 순응도를 높일 수 있음(사회적 참여, 특히 고령자).
- **"단일 개입으로 제공하지 말고, 생물심리사회적 평가에 기반한 더 넓은
  치료 묶음의 일부로 고려해야 한다."**

### 18.4 근거 요약(정량 리뷰) — 원문 49쪽

- 기존 Cochrane 리뷰(89)를 갱신·확장한 것이 근거 합성의 기반.
- **최종 합성: 68개 RCT, 참가자 4195명**(연구당 14~313명), 2018-04-28~
  2022-05-17 수집분 + 그 이전 저위험/불명확위험 편향 연구 포함.
- 국가 분포: 고소득국 10개국(호주3, 캐나다2, 프랑스1, 독일3, 이탈리아1,
  일본1, 네덜란드1, 한국4, 영국1, 미국2), 중상위소득 5개국, 저소득~중하위
  5개국.
- 평균 연령 20.4~74.3세. 고령자(60세+) 대상 9개 연구(524명).
- **운동 양식별 시험 수**: 유산소 11, 코어강화 21, 일반근력훈련 5, 혼합
  (2종 이상) 14, 필라테스 2, 스트레칭/유연성 8, 요가 2, 운동조절(motor
  control) 7, 태극권 2, 기공 2. **"기능회복운동(functional restoration
  exercise, 단일 양식으로서)"을 평가한 연구는 0건.**
- 서브그룹 분석은 "대부분 유의한 차이를 보이지 않거나 이질성을 충분히
  설명하지 못해 해석에 주의가 필요하다"고 원문이 직접 경고.

### 18.5 Table 7·8 — 운동 양식별 결과 표 (원문 51쪽·53쪽, 표 그대로 옮김)

**주의**: 원문의 표시는 "no trials(시험없음)/little-to-no-difference
(certainty 등급별)/benefit(임상적으로 유의한 차이, certainty 등급별)"
6단계 척도다. 아래는 그 등급을 요약한 것이며 정확한 세부(빈 원 개수 등)는
원문 표를 직접 봐야 한다.

**Table 7 — "sham(위약)" 대비** (통증/기능 × 즉시/단기/장기):
| 양식 | 즉시 통증 | 장기 통증 | 즉시 기능 |
|---|---|---|---|
| 코어강화 | **이득(매우낮은 확실성)** | 시험없음 | **이득(매우낮은 확실성)** |
| 운동조절(motor control) | 차이없음 | **이득(매우낮은 확실성)** | **이득(매우낮은 확실성)** |
| 근력훈련/스트레칭 | 차이없음(낮은 확실성) | 시험없음 | 차이없음 |
| 유산소/혼합/필라테스/기공/태극권/요가 | 시험없음(sham 대조 자체가 없음) | | |

**Table 8 — "무개입/운동 효과 단독 분리" 대비**(더 많은 양식에서 이득 확인):
유산소·코어강화·혼합·필라테스·기공·스트레칭·태극권 대부분 즉시/장기 통증·
기능에서 **이득**(단, 모두 매우낮은~낮은 확실성). **운동조절과 근력훈련만
"무개입" 대비에서는 오히려 차이없음**으로 나온 게 흥미로운 지점 —
운동조절은 sham 대비로는 이득이 보였는데 무개입 대비로는 차이없음이라는
뜻으로, **비교 대상이 뭐냐에 따라 같은 양식의 결론이 바뀐다.**

### 18.6 이게 우리 레벨 표에 주는 실무적 함의

1. **"어느 게 더 나은 운동이냐"는 원문 자체가 답을 안 준다(§18.3
   불충분 근거 명시)** — 우리가 레벨을 "효과 우열"이 아니라 "기능제한
   정도에 맞는 적합성 + 안전 순서"로 설계하려던 원래 방향이 원문과
   부합한다. 이 방향을 다시 흔들 필요 없다.
2. **20시간 총 프로그램 시간**은 "얼마나 지속해야 하는지"에 대한 유일하게
   구체적인 원문 수치다. 우리 앱에 "최소 몇 주/몇 회" 안내 문구를 넣는다면
   이 20시간을 참고 근거로 인용할 수 있다(단, 조건부·저확실성 권고의
   부속 remark라는 것도 같이 명시해야 함).
3. **단일 개입으로 제공하지 말라**는 원문 remark는, 우리 앱이 운동만
   따로 떼어 "처방"처럼 제시하기보다 전체 문진·판단 맥락 안에서 제시해야
   한다는 기존 설계와 맞다 — 추가 조치 불필요, 확인 차원.
4. 고령자 관련 remark(내재역량 평가, 무감독 부작용 위험)는 **우리 문진에
   "고령/동반질환" 관련 필드가 있다면 그 필드와 연결할지 검토할 만한
   지점**이다 — 이번 리서치 범위 밖이라 여기 기록만 해둔다.

### 18.7 (정정) 원문 55~60쪽도 이어서 확인함 — B.1 섹션 전체 완료

**이전 버전에서 "원문 55쪽 이후 미확보"라고 적은 건 제 실수였다.** 실제로는
같은 파일(`who___3.pdf`, 43페이지)에 55~89쪽이 전부 들어있었는데, 앞 8페이지
(48~54쪽)만 읽고 "여기까지가 이 파일의 전부"인 것처럼 보고했다. 원장님이
지적하셔서 이어서 읽었고, **B.1(구조화된 운동) 섹션은 정확히 원문 60쪽에서
끝나고 61쪽부터는 B.2(침 치료/니들링요법)로 — 완전히 다른 개입 항목으로
넘어간다.** 즉 원장님이 요청하셨던 "B.1 서술 섹션"은 이제 48~60쪽 전체를
확인했다.

**55~58쪽 — 근거 요약 계속**:
- 무개입/저사용(usual care) 대비 Table 9(양식별): 유산소는 기능 즉시개선만
  이득, 나머지는 대부분 시험없음이거나 소수 이득. **요가만 유일하게 여기서
  즉시·장기 통증/기능 모두 "차이없음(매우낮은 확실성)"으로 나옴** — Table
  7/8/9를 합쳐 보면 **양식별 결론이 "무엇과 비교하느냐"에 따라 계속 뒤집힌다**
  는 §18.5의 관찰이 세 번째 표에서도 재확인된다.
- 이전 Cochrane 리뷰(38개 연구)로부터의 간접 근거: 조기 추적관찰 시점 기준
  통증 감소(작은 효과, 저확실성)·기능 개선(미미한 효과, 저확실성).
- 이상반응을 체계적으로 보고한 12개 연구 중 **위해는 매우 적고 경미**했다고
  원문이 명시. 위해에 대해서는 GRADE 평가를 수행하지 않았다.
- **정성적 리뷰(질적 근거)**: 고령자는 운동의 "연속성"(중단 시 악화 위험)과
  "그림·설명이 포함된 교육자료 병행"을 중요하게 여김. 나이지리아 농촌 지역
  사례에서는 "운동이 정당한 치료로 인식되지 않아 낙인이 될 수 있다"는 형평성
  우려가 확인됨(신뢰도: LOW). 그룹 형식은 사회적 지지·순응도 향상 때문에
  선호도가 높음(신뢰도: MODERATE). 일부는 기존 치료 실패 후 "자가관리
  수단"으로 운동을 채택("실험해보는 것"으로 인식)(MODERATE). 짧은 세션을
  선호(VERY LOW).

**59~60쪽 — GDG(가이드라인개발그룹) 판단 근거, 가장 중요한 대목**:
- GDG는 "전체적 이득은 작음~중간 크기이지만 임상적으로 의미있다"고 판단,
  위해는 경미하다고 판단(단, 고령자는 위해 모니터링 연구 자체가 적어
  불확실). 전체 근거 확실성은 성인 전반 low~very low, 고령자는 very low.
- **"특정 운동 양식을 권고하는 대신, GDG는 어떤 프로그램 특성(설계·전달
  방식·용량)이 통증·기능에 가장 유익한지를 다룬 선행 체계적 문헌고찰을
  참조했다"** — 이게 §18.3의 "20시간" 수치의 정확한 출처다. 개별화·감독·
  높은 용량(20시간+) 프로그램이 일반적으로 더 유익했다는 것.
- **위원 2명은 합의된 "조건부 권고"에 동의하지 않고 "강한 권고"가 더
  적절하다는 소수 의견을 냈다** — 원문에 명시된 실제 이견.
- **가장 중요한 구분 하나**: "이 근거 검토에서 고려한 운동 개입은 보건의료
  종사자가 처방·전달하는 구조화된 프로그램에 초점을 맞췄다. **자가 선택·
  자가 주도 운동/신체활동(예: 걷기, 수영, 자전거)은 [이 권고의 근거 검토
  범위에] 포함되지 않았다.**" 그리고 "**구조화된 프로그램이 전반적 건강과
  웰빙을 위한 자가 선택·자가 주도 운동/신체활동을 반드시 대체할 필요는
  없다**"고 명시하며, WHO의 별도 신체활동 가이드라인(모든 성인·고령자
  대상 강한 권고)을 참조하라고 안내한다.

  **이건 §18.1에서 이미 확인한 구분(구조화된 운동 vs 자가주도 활동)을
  다시 한번, 더 명시적으로 확인해주는 대목이다** — 우리 Core-20의 #1/#2
  (짧게 걷기류)를 "치료적 처방 운동"이 아니라 "자가주도 활동재개"로 분류해
  온 것이, WHO 권고의 적용 범위 밖에 있다는 뜻이지 "근거가 부족하다"는
  뜻이 아니다. 오히려 WHO는 이런 자가주도 활동을 별도 가이드라인으로
  강하게 권장하고 있다.

### 18.8 남은 것 (재갱신)
- 원문 61쪽부터는 B.2(침 치료/니들링요법)로 — 이건 "운동 라이브러리"와
  무관한 별개 개입 항목이라 이번 리서치 범위 밖으로 판단해 더 읽지 않음.
  필요하시면 별도로 알려주시면 확인하겠음.
- Saragiotto 2016 Cochrane 원문 — 여전히 미확보
- TBC가 급성/만성 어느 쪽에 적용되는지 — 여전히 미확인
