...
assert('0: exams 12 (archive 4 + 6 + 2 from the 2026-09-07 original cross-check), direct-support 8 keys; effusion / joint line / target-function / patellar apprehension have no pair', PACK.clinicianAddableExams.length === 12 && ['knee_exam_target_function_reproduction', 'knee_exam_patellar_mobility_apprehension'].every((id) => PACK.clinicianAddableExams.some((x) => x.id === id)) && Object.keys(PACK.directSupportByExam).length === 8 && ['knee_exam_effusion', 'knee_exam_joint_line', 'knee_exam_target_function_reproduction', 'knee_exam_patellar_mobility_apprehension'].every((id) => !(id in PACK.directSupportByExam)))
assert('0: the PATELLAR_INSTABILITY hypothesis now has its matching exam (original §5-G)', PACK.hypothesisPatterns.some((p) => p.id === 'PATELLAR_INSTABILITY') && PACK.examHelp.knee_exam_patellar_mobility_apprehension.whyKo.includes('슬개골 불안정'))
