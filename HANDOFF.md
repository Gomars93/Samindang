...
- **검수 4편 완료(01 침 · 02 약침 · 06 매선 · 07 재활), 3편 미착수(03 부항 · 04 추나 · 05 도침).** 인용 검증은 WebSearch로 수행했다 — 이 환경의 egress가 PubMed·Europe PMC 직접 접근을 차단해 원문 fetch가 불가능하다.
...
2. **매선 미해결 4건 → 2건.** 회전근개(Park YC 2026, PMID 41496087)와 무릎 OA(Zhao 2025, PMID 41431000) **해소 완료**. 남은 2건 = ① DR_06 신규 근거를 §6까지 전면 반영 ② `_공통-안전관리.md` 소재 확인. 둘이 끝나고 PO 승인이 나면 `status: SSOT` 승격.
   - **선결 과제**: v1.1 파일을 Drive에 원본 형식으로 올릴 경로 확보(위 커넥터 제약). Obsidian 볼트 직접 배치 또는 `.md` 보존 업로드 수단 중 택일 — PO 판단 필요.
3. **저장소 PR 3건은 아직 시작 안 함** (PO가 "수정은 아직 하지 말아주고"). 승인된 계획은 `DECISIONS.md` 참조 — ① 원칙 기록(문서 3) ② 태블릿 +2 문항(소스 4~5 / 테스트 5) ③ 원장 Task Response 블록(소스 8~10). **②는 `questionnaire-volume`의 요통 프로필 3개 고정값(현재 23화면/46탭)을 손으로 갱신해야 한다.**
