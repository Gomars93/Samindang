...
## 16. Batch 4.1-B/C 브리프 — 명리 표시 제거 + 사주 자유서술 2필드 제거 (PO 결정 2026-09-04, Fable 설계)

### 16.0 PO 답변으로 확정된 것

> 1. 남기자 / 2. 응 빼줘 / 3. 이해 못함
> **"사주 넣어보는 프로그램이 별도로 있어"**

- **§15.6(1) → 남긴다.** `BIRTH_01`(생년월일) / `BIRTH_02`(양·음력) /
  `BIRTH_02A`(윤달) / `BIRTH_03A`(시간 확신도)는 원장 화면에 그대로 둔다.
  **근거가 강화됐다**: 대표님이 별도 프로그램에 입력하려면 생년월일이 필요하다.
- **§15.6(2) → 뺀다.** `선천 특징`(`innate_features`) / `현재 증상과 연결되는
  핵심`(`symptom_links`) 입력을 제거한다 (**4.1-C**).
- **§15.6(3)** — 질문이 불명확했다. 아래 16.4에서 다시 설명하고 **Fable이
  추천안 하나로 확정**한다(PO 반대 없으면 그대로 간다).
- **§15.4의 유일한 미확인 리스크가 해소됐다.** "대표님이 원국을 어디서 보는지
  모른다"가 막고 있었는데, **별도 프로그램**이라는 답으로 원장 화면의 원국 계산
  표시는 중복임이 확정됐다. **4.1-B 진행.**

### 16.1 (출력 방향) 제거 경로 R3 — `선천 특징` / `현재 증상과 연결되는 핵심`

제거 대상: `src/doctor/JudgmentPanel.tsx:368-378` (`judgment__grid` 안의
`TextList` 2개) + `:505-523` (설명 개요의 두 `<li>` read-back).

**전수 확인 결과(grep, `src/` 전체)** — 이 두 필드가 지금 도달하는 곳은 **4곳뿐**:

| 필드 | 도달처 | 초진 | 재진 | 한약 | mixed | fixture | 제거 후 |
|---|---|---|---|---|---|---|---|
| `innate_features` | JudgmentPanel `TextList` 입력 | 보임 | 보임 | 보임 | 보임 | 보임 | **제거** |
| | 설명 개요 read-back | 보임 | 보임 | 보임 | 보임 | 보임 | **제거**(R4로 disclosure 자체가 사라짐) |
| | `judgmentRecordedFieldCount` 배지 | 반영 | 반영 | 반영 | 반영 | — | **제거** |
| | 서버 저장(PUT judgment) | 저장 | 저장 | 저장 | 저장 | — | **유지** (타입/검증/`finalizeJudgment` 그대로) |
| `symptom_links` | 위 4곳 동일 | 〃 | 〃 | 〃 | 〃 | 〃 | 〃 |

**EMR(pain/herbal 양쪽), 환자 안내문, CRM, 이전 방문 카드 — 어디에도 가지 않는다.**
따라서 **대체 경로가 필요한 값이 하나도 없다.** 전부 PO 지시에 따른 **의도적 폐기**이며,
이미 저장된 값은 타입 유지 덕에 round-trip 보존되고 `원본 JSON`에서 계속 보인다.

**타입·검증은 반드시 유지한다** (4.1-A와 같은 이유, 그러나 근거가 더 강하다):
`tests/server.spec.mjs`가 **`innate_features`를 저장·CAS(낙관적 잠금) round-trip
프로브로 쓴다**(`:212`, `:231`, `:325`, `:357-391`). `server/**`는 FROZEN이므로
`ClinicianJudgment.innate_features`/`symptom_links`, `MAX_*` 상수,
`validateJudgment`, `finalizeJudgment`는 **전부 그대로 둔다** — `tests/server.spec.mjs`
와 `tests/doctor.spec.mjs:1006-1051`이 **zero-diff로 통과해야 한다.**
UI만 제거하고, 타입 4줄에 4.1-A와 같은 deprecated 주석을 단다.

### 16.2 (출력 방향) 제거 경로 R4 — `설명 개요` disclosure

`judgment__outline`(`:526-575`)의 `<ol>` 항목은 원래 4개였다:

| 항목 | 상태 |
|---|---|
| 선천 특징 | R3로 소스 제거 |
| 현재 증상 연결 | R3로 소스 제거 |
| 치료 우선순위·한약 방향 | **4.1-A에서 이미 제거됨** |
| 질문 | `outlineQuestion` — **로컬 `useState('')`, 저장되지 않고 어디서도 읽히지 않음**(grep 확인: `:160` 선언, `:530` 사용, 그 외 0곳) |

즉 R3 이후 이 disclosure에 남는 것은 **한 번도 저장된 적 없는 스크래치 메모 한 칸**
뿐이고, 스스로 밝힌 목적("원장이 입력한 내용을 그대로 **재구성해서** 보여줄 뿐")의
재구성 대상이 **0개**가 된다. `1. 질문` 하나짜리 번호 목록만 남는다.

→ **disclosure 전체를 제거한다.** 손실은 0이다(`outlineQuestion`은 저장 경로가
애초에 없었다). 이것은 PO 지시의 직접적·기계적 귀결이며, CLAUDE.md 규칙이 요구하는
**"지운 쪽이 아니라 안 지운 쪽 화면을 보라"**의 적용 결과다.

### 16.3 (출력 방향) 제거 경로 R2 — `명리` 아코디언 (§15.4 그대로, 리스크 해소됨)

§15.4의 표를 그대로 적용한다. 대체 표시도 §15.4 그대로:
`<Field qid="BIRTH_03" label="출생 시간대" value={r.birth_info.birth_time_branch} />`
(`viewProfile !== 'pain'` 게이트 유지). `src/saju/`·`computeSaju`·payload의
`myungri_calculation`은 **전부 유지** — 계산·저장은 계속되고 원장 화면 렌더링만 없어진다.
`원본 JSON` 아코디언에도 그대로 남는다.

죽은 export(`MyungriCompactCard`/`sajuStatusLine`/`myungriGroupCount`) 처리도
§15.5 그대로 **남긴다**(읽기 전용 표시 함수 + 12차/13차 하드닝 회귀 테스트 보존).

### 16.4 §15.6(3) 다시 설명 — 아코디언 이름 (Fable 확정)

**무엇이 문제인가**: 원장 화면 `참고` 영역의 접이식 상자 하나가 지금
**`명리·감사 기록`**이라는 이름을 달고 있다. 그 안에는 원래 사주 관련 입력이
들어 있어서 그 이름이었다. 그런데 4.1-A가 사주 검증 4칸을 뺐고, **4.1-C가
선천 특징·증상 연결까지 빼면 그 상자 안에 "명리"라 부를 것이 하나도 안 남는다.**
남는 것은:

| 남는 내용 | 성격 |
|---|---|
| 객관적 근력저하 / 회전근개 소견 read-only echo | 안전 표시 (입력은 진료 탭) |
| `기록` 버튼 + 저장 상태 + 충돌 배너 | 저장 |
| `기록된 판단 (JSON)` | 원자료 |
| `1분 디브리핑 (선택)` 4문항 | 디브리핑 |
| `학습 케이스` 체크 | 학습 |

→ **상자 이름을 `디브리핑·학습 기록`으로 바꾼다.** 안쪽 제목(`원장 판단 기록`)은
그대로 둔다. `원장 판단 기록`을 상자 이름으로 쓰지 않는 이유는, 진료 화면의
`판단·처치` 레인(FinalAssessmentCard)과 **같은 것으로 오해되면 안 되기 때문**이다
— 그게 원래 `명리·감사 기록`이라는 이름이 존재했던 이유이기도 하다.

**PO가 다른 이름을 원하면 그 한 단어만 바꾸면 된다. 다른 설계는 영향받지 않는다.**

### 16.5 범위 밖 관찰 (기록만, 이번에 건드리지 않음)

4.1-C 이후 `JudgmentPanel`에 남는 실제 입력은 **`1분 디브리핑` 4문항 + `학습 케이스`
체크 둘뿐**이고, 디브리핑은 스스로 "녹취 연동은 이후 단계이며 지금은 데이터 계약만
준비되어 있습니다"라고 적고 있는 **미배선 자리표시자**다. 이 패널 자체의 존치 여부는
별도 판단 대상이지만, **PO가 지목하지 않았으므로 이번 범위에 넣지 않는다.**

### 16.6 테스트 (§15.7 확장)

**⚠️ 규약 적용**: DECISIONS 2026-09-04 "테스트 규약 2건"에 따라, 한글 문자열 단언은
**렌더된 `html` 대상**으로 쓴다(raw 한글이 살아 있음). `.cjs` 번들 텍스트에
한글로 단언해야 하면 `esbuildEscapeNeedle()`을 쓴다 — 그냥 쓰면 **항상 통과하는
공허한 단언**이 된다.

제거 단언:

| # | 경로 | 단언 (herbal fixture 렌더 html 기준) |
|---|---|---|
| T6 | R2 명리 아코디언 | herbal·mixed 렌더에 `명리 검토` / `오행 분포` / `4주 8자` / `일간:` 이 없다 |
| T13 | R3 입력 | `핵심 선천 특징` / `현재 증상과 연결되는 핵심` 라벨이 **어느 프로필에서도** 렌더되지 않는다 |
| T14 | R4 설명 개요 | `설명 개요` / `judgment__outline` 이 렌더되지 않는다 |
| T15 | R3 배지 | `judgmentRecordedFieldCount`가 `innate_features`/`symptom_links`**만** 채워진 judgment에 `0`을 반환한다 |
| T18 | 아코디언 목록 | `참고` 그룹 목록에 `명리`가 없고, `명리·감사 기록` 대신 `디브리핑·학습 기록`이 있다 (`doctor.spec.mjs:3331` 갱신) |

유지 단언(회귀 방지) — **이게 이번 배치의 핵심이다. 지운 쪽이 아니라 안 지운 쪽:**

| # | 단언 |
|---|---|
| T7 | herbal 렌더에 `출생 시간대` 라벨 + 해당 지지 문자열(예: `인시`)이 함께 보인다 |
| T8 | mixed 렌더에서도 T7이 성립한다 |
| T9 | **pain 렌더에는 `출생 시간대`도 어떤 `BIRTH_*` 값도 보이지 않는다** (PR #24 invariant) |
| T10 | 출생시간 `unknown` fixture에서 `잘 모르겠어요`가 보이고 `출생시간 확인됨` 류 문구는 어디에도 없다 |
| T20 | **PO 결정(1) 유지**: herbal 렌더에 `생년월일`·양음력·윤달·시간 확신도가 **여전히 보인다** — 4.1-B가 이것들까지 지우지 않았음을 증명 |
| T21 | **서버 round-trip 보존**: `tests/server.spec.mjs` **zero-diff**로 통과 (`innate_features`를 CAS 프로브로 쓰는 테스트 포함) |
| T22 | `doctor.spec.mjs:1006-1051`의 `createEmptyJudgment`/`validateJudgment`/`finalizeJudgment` 단언들이 **무변경으로** 통과 |
| T23 | `1분 디브리핑` / `학습 케이스` / `기록` 버튼 / 객관적 근력저하 echo는 **여전히 렌더된다** |
| T11/T12 | 4.1-A의 `O` 경계·6키 단언 그대로 통과 |

**mutation 검증 필수**: T6·T13·T14·T15·T18 각각에 대해 **제거를 되돌린 상태에서
그 단언이 실제로 실패하는 것**을 보이고 원복한 기록을 남긴다.
...
## 18. 실제 환자 파일럿 계획 (PO 승인 2026-09-04, Fable 설계)

### 18.0 왜 지금인가 — 그리고 왜 이게 다음 기능보다 급한가

지금까지 만든 것 중 **환자로 검증된 것이 하나도 없다.** 그동안 쌓인 증거:

- PO가 "치료 직후 값은 거의 안 적는다" 한 마디 하자 **입력칸 3개가 즉시 사라졌다.**
- PO가 "사주 넣어보는 프로그램이 별도로 있어" 한 마디 하자 **아코디언 2개와 컴포넌트 1개가 통째로 사라졌고**, Fable이 며칠 붙잡고 있던 리스크가 즉시 해소됐다.
- 이번 세션에서만 원장 화면 설계가 **4번**(4.1-A/B/C/D) 바뀌었다. 전부 환자 0명 상태.
- Fable의 설계 실수 2건(§17.0의 디브리핑 문항, §17.3의 import 의존성)은 **코드 리뷰로는 안 잡혔고, 화면을 직접 렌더하거나 빌드를 돌려서야** 잡혔다.

**패턴이 명확하다: 이 시스템의 남은 문제는 코드에 있지 않고 "실제 진료 흐름과 화면의 불일치"에 있다.** 그건 리뷰로 못 찾는다. 기능을 더 얹으면 검증 안 된 표면만 늘어난다.

### 18.1 파일럿 범위 — 최소로 잠근다

| 항목 | 값 | 이유 |
|---|---|---|
| 환자 수 | **2~3명** | 문제는 3명이면 다 나온다. 많으면 고치기 전에 데이터만 쌓인다 |
| 주호소 | **요통(LBP) 초진만** | v1이 유일하게 end-to-end로 완성된 경로. 재진은 초진 데이터가 있어야 성립 |
| 제외 | 한약 단독, mixed, 재진, 응급 의심 케이스 | 응급은 파일럿으로 검증할 대상이 아니다(아래 18.3) |
| 기간 | **하루 진료 안에서** | 며칠 끌면 원장 기억이 흐려지고 관찰이 인상평으로 바뀐다 |
| 병행 | **기존 방식(수기/구두 문진)을 반드시 병행** | 시스템이 멈춰도 진료가 안 멈춰야 한다 |

### 18.2 착수 전 반드시 참인 것 (하나라도 아니면 파일럿 금지)

1. **Batch 4 게이트 CLOSED** (Opus closing 재검수 통과).
2. **로컬 handoff 서버가 클리닉 LAN 전용**이고 인터넷에 노출되지 않는다
   (`docs/RUNBOOK_LOCAL_HANDOFF.md` 확인).
3. **`.data/`가 `.gitignore`에 있고 실제로 커밋되지 않는다** — 파일럿 후
   `git status`로 재확인.
4. **환자에게 "문진 데이터가 진료 목적으로 이 태블릿/원내 서버에 저장된다"는
   설명과 동의**를 받는다.
5. **`URGENT_REVIEW` / 응급 의뢰 판정이 떴을 때 원장이 무엇을 하는지가
   파일럿 전에 문장으로 정해져 있다.** 이건 소프트웨어가 아니라 **원장의 임상
   행동 규약**이다 — 화면이 뭘 띄우든 최종 판단과 조치는 원장이 한다.
   **정해져 있지 않으면 파일럿을 하지 않는다.**
6. **중단 절차**: 태블릿이 멎거나 서버가 안 뜨면 **그 자리에서 기존 방식으로
   전환**하고 그 사실을 기록한다. 환자 앞에서 디버깅하지 않는다.
7. **Opus closing 재검수가 지적한 게이트 차단 결함 5건(H-1/H-2/M-1/M-2/M-3)이
   전부 수정되고, 살아남았던 변이 4개(m6/a4/m7/c9)가 죽는 것이 확인**되어 있다
   (`docs/LBP_V1_BATCH4_AND_41_OPUS_CLOSING_REVIEW_v0.1.md`). 이 중 H-1/H-2는
   `O | 객관적 소견` 경계와 미평가-정상 혼동 위에 있는 구멍이므로, **수정 전
   파일럿은 금지**다.

### 18.3 파일럿이 검증하는 것 / 검증하지 않는 것

**검증한다** — 리뷰로 못 찾는 것만:

| 구간 | 관찰 질문 |
|---|---|
| 태블릿 문진 | 환자가 **끝까지** 하는가? 총 몇 분? **어느 문항에서 멈칫하거나 되묻는가?** |
| 원장 화면 진입 | 진료 시작 후 **몇 초 안에** 필요한 걸 찾는가? 스크롤을 몇 번 하는가? |
| 레인1 안전 확인 | 안전 항목을 **실제로 읽는가**, 아니면 넘기는가? |
| 레인2 확인 | 제안된 이학적 검사 중 **실제로 시행한 것 / 무시한 것**은? 무시한 이유는? |
| 판단·처치 | 어떤 칸을 채우고 **어떤 칸을 끝까지 비워두는가?** (비워두는 칸 = 다음 제거 후보) |
| 재활 운동 | 제안을 **채택하는가**? 채택 안 했다면 이유는? |
| EMR 6키 | 복붙 후 **손으로 고친 부분이 어디인가?** (고친 곳 = 형식이 틀린 곳) |
| 환자 안내문 | 환자가 **읽는가**? 질문하는가? |

**검증하지 않는다** (파일럿의 목적이 아님):
- 임상 판정 로직의 정확도 — 이미 FROZEN이고 시뮬레이션 검증됨. 3명으로는 통계가 안 됨.
- 응급 판정의 민감도/특이도 — 응급 케이스를 파일럿에 넣지 않는다.
- 성능·부하.

### 18.4 기록 방법 — 인상평이 아니라 데이터로

**환자 1명당 아래를 그 자리에서 기록한다.** 진료 끝나고 몰아서 쓰면 관찰이 아니라 회상이 된다.

1. **시간 3개**: 태블릿 문진 소요 / 원장 화면 첫 진입~첫 처치 결정 / EMR 복붙~저장 완료.
2. **막힘 지점**: 환자든 원장이든 **멈칫한 순간과 그때 보던 화면**. 한 줄씩.
3. **안 채운 칸 목록**: 화면에 있었지만 비워둔 칸 전부.
4. **손으로 고친 EMR 부분**: 원문 → 고친 내용.
5. **"이건 왜 여기 있지?" 순간**: 원장이 불필요하다고 느낀 표시.
6. **기존 방식 대비**: 더 빨랐나 느렸나. 놓친 것이 있었나.

> **환자 개인정보(문진 답변 실제 값, 이름, 생년월일)는 이 기록에 남기지 않는다.**
> 필드명·화면명·소요시간 수준에서만 적는다. 이 규칙은 `CLAUDE.md`의 기존
> 규칙과 동일하며, 파일럿 기록이 `docs/`에 커밋될 것이므로 특히 중요하다.

### 18.5 중단 기준 (kill criteria)

아래가 발생하면 **그 환자에서 즉시 중단하고 기존 방식으로 전환**한다:

- 안전 관련 표시가 **틀린 값**을 보여준다 (예: 미평가를 정상으로).
- EMR 텍스트에 **환자가 하지 않은 말**이 원장 확인 소견처럼 들어간다 (`O` 경계 위반).
- 환자가 문진 중 **불편·불안을 표현**한다.
- 시스템이 멎거나 저장이 실패한다.

앞의 두 가지는 **중단으로 끝나지 않는다** — 파일럿 전체를 멈추고 원인을 규명한 뒤
다시 시작한다. 이 둘은 이 시스템이 절대 하면 안 되는 것으로 정의한 바로 그것이다.

### 18.6 파일럿 후 처리

1. 기록 6종을 `docs/LBP_V1_PILOT_OBSERVATION_v0.1.md`로 정리(개인정보 없이).
2. **"안 채운 칸"과 "고친 EMR 부분"을 먼저 본다** — 이게 다음 배치의 입력이다.
3. **§13(Batch 2.7-A) 운동 준비조건 설계를 그대로 쓸지 고칠지 여기서 결정한다.**
   이 결정을 위해 파일럿을 기다린 것이다.
4. 파일럿에서 나온 문제를 고치기 전까지 **새 기능을 추가하지 않는다.**

### 18.7 Fable의 솔직한 예상

**3명이면 "안 채우는 칸"이 최소 3개는 나온다.** 지금 화면에 남아 있는 자유입력
칸 중 진료 중에 실제로 채워지는 것은 절반 이하일 것이다 — 지난 두 번의 PO 한마디가
각각 3칸/2개 아코디언을 즉시 지운 것이 그 근거다.

**EMR 6키는 형식이 아니라 어순에서 문제가 날 가능성이 높다.** 원장이 실제로
쓰는 순서와 `C/C → O/S → S → O → A → P` 순서가 다르면 복붙 후 재정렬하게 되고,
그러면 6키 고정의 이점이 사라진다. **"손으로 고친 부분"을 반드시 기록해야 하는
이유가 이것이다.**

**가장 놓치기 쉬운 것**: 파일럿이 잘 굴러가면 "됐다"고 판단하고 기능을 더 얹고
싶어진다. 파일럿의 성공은 "문제가 안 나온 것"이 아니라 **"문제를 3개 이상 찾은 것"**이다.
문제가 하나도 안 나왔다면 관찰이 부족했던 것으로 본다.
