...
최신 74가 PO 판단을 요청한 항목의 답이다. **「마무리」 단계 자체를 herbal
단독에도 만들었다** — 세 프로필이 같은 2단계 구조(진료 / 마무리)가 됐다.
...
| 그 방문에서 **재진 링크 발급** | ❌ → **✅** |
| 그 방문에서 **진료 완료** | ❌ → **✅** |
| CRM 복약 코스 | ❌ → ❌ (자기 가드 신설, PR-A 유지) |
| `HerbalWorkspaceNext` 4블록 | ❌ → ❌ (mixed 전용 가드 유지) |
| 점프 내비 (실제 진료 = server + patient_id) | 3개 → **4개** (안전·확인·판단·처치·**마무리**) |
| 점프 내비 (푸터 없는 미리보기·fixtures) | 3개 → **3개 그대로** — 갈 데 없는 버튼을 내지 않는다 (검수 1번) |

구조 변경: 바깥 프로필 가드 1개 → **블록별 자기 가드**. 빠뜨리기 쉬운 형태가
된 것은 사실이라(원래 바깥 가드를 쓴 이유가 그것) 블록 하나하나를 테스트로
명시 단언했다. `LaneJumpNav`의 `showNext`·`nextOnly`는 `showWrapup`으로
교체 — **처음엔 조건을 통째로 없앴다가 1차 검수 1번에서 되돌렸다**(기준이
프로필에서 내용물 유무로 바뀐 것이지, 조건이 사라진 게 아니다).

### PR #58 독립 검수 3건 (전부 CONFIRMED · 수정 완료)

| | 내용 |
|---|---|
| **1** | **죽은 버튼이 자리만 옮겨 되살아났다.** herbal 마무리에 들어가는 건 server 전용 `nextLaneFooter` 하나뿐이라, 미리보기에서 버튼이 원장을 **빈 화면**(헤딩만, 실측 155자)으로 데려갔다. 조건을 되살리되 기준을 **프로필 → 내용물 유무**로 바꿨다(`showWrapup`의 실인자; 3차 검수 2번으로 인라인됐다) — 옛 조건은 server 모드 herbal에서도 틀렸었다 |
...
### PR #58 2차 검수 4건 (코드 결함 0 · 전부 문서 정확성 · 수정 완료)

| | 내용 |
|---|---|
| **1·4** | `DECISIONS.md`/`HANDOFF.md` 표가 "내비 3개 → 4개"를 **무조건**으로 적음 — 1차 수정 이후 미리보기는 3개다. 앵커·래퍼(무조건) / 버튼(푸터 있을 때만) 두 행으로 갈랐다 |
| **2** | `HerbalWorkspace.tsx` 카드 JSDoc이 자리 근거로 "herbal엔 마무리가 없다"를 계속 듦 |
| **3** | `DoctorWorkspace.tsx` 마무리 주석 첫 문단이 PR-A 당시를 **현재형**으로 서술 |

**2번은 1차 검수 2번과 같은 부류 — 한 파일 고치고 옆 파일을 놓쳤다.** 같은 주장이
두 배치 연속 낡은 채 남았으므로 **테스트로 고정**했다(`PR #58 검수2`): 인용·철회는
정당하니 금지가 아니라 "가까이에 철회 표지가 있는가"로 판정하고, 줄바꿈을 눌러
두 줄짜리 문장도 잡는다. mutation 양방향 확인 완료.

### PR #58 3차 검수 3건 (코드 결함 0 · 테스트 품질 3건 · 수정 완료)

검수자가 "`wrapupHasContent === false`인 채 `visitStep === 'wrapup'`"이 **도달
불가**임을 직접 추적해 확인했다(환자 링크 해제 UI가 없어 푸터 게이트가
non-null → null로 갈 수 없다).

| | 내용 |
|---|---|
| **1** | **내가 만든 비공허성 가드가 산문에 결합돼 있었다** — 낡은 주장 스캐너의 `found >= 1`을 만족시키는 게 JSDoc 문장 하나뿐. **옆 파일 A-pre 주석에 "그러지 말라"고 적어둔 결합을 같은 배치에서 재현했다.** 합성 표본 3종으로 교체 |
| **2** | `wrapupHasContent` 선언이 고아 — 545줄 뒤에서 한 번 쓰였다. 호출부로 인라인 |
| **3** | A-5의 맨몸 `/showNext/`가 실재하는 `showNextVisitCheckItem`에 부분 일치. `\b`로 묶음 |

2번 수정이 A-4를 건드려(조건에서 이름을 읽는 것까지 위반으로 잡혔다) A-3·A-4를
**JSX 렌더 사이트** 기준으로 좁혔고, mutation 4종을 다시 돌려 확인했다.

### PR #58 4차 검수 3건 (코드 결함 0 · 수정 완료)

| | 내용 |
|---|---|
| **1** | **실측 단언이 또 공허했다** — `innerText`는 렌더된 텍스트만 돌려줘 `hidden`인 마무리 단계를 건너뛴다. 확인하려던 블록이 바로 거기 있다. 가드를 지워도 통과했다(mutation 확인). `textContent` + 대상 좁히기 + 자기점검으로 교체 |
| **2** | `navLabels.every(...)`에 `Array.isArray` 누락 — 내비가 없으면 이름 있는 실패 대신 uncaught TypeError |
| **3** | **`wrapupHasContent`는 코드에 없는 식별자** — 3차 검수로 인라인했는데 문서·주석 3곳이 현행 코드로 인용. 검수 2번과 같은 부류 |

**부재를 단언할 때의 규칙(두 번 당하고 확장):**
1. prop의 부재를 렌더로 단언하면 → 같은 prop을 넘긴 **대조군**을 함께.
2. 숨은 서브트리의 부재를 단언하면 → `innerText` 말고 `textContent`, 그리고
   **"대상 텍스트를 실제로 읽었는가" 자기점검**을 함께.
3. 둘 다: 단언을 쓴 직후 **그 단언이 막으려는 mutation을 실제로 넣어본다.**
   공허했던 둘 다 그 한 번을 생략한 자리에서 나왔다.

### PR #58 5차 검수 4건 (코드 결함 0 · 수정 완료)

검수자가 **새로 열린 herbal 경로가 빈 경로가 아닌지**까지 확인했다 — EMR 조립,
재진 발급(server가 pain+herbal 재평가 대상을 union), 그 응답을 띄우는 화면까지
다 있다. 쓰기만 되고 안 읽히는 필드 없음.

| | 내용 |
|---|---|
| **1** | `DECISIONS.md`/`HANDOFF.md`가 **철회된 근거를 현재형으로** 들고 있었다("조건이 가리킬 대상이 없다") |
| **2** | `tablet-viewport` 수치 3 어긋남(156 → 실제 **159**) |
| **3** | 경로×테스트 표가 "내비 3→4"의 근거로 실측 스위트를 들었는데 그쪽은 **3개**를 단언한다 |
| **4** | **A-0이 산문에 결합** — 래퍼 위 주석 존재까지 요구. **이 파일 자신의 A-pre 주석이 경고하는 결합**. 양방향 mutation으로 확인 후 수정 |

**검수 5회 중 4회가 같은 뿌리다: "코드를 고치면서 그 코드를 설명하는 글·검사하는
단언을 함께 옮기지 않는다."** 산문 결합 3회(A-5 → 스캐너 자기점검 → A-0),
공허한 단언 2회(복약 코스 → innerText). 규칙은 4차 검수 항목에 적어뒀다.

### PR #58 6차 검수 1건(latent) + PR 설명 갱신

검수자가 `{nextLaneFooter}` 렌더 사이트에 네 가지 변형 mutant를 넣어 A-4·검수1이
철자에 의존하는지 시험했고 네 개 다 잡혔다(자기 의심 기각). 코드 결함 0.

- **latent**: `showWrapup`의 실인자는 "herbal에 남는 건 푸터 하나뿐"을 전제한
  **열거식**인데 그 전제가 강제돼 있지 않았다. → **A-5d 신설**(레인 직계 렌더
  대상 4개를 고정, 실패 메시지가 `showWrapup`을 가리킨다). mutation 확인.
- **PR 설명이 낡아 있었다** — 5차까지 저장소 문서만 고치고 GitHub PR body를 안
  고쳐, 수치 4개와 **뒤집힌 근거**가 그대로였다. PO와 검수자가 읽는 문서가 바로
  그것이다. 같은 뿌리가 **저장소 밖**에서 한 번 더 난 것. 갱신했다.

### PR #58 7차 검수 — **진짜 결함 1건** (Batch 4 D-2 재현 직전)

여섯 번의 "코드 결함 0" 뒤 일곱 번째에 나왔다.

herbal 단독 화면의 「+ 다른 유형 입력 추가」로 원장이 **통증 판단 4칸을 편집할 수
있는데**, herbal EMR 조립은 그 키를 안 낸다. 복사가 없던 동안엔 안 보였지만 이
PR이 복사를 붙이면서 **원장이 타이핑한 판단이 빠진 텍스트에 "복사됨"**을 띄우게
됐다.

**내 필드×화면 표가 왜 못 잡았나**: 입력 방향에 "새로 편집 가능해진 필드는 없다"
— 사실이지만 초점이 틀렸다. 새로 생긴 건 **출력**이고, 그래서 **전부터 안 읽히던
필드가 그 순간 거짓말이 된다.**

> **규칙 보강**: 경로를 **여는** 변경에서는, 새 출력이 **그 화면에서 이미 편집
> 가능하던 모든 필드**를 싣는지 확인한다. 편집 UI가 이번에 생겼는지는 무관하다.

**수정은 8차 검수로 한 번 뒤집혔다.** 첫 판은 통증 블록을 통째로 붙였는데,
그러면 중복 `C/C:` · 환자 태블릿 `S:`(slim이 일부러 빼는 재진 인용 포함) · 빈
`O:`/`P:`가 딸려 온다 — slim의 존재 이유를 정면으로 어긴 수정이었다.

지금은 한약 빌더가 이미 쓰는 **optional 라벨 줄** 패턴으로, 적혀 있을 때만
`통증 판단(다른 유형 입력): …` **한 줄**을 낸다. 라벨은 통증 빌더와 공유
(`PAIN_FINAL_ASSESSMENT_LABELS`)하고 B-0e가 소스에서 대조한다. **slim일 때만**
넘긴다(mixed는 통증 블록이 따로 붙어 중복).

B-0a~B-0f(59단언) + mutation 5종. **B-0f 첫 판은 `?` 분기만 봐서 "mixed에도
넘기기"를 놓쳤다** — 이 PR에서 세 번째 만난 같은 교훈(부재를 함께 단언하라).

**PO 판단 필요 2건** (임상 문서 내용을 바꾸는 결정이라 이 PR에서 안 고쳤다):
1. **통증 화면의 거울상** — 거기서 편집 가능한 한약 판단이 통증 EMR에 안 들어간다. 이 PR 이전부터.
2. **herbal EMR에 재진 간단 문진 인용이 없다** — 화면엔 두 곳에 보이는데 복사엔 없다. 통증은 §14.1 S에 싣는다. 이 PR 이전부터.

### PR #58 9차 검수 5건 — 2건 수정, **3건 PO 판단 대기**

9차는 이 PR 밖(PR #55~#57이 남긴 것)까지 봤다. 이 PR이 만들거나 넓힌 것만 고쳤다.

**수정**
- **「근거 보기」가 마무리 화면에서 죽은 링크였다** — 목적지가 진료 단계 안이라
  `hidden`. F1과 같은 부류인데 이 링크만 빠져 있었다. herbal은 갈 단계가 없어
  **항상 살아 있었는데 이 PR이 새로 죽인다** → 고침(W-D0/W-D0b + mutation).
- `detail-check`의 `NRS_PROMPT` 키 수집이 브리틀 → 리터럴 구간만 스캔 + 자기점검.

**⚠ PO 판단 대기 3건** (전부 이 PR 이전부터 있던 것 — 승인 없이 끼워 넣지 않았다)

| | 내용 | 고치는 법 |
|---|---|---|
| **1** | **통증 화면의 D-2 거울상** — 거기서 편집 가능한 **한약 판단 4칸**이 통증 EMR에 안 들어간다. 원장이 적은 게 빠진 채 "복사됨". herbal 쪽과 똑같은 결함 | 방금 herbal에 넣은 optional 라벨 줄 패턴을 통증 빌더에 대칭으로. **"대칭으로 해줘" 한마디면 된다** |
| **2** | **herbal EMR에 재진 간단 문진 인용이 없다** — 화면엔 두 곳에 보이는데 복사엔 없다. 통증은 §14.1 S에 싣는다 | 차트에 무엇을 남길지의 판단 |
| **3** | **「간단 재확인」 카드가 herbal·mixed 확인 레인에 남아 있다** — PR #57의 이동이 통증에만 적용돼 mixed는 같은 답을 두 화면에 보여준다 | 아래 남은 작업 3번과 같은 건 |

### 이번에 스스로 잡은 두 가지

- **A-5 초안이 주석을 스캔했다** — `!/showNext/`가 폐기 이유를 적은 JSDoc의
  산문에 걸렸다. T-6과 같은 클래스. 주석 뗀 사본을 보게 고쳤고, 스트리퍼
  자기점검은 실제 소스의 낱말이 아니라 **합성 표본**으로 했다.
- **새 실측 단언이 mutation에서 이름 없이 죽었다** — 20초 타임아웃으로 같은
  뷰포트의 나머지 단언까지 날아갔다. 기다림 조건을 좁혀 이제 첫 뷰포트에서
  이름 있는 실패로 걸린다.

검증: `herbal-workspace-slim` 37→**59** · `doctor-workspace` 408→**419** ·
`tablet-viewport` 138→**159**(3 뷰포트 실측) · `doctor-reset-key` 12 ·
`tsc -b` 0 · `build` 0 · **`test:all` exit 0 (7233 단언)**.
Mutation **27종** 전부 이름 있는 실패로 잡힘 — 그중 검수 후 추가한 3종은
...
1. **실기기에서 한약 단독 진료를 끝까지 한 번** — 마무리 → EMR 복사 → 진료
   완료. fixtures 모드에는 `nextLaneFooter`가 안 들어오므로 자동 테스트는
   stand-in으로만 확인한다. 이 한 번이 이 배치의 진짜 확인이다.
2. **PO 승인 대기 중(계속)**: 환자 화면 NRS 눈금 양끝 문구
   (`전혀 불편하지 않음 ~ 매우 불편함` 등) — 내 초안이 PR #56/#57에 승인 없이
   들어가 있다.
3. **한약 「간단 재확인」** 카드를 통증처럼 마무리로 옮길지 — 이제 herbal에도
   마무리가 있으므로 가능해졌다. 별도 PO 판단.
...
### PO 판단 요청 (새로 생긴 것) — **해결됨: 최신 75 참고**

> 2026-09-26: PO가 "1번으로 진행해"로 아래 첫 번째 방법(「마무리」 단계를
> herbal에도 만든다)을 지시했고 구현·검증 완료. 아래는 당시 기록이다.
