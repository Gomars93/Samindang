...
## 6. 검사(8) → 직접 뒷받침(5쌍) — 확정
`shoulder_exam_rom → SH_MOB_01, SH_MOB_02` / `empty_can → SH_RC_01` / `er_resist → SH_RC_01, SH_RC_02` /
`scapular_control → SH_SCAP_01, SH_SCAP_02` / `apprehension_relocation → SH_STAB_01`. subscap·horizontal adduction·distal neuro는
쌍 없음(가설 근거 / 차단 게이트). 순위 버킷만 바꾼다.
...
- `tests/shoulder-exercise-core.vignettes.spec.mjs`(엔진 구동 27단언), `tests/region-pack.spec.mjs` 갱신(B-fallback·C·D·H·J·E-2·E-3).
