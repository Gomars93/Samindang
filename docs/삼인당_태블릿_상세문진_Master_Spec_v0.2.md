# 삼인당 태블릿 상세문진 Master Specification v0.2

- 문서 상태: Draft for Prototype — CORE_01 주상담 영역 정보구조 개편 반영
- 기준일: 2026-08-06
- 이전 버전: v0.1 (`삼인당_태블릿_상세문진_Master_Spec_v0.1.md`, 보존)
- v0.2 변경 범위: CORE_01 정보구조 / CORE_01B~E 신설 / `primary_goal_bucket` 신규 / `primary_goal_category` enum 확장 / CORE_02·CORE_03 show_if. 그 외 조항은 v0.1과 동일하다.
...
11인치 태블릿 사용성 검토 결과, 기존 CORE_01은 14개 선택지가 한 화면에 노출되어 선택 부담과 스크롤이 컸다.
또한 증상(수면·소화·통증) / 진료영역(여성 건강) / 상태(임신·산후) / 목표(체중) / 상담목적(체질·보약)이 한 화면에 혼재했다.

v0.2에서는 첫 화면을 `상담받고 싶은 부분` 중심의 9개 선택지로 재구성하고, 2차 분기로 세분화한다.

1. CORE_01 → `primary_goal_bucket` (9지선다, 신규 raw variable)
2. CORE_01B / CORE_01C / CORE_01D → `primary_goal_category` 2차 분기
3. CORE_01E → `primary_goal_other` free text (기존 CORE_01A 대체)
4. `primary_goal_category` enum에 `bowel`, `digestion_bowel`, `herb_consult` 추가
5. CORE_02 / CORE_03은 증상성 주소증에만 표시
6. `primary_goal_bucket`과 `primary_goal_category`를 **둘 다** 저장 (환자가 본 1차 선택 + Router용 정규화 값)

`skin`, `respiratory`는 v0.2에서 첫 화면 독립 버튼으로 두지 않으며 `other` free text로 흡수한다.
실제 환자 테스트 후 빈도가 높으면 추후 재검토한다.
...
## 3.2A 주상담 영역 raw / 정규화 값 병행 저장 (v0.2)

환자가 실제로 본 1차 선택(`primary_goal_bucket`)과 Router가 사용할 정규화 값(`primary_goal_category`)을
가능하면 **둘 다** 남긴다.

```json
{
  "primary_goal_bucket": "digestion_bowel",
  "primary_goal_category": "bowel"
}
```

```json
{
  "primary_goal_bucket": "sleep",
  "primary_goal_category": "sleep"
}
```

환자가 뒤로가기로 CORE_01 선택을 변경한 경우, 더 이상 표시되지 않는 2차 분기 화면의 응답은
current responses에서 `null`로 되돌린다(stale branch 값이 임상 응답으로 남지 않게 한다).
...
## CORE_01 주상담 영역 (v0.2 개편)

**질문**
> 오늘 어떤 부분을 가장 먼저 상담받고 싶으세요?
...
|---|---|
| 잠이 불편해요 | sleep |
| 속이나 대변이 불편해요 | digestion_bowel |
| 몸이 아파요 | pain |
| 소변·방광이 불편해요 | urinary |
| 기운이 없고 피곤해요 | fatigue |
| 여성·임신·산후 상담이에요 | women_reproductive |
| 스트레스·마음이 힘들어요 | stress |
| 체중 관리를 상담하고 싶어요 | weight_metabolic |
| 다른 증상·체질·보약 상담 | other_consult |

UI 규격:

- 800×1280 기준 9개 선택지를 한 화면에서 읽을 수 있어야 한다.
- 1열 배치 우선.
- 버튼 최소 높이 72px, 질문 32px, 답변 23px, 최소 터치영역 원칙 유지.
- 버튼/글씨를 축소해 억지로 맞추지 않는다.

### v0.1 → v0.2 CORE_01 선택지 이동

첫 화면에서 제거되었으나 데이터 의미는 유지된다.

| v0.1 첫 화면 값 | v0.2 위치 |
|---|---|
| digestion | CORE_01B |
| women | CORE_01C |
| pregnancy_postpartum | CORE_01C |
| constitution | CORE_01D |
| tonic_health | CORE_01D |
| other | CORE_01D → CORE_01E |
| skin | CORE_01D `other` + CORE_01E free text |
| respiratory | CORE_01D `other` + CORE_01E free text |

---

## CORE_01 직접 매핑

다음 `primary_goal_bucket`은 추가 화면 없이 내부적으로 `primary_goal_category`로 정규화한다.

```text
sleep            -> primary_goal_category = sleep
pain             -> primary_goal_category = pain
urinary          -> primary_goal_category = urinary
fatigue          -> primary_goal_category = fatigue
stress           -> primary_goal_category = stress
weight_metabolic -> primary_goal_category = weight_metabolic
```

환자에게 별도 화면을 보여주지 않는다.
...
## CORE_01C 여성·임신·산후 분기

show_if:
```text
primary_goal_bucket == women_reproductive
```
...
| label | value |
|---|---|
| 생리·갱년기 등 여성 건강 | women |
| 임신·산후 관련 상담 | pregnancy_postpartum |

※ 이 화면은 **상담목적만** 저장하며 임신 중/산후 상태를 확정하지 않는다.
정확한 상태 구분은 CORE_15 `reproductive_status`에서 확인한다.

---

## CORE_01D 다른 증상·체질·보약 분기

show_if:
```text
primary_goal_bucket == other_consult
```
...
| label | value |
|---|---|
| 다른 불편한 증상이 있어요 | other |
| 체질과 전반적인 몸 상태를 보고 싶어요 | constitution |
| 기력 보강·보약 상담을 받고 싶어요 | tonic_health |
| 한약이 필요한지 상담받고 싶어요 | herb_consult |

---

## CORE_01E 기타 증상 free text
...
※ v0.1의 CORE_01A를 대체한다.

---

## primary_goal_category enum (v0.2)

```text
sleep
digestion
bowel               <- v0.2 추가
digestion_bowel     <- v0.2 추가
pain
urinary
fatigue
women
...
constitution
tonic_health
herb_consult        <- v0.2 추가
other
```

v0.1에 있던 `skin`, `respiratory`는 첫 화면 선택지에서 제거되었으며,
해당 환자는 `other` + CORE_01E free text로 수집한다.

---

## 향후 Router mapping 메모

이번 Sprint에서는 상세 Module Router를 구현하지 않는다. 아래는 확정된 매핑 의도만 기록한다.

```text
primary_goal_category == digestion       -> GI
primary_goal_category == bowel           -> BOWEL
primary_goal_category == digestion_bowel -> GI + BOWEL
```

- `herb_consult`는 전용 모듈이 없으며, 6장 ROUTER의 Quick Screen 결과에 따른 모듈 활성화만 적용한다.
- 6장의 기존 Router 조건에서 `primary_goal_category == digestion` 항목은
  `digestion_bowel`도 GI를 활성화하도록 v0.3에서 조건식을 갱신한다.
...
이유: `체질상담이 언제부터 있었나요?` / `보약상담이 언제부터 있었나요?` 같은 부자연스러운 질문을 방지한다.
`weight_metabolic`, `pregnancy_postpartum`의 기간/상태는 향후 전용 Module에서 받는다.
...
CORE_02와 동일한 증상성 주소증에만 표시하며, 그 외 상담목적에서는 skip한다.
...
※ v0.2 주의: `skin` / `respiratory`는 CORE_01 첫 화면 선택지에서 제거되었으므로 아래 GEN_01의 show_if는
v0.2 CORE 흐름에서 성립하지 않는다. 해당 환자는 CORE_01D `other` → CORE_01E `primary_goal_other`로 수집된다.
GEN_01의 존치/폐기는 v0.3에서 `other` free text 사용빈도를 확인한 뒤 확정한다.
