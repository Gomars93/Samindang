...
### 검수 1번: 내가 과장했다 (PR #57 검수)

"이제 한약 단독도 재진 NRS 추적이 된다"고 적었는데 **절반만 맞다.**

| | 한약 단독 |
|---|---|
| 원장이 0~10으로 기록 | ✅ |
| 저장 | ✅ |
| 다음 방문 「지난번 추적」·재진 후보·이전 방문 원문 | ✅ |
| **그 방문에서 EMR 복사** | ❌ |
| **그 방문에서 재진 링크 발급** | ❌ |
| **그 방문에서 진료 완료** | ❌ |

EMR textarea·복사·재진 발급·진료 완료가 전부 `nextLaneFooter` 안에 있고
그게 `activeProfile !== 'herbal'` 가드에 걸린다. **PR-A가 만든 구멍**이고
(진료 완료는 원래 헤더에 있다가 Core Reduction P3에서 그 레인으로 내려갔다)
이 배치 범위가 아니다. 사실을 테스트 3개로 고정했다.

### PO 판단 요청 (새로 생긴 것)

**한약 단독 진료를 그 방문 안에서 끝낼 수 없다.** EMR 복사도, 재진 링크
발급도, 진료 완료도 화면에 없다. 셋 중 무엇을 되살릴지 정해주셔야 한다 --
「마무리」 단계를 herbal에도 만들어 거기 두는 방법, 또는 `nextLaneFooter`만
판단·처치 뒤에 붙이는 방법.
...
전부 고쳤다. 상세는 `DECISIONS.md` 첫 항목.

| | 내용 |
|---|---|
| **F1** | 마무리 화면에서 401 → 토큰 재입력 폼이 `hidden`이라 복구 불가. 최신 69가 만든 회귀 |
| **F2** | 재진 화면이 통증 집합만 넘겨 원장=텍스트 / 환자=버튼 |
| **F3** | `(내용 없음)`이 EMR에서 아무 말도 안 함 (EMR엔 배지가 없다) |
| **F4** | 라벨 변경 경계를 넘어 화살표 → **악화를 호전으로 표시 가능** |
| **F5** | 닿는 범위 과장 — 실제로는 mixed·재진 화면뿐(한약 단독엔 picker가 없다) |
| **F6** | 운동 카드 안내문에 옛 `'다음' 레인` 잔존 |

**여섯 건 다 `test:all` 녹색인 상태에서 나왔다.** 공통점: 내가 바꾼 것의
**반대편**을 테스트하지 않았다 — 숨긴 쪽에서 여는 UI / 세 번째 호출자 /
배지 없는 출력 / 입력이 아니라 표시 / 안 닿는 프로필 / 다른 파일의 같은 문구.

`doctor-workspace` 396 → **404단언**.

### 한약 태블릿 문진은 원래 되고 있었다

PO 질문에 답하자면 — 한약 문진 모듈(`SLEEP_*`·`GI_*`·`BOWEL_*`·`FATIGUE_*`·
`STRESS_*`·`WOMEN_*`·`WEIGHT_*`·`URINARY_*`)은 `coreSpec.ts`에 오래 전부터
있다. 이번 PR이 바꾼 것은 **재진 쪽 한 칸**(재평가 대상 값이 자유 텍스트 →
0~10)이다.

단, **한약 단독 진료는 재진 NRS 추적이 안 된다**(F5) — 그 picker가 PR-A로
폐기된 `다음` 레인 안에 있다. 되살릴지는 PO 판단.

---

## 2026-09-25 (최신 72): **한약 비교 행이 그려진다** — `기준값 7 → 4 ↓3`

**브랜치**: `claude/gracious-bell-epfxem` (커밋 3개 누적, PR 생성 예정).
상세는 `DECISIONS.md` 첫 항목.

### 한 것

「지난번 추적」 줄에 오늘 기준값을 나란히 놓는다. 둘 다 NRS일 때만, 산술만.

```
지난번 추적: 수면 불편 — 기준값 7 → 4 ↓3 · 속 불편 — 기준값 4 → 4
```

### 또 규모를 틀리게 말했다 (최신 71에서)

"`PriorVisitSummary`에 싣는 경로가 필요하다"고 적었는데 **이미 있었다** —
`followUpTargets` union이 baseline/직후값을 통째로 싣고 온다. 서버 무변경.
그리고 왼쪽 절반(`기준값 7`)은 **이미 한약 화면에 떠 있었다**. 없던 것은
오른쪽 절반뿐이라 새 컴포넌트가 필요 없었다.

이번 세션에서 규모·구현을 틀리게 말한 것이 네 번째다. 패턴이 같다 —
**만들기 전에 이미 있는 것을 먼저 찾지 않았다.**

### 검증

`test:all` exit 0 / `build` exit 0. `doctor-workspace` 384 → **396단언**.
뮤테이션 3회 전부 잡힘.

### Next Recommended Action

**1. PR 생성** — 커밋 3개(`65e6335` 마무리 이동 / `fe226f0` 한약 NRS /
이번 비교 행). 한 줄기라 한 PR로 묶는다.

**2. 한약 「간단 재확인」 카드 — PO 판단.** 이제 숫자가 「지난번 추적」 줄로
올라오므로 **카드를 내려도 되는 조건이 갖춰졌다**(최신 70에서 통증만 뺐고,
한약은 대체 경로가 없어서 남겼다).

**3. `최종 임상 판단` 제거** — `server/store.js`가 이 값으로
`pain_final_assessment_summary`를 만들고 두 화면에 뜬다.

**4. 2주 무재진 자동 링크 — PO 지시로 보류.** 남은 것은 자동 발송 하나.

**5.** 옛 `doctor.css` 폐기 / 컨택트 시트 `--profile` 분리.

### PO 확인 대기
- **환자 화면 눈금 양끝 문구** — `전혀 불편하지 않음 ~ 매우 불편함` 등은
  **내 초안**이다(최신 71).
- **한약 「간단 재확인」 카드** / **재진 화면 카드** (최신 70).
- **마무리 화면 동선** — 원장이 실제로 그 화면을 여는지(최신 69).
- **치료 초점 층 라벨** `증상 조절 · 가동성 확보 · 부하 능력` — 내 초안.
- 약침 **제제**(미주란·태반) 기록 위치 — 지금은 `기타`로 떨어진다.
- **안전 칩 색 대비** — `긴급 확인`이 진료실 모니터에서 충분히 띄는지(최신 67).

---

## 2026-09-25 (최신 71): **한약 재평가를 NRS 0~10으로** — 새 문항 0개

**브랜치**: `claude/gracious-bell-epfxem` (최신 70 커밋 `65e6335` 위에 이어서,
아직 PR 없음). 상세는 `DECISIONS.md` 첫 항목.

### 한 것

`수면 불편` · `속 불편` · `피로` 세 재평가 대상을 0~10 버튼으로 받는다.
`대변`은 일부러 텍스트로 남겼다(횟수는 강도가 아니다).

**새 문진 문항 0개.** 2026-09-06에 통증에 켠 `nrsTargetIds` 스위치를 한약에도
켠 것이 전부다 — 상수 하나 + prop 한 줄.

### 내가 틀렸던 과정 (기록용)

PO 질문 "한약 4지표로 충분한가"에 대해 내가 쌓아 올린 것: ① 처방별 證治要點
후보 ② 주호소별 재질문 목록 ③ 값 형식 3종 + 방향 지정. **세 개 다
폐기했다.** PO가 "복잡해지는 것 같은데 실무진들 스타일은?"이라고 물은 뒤
코드를 다시 보니 `finalAssessment.ts:99`에 답이 이미 있었다 — 2026-09-06
원장 지시 "재진시 추적항목만 체크 — NRS라던지". 한약은 그 prop을 안 넘기고
있었을 뿐이다.

앞 대화에서 내가 사실을 세 개 틀리게 말했다(통증 비교 행이 FollowUpTarget을
쓴다 / FollowUpTarget은 자유 텍스트다 / 전반적 변화는 자유 텍스트다).
정정은 `DECISIONS.md` 표에 있다.

### 라벨이 방향을 결정한다

`수면`(중립 명사)으로는 0~10 중 10이 좋은 쪽인지 알 수 없다. `수면 불편`이면
통증과 같은 한 방향(`↓ 좋다`)으로 읽힌다. 그래서 라벨 셋을 바꿨고, 환자 화면
눈금 양끝 문구도 target별로 갈랐다 — **통증 문구는 한 글자도 안 바꿨다**
(확립된 도구 문구이고, 바꾸면 통증 환자 화면이 조용히 달라진다).

### 검증

`test:all` exit 0 / `build` exit 0.
`doctor-workspace` 381 → **384단언** · `detail-check` 80 → **84단언**.
뮤테이션 3회 전부 잡힘.

### Next Recommended Action

**1. PR 만들기.** 브랜치에 커밋 2개가 쌓여 있다(최신 70 `65e6335` + 이번 것).

**2. 한약 프로필의 「간단 재확인」 카드 — PO 판단 대기.** 최신 70에서 통증만
뺐다. 이번 NRS로 한약에도 숫자가 생기므로 **판단 근거가 달라졌다** — 숫자가
브리핑 비교 행으로 올라가면 카드를 내려도 된다.

**3. 브리핑 비교 행에 한약 NRS를 싣기.** 지금 NRS는 원장 화면·환자 링크까지
왔지만, 통증처럼 `7 → 4 ↓3` 비교 행이 되려면 `PriorVisitSummary`에 싣는
경로가 필요하다(통증은 최신 68에서 했다). **이게 "추적 데이터화"의 마지막
한 칸이다.**

**4. `최종 임상 판단` 제거** — `server/store.js`가 이 값으로
`pain_final_assessment_summary`를 만들고 두 화면에 뜬다.

**5. 2주 무재진 자동 링크 — PO 지시로 보류.** 남은 것은 자동 발송 하나.

**6.** 옛 `doctor.css` 폐기 / 컨택트 시트 `--profile` 분리.

### PO 확인 대기
- **환자 화면 눈금 양끝 문구** — `전혀 불편하지 않음 ~ 매우 불편함` 등은
  **내 초안**이다. 라벨 셋은 승인받았지만 이 문구는 아니다(최신 71).
- **한약 「간단 재확인」 카드** / **재진 화면 카드** (최신 70).
- **마무리 화면 동선** — 원장이 실제로 그 화면을 여는지(최신 69).
- **치료 초점 층 라벨** `증상 조절 · 가동성 확보 · 부하 능력` — 내 초안.
- 약침 **제제**(미주란·태반) 기록 위치 — 지금은 `기타`로 떨어진다.
...
## 2026-09-24 (최신 70): **「간단 재확인」 카드가 확인 레인을 떠났다** (카드만)

**브랜치**: `claude/gracious-bell-epfxem` (PR #55 머지 후 새로 시작).
상세는 `DECISIONS.md` 첫 항목.

### PO 판단
C그룹 4개 중 **1번만** 뺀다 — 나머지는 그대로다.

| 칸 | PO 판단 |
|---|---|
| 재진 간단 문진 | **뺀다** (카드만) |
| 추가 문제 | 유지 — "포스트잇 좋네, 업세일링 가능" |
| 다시 검사하기 | 유지 |
| 지지/반증 | 유지 — "진료 확신 / 재검증 필요성 판단에 쓴다" |

### 한 것

카드를 확인 레인에서 빼서 「마무리」 참고 자료 서랍으로 내렸다. **기능은
그대로다** — 이 응답에서 비교 행(NRS), 「지난 대비」 한 줄, EMR §14.1 S가
전부 나온다. 카드 전용이던 prop 둘(`microFollowUpResponse`, `priorVisits`)은
`PainWorkspace`에서 빼고 `PainWorkspaceNext`로 옮겼다.

### 이동하면서 결함 하나가 드러났고, 떼기 **전에** 고쳤다

`microFollowUpQuoteLine`이 두 신고 분기를 `reported && note.trim()`으로 걸어서,
**신고는 했는데 내용을 안 적은** 환자의 이상반응이 조용히 `전반적 변화` 문장으로
바뀌어 나가고 있었다 — 좌측 요약에도, **EMR에도**. 카드가 확인 레인에 있는
동안은 그 카드가 저절로 펼쳐지며 가려주고 있었다. 카드를 떼면 진짜 구멍이 된다.

순서: 함수 수정 → 배지 추가 → 카드 이동.

### 배지

좌측 요약 「지난 대비」는 문장 하나만 싣는다. 같은 글자가 "이상반응으로 적은
것"인지 "근황"인지 구분이 안 되므로 `이상반응` / `새 증상` 배지를 문장 **앞**에
붙인다(줄이 잘리면 뒤가 먼저 사라지므로). 배지 우선순위는 문장 우선순위와
같음을 테스트가 직접 단언한다 — 갈라지면 원장이 다른 신고 내용을 읽는다.
새 색은 만들지 않고 기존 `--warning`을 쓴다.

### 검증

`doctor-workspace` 368 → **381단언**. 뮤테이션 3회 전부 잡힘.

### Next Recommended Action

**1. 한약 프로필의 같은 카드 — PO 확인 필요.** 한약 재평가 대상은 수면·소화·
대변·피로 4개이고, 통증의 NRS와 달리 **브리핑 비교 행 같은 대체 경로가 없다**
(한 줄에는 4개 중 하나만). 통증과 같은 판단을 자동 적용하지 않았다.

**2. 재진 화면(`RevisitWorkspace`)의 같은 카드 — 그대로 두기를 추천.** 그
화면에는 문진 응답이 없어서 이 카드가 「오늘 환자 입력」 블록의 전부다.

**3. `최종 임상 판단` 제거** — 전제 작업이 먼저(`server/store.js`가 이 값으로
`pain_final_assessment_summary`를 만들고 두 화면에 뜬다).

**4. 2주 무재진 자동 링크 — PO 지시로 보류.** 남은 것은 **자동 발송 하나뿐**.
손으로 발급하는 경로(「마무리」의 원내 태블릿 / QR / 메시징)는 이미 동작한다 —
최신 69까지 이 둘을 뭉뚱그려 적은 곳이 있었다.

**5.** 옛 `doctor.css` 폐기 / 한약 프로필 / 컨택트 시트 `--profile` 분리.

### PO 확인 대기
- **한약 카드 / 재진 화면 카드** (위 1·2).
- **마무리 화면 동선** — 원장이 실제로 그 화면을 여는지(최신 69).
- **치료 초점 층 라벨** `증상 조절 · 가동성 확보 · 부하 능력` — 내 초안.
- 약침 **제제**(미주란·태반) 기록 위치 — 지금은 `기타`로 떨어진다.
- **안전 칩 색 대비** — `긴급 확인`이 진료실 모니터에서 충분히 띄는지(최신 67).
...
## 2026-09-24 (최신 69): **「마무리」 화면 분리** — 진료 / 마무리 두 단계

**브랜치**: `claude/gracious-bell-epfxem` (PR #54 머지 후 새로 시작).
상세는 `DECISIONS.md` 첫 항목.

### 먼저, 내가 추천할 때 쓴 숫자를 바로잡는다

최신 68에 "LBP 화면이 4.65화면이라 효과가 가장 큰 남은 작업"이라고 적었는데,
4.65화면은 **화면 전체 길이**이지 이 작업이 줄이는 양이 아니다. 실측:

| 뷰포트 | 분리 전 | 분리 후 | 마무리 화면 |
|---|---|---|---|
| desktop 1440x900 | 4182px (4.65화면) | **3605px (4.01화면)** | 568px |
| tablet landscape | — | 3681px | 644px |
| tablet portrait | — | 3837px | 590px |

**0.64화면**이다. 한약 축소(1.41→0.97화면)와 같은 급이 아니다.

### 한 것

지시대로 분리하되, 이득이 0.64화면인 만큼 **분리의 비용을 사지 않았다**:

- **라우트/탭 신설 없음** — 두 단계가 같은 `workspaceState`를 공유한다.
  저장 경로도 타이밍도 그대로다.
- **언마운트 없음** — `hidden` 속성 하나로만 접는다. 조건부 렌더로 바꾸면
  마무리 안의 `<details>` 열림 상태와 latch가 화면을 오갈 때마다 날아간다
  (D-3이 그 부류였다).
- **새 컨트롤 없음** — 이미 있는 sticky 점프 내비가 단계 전환기를 겸한다.
  마지막 버튼 라벨만 「다음」 → 「마무리」. 지금 보고 있는 단계는
  `aria-current="step"`으로 표시된다.

옮긴 블록은 11개(재평가 대상 · 다음 방문 확인 메모 · 다음 액션 · 관리 계획 ·
다음 상세 재평가 · 이전 방문 · 환자 전달문 · EMR 미리보기 · HerbalWorkspaceNext ·
CRM 복약 코스 · 재진 발급/메시징/종결). **버린 값은 없다** — 필드 × 화면 표는
`DECISIONS.md`에 있다. 한약 단독은 PR-A로 이미 이 레인이 없어 그대로 영향 없음.

### 이 설계의 진짜 위험은 CSS 한 줄이다

`hidden`이 듣는 근거는 UA 규칙 `[hidden] { display: none }` 하나뿐이라,
`.doctor__visitStep`에 `display`를 주는 규칙이 생기면 **숨겼다고 믿은 화면이
그대로 붙는데 SSR 단언은 전부 통과한다.** 두 겹으로 막았다 — 소스 CSS 스캔
(W-C5)과 **진짜 브라우저 실측 높이 0px**. 선택자 이름을 우회한 규칙
(`div[data-step]`)을 실제로 넣어, W-C5는 통과하고 브라우저 쪽만 실패하는 것을
확인했다.

절대 px 상한은 일부러 안 박았다: 기계 간 여유를 주면 분리를 되돌려도 통과하는
무의미한 핀이 되고, 안 주면 흔들린다.

### 검증

`test:all` exit 0 / `build` exit 0.
`doctor-workspace` 337 → **367단언** · `tablet-viewport` 93 → **132단언**.
뮤테이션 4회 전부 잡힘.

### Next Recommended Action

**1. C그룹 5개 판단 — PO 확인 필요.** MicroFollowUp / AdditionalConcern /
StructuredReassessment / SupportContradiction / PriorVisitHistory — 진료 중
실제로 보시는지. 안 보시면 「마무리」로 옮기거나 접는다. 이제 옮길 자리가
실제로 생겼으므로 이게 가장 앞이다.

**2. `최종 임상 판단` 제거** — 전제 작업이 먼저. `server/store.js`가 이 값으로
`pain_final_assessment_summary`를 만들고 그게 두 화면에 뜬다. 임상 가설 요약을
저장하는 새 필드가 필요하다(서버가 부위팩 `labelKo`를 못 읽는다).

**3. 2주 무재진 자동 링크 — PO 지시로 보류.** 남은 것은 **예약 발송 하나뿐**.

**4.** 옛 `doctor.css` 폐기 / 한약 프로필 / 컨택트 시트 `--profile` 분리.

### PO 확인 대기
- **마무리 화면 동선** — 원장이 실제로 그 화면을 여는지. 「진료 완료」 버튼이
  마무리 안에 있어서 방문을 닫으려면 가게 되지만, 강제는 아니다(최신 69).
- **치료 초점 층 라벨** `증상 조절 · 가동성 확보 · 부하 능력` — 내 초안.
...
## 2026-09-24 (최신 68): **재진 비교 행이 살아났다** — 이전 NRS를 싣는다

**브랜치**: `claude/gracious-bell-epfxem` (PR #53 머지 후 새로 시작).
상세는 `DECISIONS.md` 첫 항목.

### 한 것
```
지금 통증     ────────  7 → 4    ↓3     ← 처음으로 그려진다
가장 아플 때   ────────  9 → 6    ↓3
```
`nrsRow`의 비교 행 코드는 NRS 도입(9/22) 때부터 있었는데 **값을 나르는 경로가
없었다** — `PriorVisitSummary`에 필드가 없었다.

- **서버**(`server/store.js`)가 초진은 submission 응답에서, 재진은 재질문된
  `detailAnswers`에서 읽어 싣는다.
- **읽을 수 없는 값은 null**. 판정은 화면 쪽 `readScale0to10`과 **같은 규칙**
  (number · 유한 · 정수 · 0~10). `Number(['9']) === 9` 함정 때문이고, 그 사고는
  이 저장소에 이미 두 번 있었다. `|| null`을 안 쓴 이유는 **NRS 0이 유효**해서다.
- **`priorNrsFromHistory`**가 비교 대상을 고른다 — **값이 있는 가장 최근 방문
  하나**에서 **두 값을 함께**. 빈 방문은 건너뛰고(차트 읽는 방식), 필드별로
  따로 찾으면 두 행이 서로 다른 시점이 된다.

### 곁가지로 고친 것
**SYNTHETIC 미리보기가 NRS 도입 때 갱신되지 않았다.** 통증 시나리오 넷에
`PAIN_03`/`PAIN_03B`가 없어 미리보기가 계속 **미응답**으로 떴다 — 실제 환자
화면과 어긋나 있었다. 테스트의 비공허성 단언이 즉시 잡았다.

### 공허한 단언을 만들었다가 뮤테이션이 잡았다
`!html.includes('10 →')`로 썼는데 렌더에서 값과 화살표는 **다른 `<span>`**이라
그 문자열은 절대 안 나타난다. 행의 `data-kind`로 판정하도록 고치고 **매처 자체의
생존**도 단언했다.

### 검증
`test:all` exit 0 / `build` exit 0.
`server` history NRS **13단언** 신설 · `doctor-workspace` 325 → **337단언**.
뮤테이션 4회(그중 하나는 처음에 안 잡혀 단언을 고쳤다).
컨택트 시트 — `data-kind="delta"` 2건, 실제 비교 행 확인.

### Next Recommended Action

**1. 「마무리」 화면 분리** — EMR·환자전달·QR·메시징·복약·재평가 7블록을 진료
화면에서 뺀다. 한약에서 이미 검증된 수술(최신 52, 1.41→0.97화면). LBP 화면이
4.65화면이라 효과가 가장 큰 남은 작업이다.

**2. C그룹 5개 판단 — PO 확인 필요.** MicroFollowUp / AdditionalConcern /
StructuredReassessment / SupportContradiction / PriorVisitHistory — 진료 중
실제로 보시는지. 안 보시면 「마무리」로 빠지거나 접힌다.

**3. `최종 임상 판단` 제거** — 전제 작업이 먼저. `server/store.js`가 이 값으로
`pain_final_assessment_summary`를 만들고 그게 두 화면에 뜬다. 임상 가설 요약을
저장하는 새 필드가 필요하다(서버가 부위팩 `labelKo`를 못 읽는다).

**4. 2주 무재진 자동 링크 — PO 지시로 보류.** 미결 논점은 최신 67에 적어뒀다.
이제 남은 것은 **예약 발송 하나뿐**이다(NRS 싣기는 이 작업에서 끝났다).

**5.** 옛 `doctor.css` 폐기 / 한약 프로필.
...
**2. 2주 무재진 자동 링크 — PO 지시로 보류(2026-09-23).**

설계는 최신 63에 확정돼 있고 코드는 한 줄도 쓰지 않았다. 재개할 때 필요한 것은
**둘뿐**: 예약 발송(`messagingStore.js`에 `sendAt` 0건),
`PriorVisitSummary`에 NRS 싣기.

보류 시점에 남은 미결 논점 — 재개하면 여기서 시작한다:

- **오발송 허용 여부**가 유일한 갈림길이다. opt-out(2주 무재진 전원 발송)이면
  종결 칩을 안 누른 환자에게 문자가 가고, opt-in(재평가 지정한 환자만)이면
  지정 안 한 환자(대부분)에게 영영 안 간다.
- **종결 여부를 확실히 체크할 방법이 없다.** 신뢰 가능한 신호는 "그 사이
  방문했다"(서버가 방문 기록을 가짐)와 "수신 거부" 둘뿐이다.
- **`진료 완료` 버튼(P0-5)과 서버 `status: 'completed'`는 쓸 수 없다.** 그건
  "오늘 이 방문의 진료가 끝났다"는 뜻이라 **매 방문** 눌린다. 종결 신호로 쓰면
  모든 환자가 종결로 잡혀 문자가 아무에게도 안 간다 — 같은 단어 두 뜻.
- 명시적 종결은 `NextReassessmentPlan.status`에 값 하나를 더해 받는다(새 UI
  없음, `Record<Status, Label>`이 컴파일 타임 게이트). 다만 이 칩도 대체로
  안 눌릴 것이므로 **문자를 막는 주 장치가 아니라 예외 처리**다.

**2-1. `PriorVisitSummary`에 NRS 싣기는 따로 할 수 있다.** 발송과 독립이고,
이것만 해도 재진 비교 행(`8 → 5 ↓3`)이 살아난다. 지금 그 행은 코드가 이미
있는데 데이터가 없어서 안 그려진다.
...
상세는 `DECISIONS.md` 같은 날 두 항목.

### 한 것 — 처치 칩 8개 → 6개
```
전: 침 · 약침 · 부항 · 추나 · 물리치료 · 한약 · 테이핑 · 운동처방
후: 약침(소염) · 약침(재생) · 추나 · 도침 · 매선 · 운동처방
```
- **뺀 이유**(PO): 침·부항은 기본 치료(매번 켜는 칩은 정보가 없다) /
  물리치료·테이핑은 안 씀 / 한약은 통증에서 거의 안 씀.
- **더한 것**: 도침·매선 — PO 술기 스택 2층인데 목록에 애초에 없었다.
- **약침을 둘로**: PO 서술이 "염증 잡는 약침"과 "조직회복 약침"을 구분한다.
  제제명(미주란·태반)이 아니라 **목적**으로 나눴다 — 지식베이스가
  "재료·제제가 다르면 근거를 전용하지 않는다"고 못박아서, 제제는 별도 열
  (미구현). 순서는 Task–Load–Capacity 층 순.

### 조용한 소실 0건 — 제거 단어당 테스트 1개
`parseInterventionValue`가 미지 토큰을 `기타`로 보존하므로 설계상 안전하지만,
"설계상 안전"은 산문 주장이다. 제거한 6단어 각각 + 옛 조합 + 옛값·새칩 혼합까지
단언했다. **뮤테이션**(미지 토큰 버리게 수정) 시 314 → 264로 떨어지고 첫 실패가
정확히 "조용한 소실"이다. 스키마·EMR 출력 모양 무변경.

### 설계 확정 — 2주 무재진 자동 링크 (구현은 별도 PR)
- **종결 선언 버튼을 만들지 않는다.** fade-out이 대부분이라 안 눌린다.
- **opt-out**: 마지막 방문 +14일, 그 사이 재방문 없으면 1회 발송. 재평가 시점이
  지정돼 있으면 그 날짜를 쓴다. 못 보내는 손실 > 잘못 보내는 손실.
- **막는 것 넷**: 그 사이 방문(발송 직전 재검증) / `NextReassessmentPlan`에
  `COMPLETED` 값 추가 / 의뢰·전원 / 수신 거부(메시징이 담당).
- **재발송 없음** — 무응답 자체가 데이터(이탈 신호)다.
- **타이밍 ①(즉시 retest)은 만들지 않는다**(PO). 따라서 `immediateRetestTarget`
  칸도 불필요 — 그게 ①의 입력칸이었다. 무엇을 물을지는 `FollowUpTargetPicker`가
  이미 담당한다.
- 새로 만들 것은 **둘뿐**: 예약 발송(`messagingStore.js`에 0건), 돌아온 NRS를
  `PriorVisitSummary`에 싣기.

### 검증
`test:all` exit 0 / `build` exit 0. `doctor-workspace` 306 → **314단언**.
...
| 블록 | 내용 |
|---|---|
| EXAM | 검사 칩 5값(승인 enum) **+ 좌/우** |
| ASSESSMENT | 임상가설 칩 · **치료 초점 = T-L-C 층 칩 3개** · ~~최종 임상 판단~~ ~~즉시 재검 대상~~ |
| PLAN | 처치 칩 6개 + 좌/우 · **재평가 시점 칩**(+`COMPLETED`) · 운동 채택 |
| 자유입력 | **0개** |

주의 — `finalWorkingAssessment` 편집 UI만 지우면 **EMR은 계속 그 값을 읽는다**
("읽히는데 못 고치는 필드" = Batch 2.6 D-1과 같은 모양). EMR 출력 경로까지
필드 × 화면 표로 함께 다룬다.

**2. 2주 무재진 링크 구현** — 배선 2단계 머지 후 별도 PR. 한 PR에 섞으면 검수 불가.

**3.** 재진 비교 행 — `PriorVisitSummary`에 NRS를 실으면 자동으로 살아난다(2와 같은 PR).

**4.** 「마무리」 화면 분리 / C그룹 5개 판단 / 옛 `doctor.css` 폐기 / 한약 프로필.

### PO 확인 대기
- **운동처방 칩**을 남길지 — `PainExerciseSection`과 역할이 겹칠 수 있다.
  지시받지 않아 그대로 뒀다.
- 약침 **제제**(미주란·태반·봉약침…)를 별도 열로 기록할지, 기록한다면 어디에.
...
새 첫 화면 `VISIT_00_INTENT='pain_care'`로 들어온 환자에게 **HIP_00·HFJ_00과 그 뒤
안전 문항 전체가 뜨지 않고** `clinical_flags.hip`/`.tmj`가 `null`로 남던 회귀.
`hipQuestions.ts`/`tmjQuestions.ts`가 정규화를 거치지 않은 raw `VISIT_01` /
`VISIT_02_SYMPTOM_MAIN` 게이트를 복제해 쓰고 있었다. LBP 문항은 정상으로 떴기 때문에
화면상으로는 멀쩡해 보였다. 상세는 `DECISIONS.md` 같은 날 항목.

### 한 것
- `src/spec/visitRouting.ts` 신설 — `visitGoal`/`effectiveSymptomMain`/
  `effectiveConstGoal`/`primaryConcernKey`/`hasDetailedConcern`/`IS_PRIMARY_PAIN`을
  coreSpec에서 **글자 그대로** 옮겼다. coreSpec이 hip/tmjQuestions를 import하므로
  반대 방향은 순환 — 양쪽이 의존할 수 있는 하위 모듈이어야 드리프트가 구조적으로
  불가능해진다.
- `hipQuestions.ts` / `tmjQuestions.ts` — 복제 게이트 제거, `IS_PRIMARY_PAIN`만 사용.
- `coreSpec.ts` — 옮긴 정의 삭제 + import, `primaryConcernKey`는 re-export로 호환 유지.

### 임상 규칙 무변경
문항·enum·threshold·`IS_PRIMARY_HIP_SAFETY`/`IS_PRIMARY_TMJ_SAFETY`의 판정 조건은
한 줄도 바뀌지 않았다. 바뀐 것은 **누가 그 문항을 보는가**뿐이다.

### 비용을 숨기지 않는다
허리·골반 프로필 3종 **+1화면 / +2탭**(23→24, 25→26, 22→23). 늘어난 칸은 새 기능이
아니라 원래 물었어야 하는 안전 판별자다. 그 외 프로필은 전부 무변화.

### 검증
`test:all` exit 0 / `build` exit 0. 신설 62단언(`tests/visit-route-parity.spec.mjs`,
`test:all`에 등록) + **뮤테이션 3회**(hip 되돌림 16실패 / tmj 되돌림 18실패 /
정규화 무력화 45실패).

### 과거 데이터
PO 판단으로 무시(2026-09-22). 마이그레이션·재문진 배지 만들지 않는다.

### 병렬 트랙 경고 — `codex/pain-questionnaire-v2-ux`
origin에 **main 위 19커밋**짜리 문진 v2 브랜치가 따로 살아 있다(0 behind).
`PAIN_F01`(목표 활동)/`PAIN_F02`(0~10 수행능력), `OptionCard` 공통화,
`screenProgress` 등. main 쪽 HANDOFF에는 이 사실이 기록돼 있지 않았다(최신 55까지
문진 재설계는 "PR-C 미착수"로 적혀 있었다). 이 PR-1은 그 브랜치에 섞여 있던 HIP
수정만 떼어낸 것이고, **TMJ는 그 브랜치가 놓친 부분이라 여기서 함께 고쳤다.**

### Next Recommended Action
1. **PR-2 (refactor)** — `OptionCard` 공통화 + `ScreenShell`/`screenProgress`.
   임상 0줄. codex 브랜치에서 가져오되 커밋 히스토리는 재구성(중복 커밋 3+2개 존재).
2. **PR-3 (feat)** — `PAIN_F01`/`F02`. 단, `activity_tolerance: null` 하드코딩은
   빼고 간다 — 키가 있는데 값이 영원히 null이면 나중에 `'key' in payload`로 존재
   여부를 판단할 때 조용히 틀린다.
3. **닥터뷰 Figma 재설계** — PO가 기존 UI/UX를 새로 설계하기로 결정(2026-09-22).
   착수 전 결정 필요: 대상 화면 범위, 기존 워크스페이스 레인 구조(레인1 안전 확인 /
   레인2 관찰)를 유지할지, PR-B3(Layer 0)를 재설계 이후로 미룰지.
4. **`PAIN_F02` 척도 방향** — PO가 추천안 위임(2026-09-22). 기존 통증 NRS와 방향이
   반대라 같은 문진에 0~10 척도 2개가 연달아 나오면 혼동한다. **5단계 언어 척도**로
   가는 안을 PR-3에서 제시 예정.
5. **`PAIN_F04`(제한 이유)** — PO가 보류 승인(2026-09-22). F01/F02가 닥터뷰에
   실제로 도달하는 것을 먼저 닫는다.
...
왜사설·강경설 / 반발압통·근성방어 / 박동성 복부 종괴 — 설진·복진 중에 걸리지만 의미가
"변증 참고"가 아니라 **"상담을 멈추고 보내야 함"**인 셋. 2층(`＋ 전체`)에 넣으면 영원히
안 열리므로, **접히지 않는 레인1(안전 확인)**에 뒀다.

### 한 것
- `RED_FLAG` 카테고리 + 3항목 카탈로그(전부 1층, 2층 없음) + 툴팁.
- `WorkspaceState.herbalSafetyObservation` **additive 필드** 추가(배열이 아닌 단일 항목 —
  레인2 배열에 넣으면 "확인 필요 N건" 카운터와 접기 규칙이 안전 항목까지 접는다).
- `HerbalSafetyRedFlagCard` 신설 → 레인1, herbal·mixed에서만.
- EMR: 체크 시 **맨 앞** `⚠ 상담 중단 사유:`, 미체크 시 라벨 자체 없음.

### 환자 설문지 불변 (PO가 명시적으로 물은 경계)
`src/spec/` 0줄. 환자는 이 소견을 스스로 답할 수 없다 — 전적으로 원장이 진찰 후 체크.
화면에도 명시했고 테스트 §C가 고정한다.

### 비용을 숨기지 않는다
레인1에 항상 보이므로 기본 화면이 **871 → 933px(+62px, 1.04화면)**. 첫 구성은 +125px
였고 한 줄로 합쳐 줄였다. 더 줄이지 않는다 — 안전 정보를 읽기 어렵게 압축하면 존재
이유가 없어진다. 카드 높이 100px 예산을 테스트로 고정했다.

### 검증
`test:all` exit 0 (6993 단언) / `build` exit 0. 신설 27단언 + 뮤테이션 2회.

### Next Recommended Action
1. **PR-B3 (Layer 0)** — 재평가 화면에서 지난번 체크 항목을 맨 앞 + `지난번 ✓`.
   **차트번호 연결(최신 53)이 선행 조건.** PR-B1의 파서로 지난 `value`를 칩으로 복원해
   비교하면 된다.
2. **PR-A2 (진료 녹취 화면 제거)** — 갤럭시폰 시그마 external note로 대체 확정.
   **화면만 떼고 서버 recorder API는 2주 남긴다.**
3. **PR-E (옴니핏)** — PNG 전용·CSV 불가·큰 숫자만. Tesseract.js `eng.traineddata` 번들 +
   `langPath` 로컬 고정 필수(인터넷 비노출 LAN).
...
### EMR 기록 원칙 (PO 합의, 2026-09-21)
- **닥터뷰 EMR = 사실** / **시그마 녹취 요약 = 맥락**
- 순서: 닥터뷰 EMR **먼저**, 녹취 요약 **뒤**. AI 요약은 검수 없이 붙이지 않는다.

## 2026-09-21 (최신 54): **설맥복 체크식 (PR-B1)** — 3층 구조 + 호버 툴팁 + 승격 제안

**브랜치**: `claude/feat-observation-checklist`.

### 한 것
- `observationOptions.ts` 신설 — 설 24 / 맥 22 / 복 15 항목 카탈로그(1층 23칸), 감각 묘사
  툴팁, 배타 축(부·침 / 삭·지), 직렬화·파싱 함수.
- `observationPromotion.ts` 신설 — 2층 3회 사용 시 **승격 제안**(자동 재배치 금지),
  localStorage, 실패 시 조용한 무력화.
- `ObservationTooltip.tsx` 신설 — 150ms 등장 / 즉시 소멸, PC 전용(키보드 포커스는 지원).
- `ClinicianObservationChecklist.tsx` 재작성 — 설·맥·복은 칩, 추가 문진·기타는 기존 자유입력.
- **데이터 모델 무변경** — 칩 선택을 `value` 문자열로 직렬화. persistence·EMR·기존 레코드
  전부 그대로. 옛 자유입력은 `기타` 칸으로 복원된다.

### 실측 (헤드리스, 펼친 상태를 새로 측정)
PC 871→1225px(+354) · 태블릿 가로 727→1231px(+504) · 태블릿 세로 1079→1457px(+378).
모두 2화면 이내. 측정이 두 문제를 잡았다: 한 글자 맥 칩 34px(하한 위반) → `min-width:44px`,
펼침 1525px → 한 wrap 줄로 합쳐 1225px.

### 검증
`test:all` exit 0 (6960 단언) / `build` exit 0. 신설 68단언 + 뮤테이션 3회.

### PO 감수 필요
**툴팁 문안 62개는 제가 쓴 초안이다.** 표준 교과서의 감각 묘사를 기준으로 했으나 원장
감수 전까지 초안으로 취급한다. 변증 귀속은 의도적으로 넣지 않았다(테스트가 차단).

### Next Recommended Action
1. **PR-B2 (red flag 3칸)** — 왜사설 / 반발압통·근성방어 / 박동성 종괴를 `안전 확인`
   레인에. 이 셋은 "변증 참고"가 아니라 "한약 상담 중단하고 보내야 함"이라 접힌 서랍에
   두면 안 된다. 환자 설문지에는 들어가지 않는다(원장이 직접 체크).
2. **PR-B3 (Layer 0)** — 재평가 화면에서 지난번 체크 항목을 맨 앞 + `지난번 ✓`.
   **차트번호 연결(최신 53)이 선행 조건**이다.
3. **PR-A2 (진료 녹취 화면 제거)** — 갤럭시폰 시그마 external note AI 요약으로 대체 확정.
   **화면만 떼고 서버 recorder API는 2주 남긴다**(되살리기 비용 0 유지).
4. **PR-E (옴니핏)** — PNG 전용·CSV 불가·큰 숫자만. Tesseract.js는 기본값이 CDN에서
   언어 데이터를 받으므로 `eng.traineddata` 번들 + `langPath` 로컬 고정 필수.
5. PR-C(문진 문항 재설계), PR-D(재처방 체크 링크).

### EMR 기록 원칙 (PO 합의, 2026-09-21)
- **닥터뷰 EMR = 사실**(문진·설맥복·판단 4칸, 재평가 비교 대상)
- **시그마 녹취 요약 = 맥락**(환자가 한 말, 재사용 안 함)
- 순서: 닥터뷰 EMR **먼저**, 녹취 요약 **뒤**. AI 요약은 검수 없이 붙이지 않는다.

### PO 쪽 미완료
- 툴팁 문안 62개 감수
- 시그마 external note AI 요약의 **음성 데이터 처리 방식 확인**(기기 내 처리인지 외부
  전송인지 — 환자 동의 필요 여부가 갈린다)
- 저장소 폴더를 Google Drive 동기화에서 제외 → 그 다음 Drive에서 `.env.local`/`.data/` 삭제
- `docs/REAL_DEVICE_PILOT_CHECKLIST.md` §5-c 한약 경로 end-to-end
...
4. **PR #35 main 병합 완료(2026-09-20)** — PO "머지 부탁해".
5. **PO 후속 지시 3건 처리(같은 날)**:
   - *"닥터뷰에 태어난시 외에는 필요없어. 명리 계산 영역 없애줘"* → **이미 제거되어 있었다.**
     Batch 4.1-B(2026-09-04 PO 결정, "별도 명리 프로그램을 쓴다")에서 「명리」 아코디언
     (`MyungriCompactCard` + 「명리 검토」 reviewGrid)이 통째로 삭제됐고, 지금 원장 화면의
     출생 블록은 `BIRTH_01` 생년월일 / `BIRTH_02`·`BIRTH_02A` 양음력·윤달 /
     **`BIRTH_03` 출생 시간대** / `BIRTH_03A` 시간 정확도 5개뿐이다. 사주 기둥·오행·
     정책 대기 표시는 화면 어디에도 렌더되지 않는다(JSX 호출처 0건, grep 확인).
     `MyungriCompactCard`/`sajuStatusLine`/`myungriGroupCount`는 **production 호출처 0**인
     채로 남아 있고(`tests/doctor.spec.mjs`가 직접 렌더해 검증), `payload.myungri_calculation`도
     계산·저장은 그대로다 — **추가 제거는 하지 않았다**(payload 계약·judgment source
     메타데이터를 건드리는 변경이라 이득 없이 리스크만 생긴다).
   - 드라이브 `.env.local` 내용 확인: `VITE_SAMINDANG_SERVER_URL` 한 줄뿐, **토큰 없음** →
     자격증명 재발급 불필요. 삭제는 **PC에서 동기화 제외를 먼저 한 뒤**에 해야 한다 —
     미러 동기화 상태에서 드라이브 쪽을 지우면 PC의 원본까지 삭제된다.
   - 남은 미결: 동기화 제외(PC 작업) → 드라이브 사본 정리 순서로 진행.
