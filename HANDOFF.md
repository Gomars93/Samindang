...
수정: herbal 분기가 통증 판단에 글자가 있으면 통증 블록을 함께 낸다(합성 순서·
구분자는 mixed와 동일, 새 형식 없음). 판정은 `recordedAt`이 아니라 **내용**
기준(`hasPainFinalAssessmentText` 신설) — 저장 전이라도 잃으면 안 된다.
B-0a~B-0e + mutation 3종. B-0e는 타입의 문자열 키를 열거해 **칸이 늘어도 자동
커버**한다.
...
검증: `herbal-workspace-slim` 37→**52** · `doctor-workspace` 408→**417** ·
`tablet-viewport` 138→**159**(3 뷰포트 실측) · `doctor-reset-key` 12 ·
`tsc -b` 0 · `build` 0 · **`test:all` exit 0 (7222 단언)**.
Mutation **21종** 전부 이름 있는 실패로 잡힘 — 그중 검수 후 추가한 3종은
