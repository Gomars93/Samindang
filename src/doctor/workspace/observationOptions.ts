...
/**
 * Red flag (PR-B2, 2026-09-21) — 설진·복진 중에 걸리는 소견 가운데 의미가
 * **"변증 참고"가 아니라 "한약 상담을 멈추고 보내야 함"**인 것들.
 *
 * 왜 2층이 아니라 별도 카테고리인가: 이 셋을 `＋ 전체` 서랍에 넣으면
 * **영원히 안 열린다.** 접힌 서랍에 안전장치를 넣는 것은 안전장치를 넣지
 * 않은 것과 같다. 그래서 다른 칩들과 같은 타입·직렬화를 쓰되 렌더 위치를
 * 레인1(안전 확인)로 분리한다.
 *
 * 이 파일의 다른 카탈로그와 같은 규칙이 그대로 적용된다 -- 툴팁은 **무엇이
 * 보이고 만져지는가**까지만 적고, 어떤 질환을 시사하는지 단정하지 않는다.
 * 대신 "왜 여기 있는가"(= 상담을 멈출 사유가 될 수 있다)는 적는다. 그것은
 * 변증 규칙이 아니라 이 항목이 존재하는 이유 자체이기 때문이다.
 *
 * 세 항목 모두 tier 1 -- 2층이 없다. 안전 항목에 "더 보기"는 없어야 한다.
 */
export const RED_FLAG_OPTIONS: ObservationOption[] = [
  {
    label: '왜사설·강경설',
    tier: 1,
    tooltip:
      '혀를 내밀게 했을 때 한쪽으로 치우치거나(왜사), 뻣뻣하게 굳어 잘 움직이지 않는 상태(강경). 새로 나타났다면 한약 상담보다 신경학적 평가가 먼저다.',
  },
  {
    label: '반발압통·근성방어',
    tier: 1,
    tooltip:
      '복부를 누른 손을 뗄 때 더 아파하거나(반발압통), 누르려 할 때 복벽이 반사적으로 단단히 굳는 상태(근성방어). 급성 복증에서 보이는 소견으로, 확인되면 상담을 중단하고 의뢰한다.',
  },
  {
    label: '박동성 복부 종괴',
    tier: 1,
    tooltip:
      '복부에서 맥박에 맞춰 스스로 뛰는 덩어리가 만져지는 상태. 정상 대동맥 박동(제상계)과 달리 **덩어리 자체가** 좌우로 밀려 나오는 느낌이다. 확인되면 촉진을 멈추고 의뢰한다.',
  },
]
