 ...
 * 상태를 가리키지만, 그 상태에서 **고부하 운동을 시작한다**는 뜻은 아니다.
 * v0.3은 TBC 3단계를 그대로 가져오면서 그 아래를 비워둬, 급성 protective
 * muscle guarding 환자에게도 반드시 1단계 운동 9개가 배정됐다 — 설계 실수였다.
 *
 * 0단계는 "오늘은 **구조화된 고부하 운동을 유예한다**"는 명시적 선택지다.
 * 쉬게 하는 단계가 아니다 — 걷기·일상활동과 통증 없는 범위의 움직임은 그대로
 * 유지하고, 다음 진료에서 자극 민감도(irritability)와 위험신호를 다시 확인해
 * 운동 시작 시점을 정한다(`LBP_STAGE_0_GUIDANCE_KO`).
 *
 * 2026-09-19 개정 — 이전 문구는 "오늘은 능동 운동을 처방하지 않는다"였다.
 * 딥리서치 `DR_07_재활_v1.md` §11-9는 급성 요통에서 active approach 자체는
 * 타당하고 불확실한 것은 **초기 고강도 structured exercise의 추가이득**이며,
 * "3일/1주 무조건 쉰다" 같은 고정 기간보다 irritability·red flag로 조절하라고
 * 본다. 유예 대상은 능동 운동 전부가 아니라 고부하 loading이다.
 * 상세는 `docs/LBP_EXERCISE_STAGE_ASSIGNMENT_v0.4.md` §2-1.
 *
...
/** 0=보호·안정(고부하 운동 유예), 1=증상 조절, 2=움직임 조절, 3=기능 최적화. */
 ...
 *
 * 이 문장에 "능동 운동을 처방하지 않는다"류의 표현을 다시 넣지 않는다.
 * 유예 대상은 고부하 loading이며 일상활동 유지는 0단계에서도 처방의 일부다.
 * `tests/lbp-exercise-stage.spec.mjs`가 이 두 조건을 소스 텍스트로 단언한다.
 */
export const LBP_STAGE_0_GUIDANCE_KO =
  '오늘은 구조화된 고부하 운동을 유예합니다. 걷기·일상활동과 통증 없는 범위의 움직임은 그대로 유지하시고, 편안한 자세(눕기·무릎 세우고 눕기)와 호흡을 함께 안내합니다. 다음 진료에서 자극 민감도와 위험신호를 다시 확인해 운동 시작 시점을 정합니다.'
